...
type ToolInvocationResult = ZhipuCogVideoToolResult | {
...
describe('ZhipuAI CogVideo', () => {
  ...
  it('polls a processing task and uploads video and cover artifacts on success', async () => {
    ...
    expect(normalized.artifact.files).toEqual(expect.arrayContaining([
  ...
  it('throws a provider task error', async () => {
    ...
    const query = getTool('zhipu_cogvideo_query')
    await expect(invokeTool(query, { task_id: 'task-fail', wait_seconds: 0 })).rejects.toThrow(
      'content rejected'
    )
  })
...
async function invokeTool(toolInstance: { name: string; invoke: (input: unknown) => Promise<unknown> }, args: Record<string, unknown>) {
