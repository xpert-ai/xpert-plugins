...
> **Peer Dependencies**: Ensure your host service also provides `@xpert-ai/plugin-sdk`, `@nestjs/common`, `@nestjs/config`, `@xpert-ai/contracts`, `@langchain/openai`, `i18next`, `chalk`, `zod`, and `tslib`. Refer to `package.json` for exact versions.
