...
export const Xinference = 'xinference';
...
export type XinferenceModelCredentials = {
	server_url?: string;
	model_uid?: string;
    api_key: string;
...
export function toCredentialKwargs(credentials: XinferenceModelCredentials, model: string) {
    const credentialsKwargs: OpenAIBaseInput = {
		apiKey: credentials.api_key || 'no-key-required',
		model: credentials.model_uid || model,
	} as OpenAIBaseInput
	const configuration: ClientOptions = {}

	if (credentials.server_url) {
		const openaiApiBase = credentials.server_url.replace(/\/$/, '')
