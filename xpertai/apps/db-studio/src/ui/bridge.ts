...
export function request(type: string, body: JsonRecord): Promise<JsonRecord> {
...
export const command = async (commandKey: string, payload: unknown) => {
  const response = await request('invokeClientCommand', { commandKey, payload })
