...
export class StudioService implements OnModuleDestroy {
  ...
  async purgeExpiredResults(scope: StudioScope, now = Date.now()) {
    const executions = await this.records.find({
      where: { ...scopeWhere(scope), kind: 'execution' },
      take: 500,
    })
    const expired = executions.filter((item) => {
      const expiresAt = item.payload.expiresAt
      return typeof expiresAt === 'string' && Date.parse(expiresAt) < now && 'result' in item.payload
    })
    await Promise.all(
      expired.map((item) => {
        ...
        return this.records.update(
          ...
          { payload: updatePayload({ ...retained, resultExpired: true }) }
        )
      })
    )
  ...
  async list(scope: StudioScope, kind: StudioRecordKind, page = 1) {
    ...
    const items = await this.records.find({
      where: { ...scopeWhere(scope), kind },
      order: { updatedAt: 'DESC' },
      skip: (Math.max(1, page) - 1) * 50,
      take: 50,
    })
    return items
      .filter((item) => !item.dataSourceId || bound.has(item.dataSourceId))
      .map(({ payload, ...item }) => ({
        ...item,
  ...
  async approve(scope: StudioScope, id: string, digest: string, approve: boolean) {
    ...
    if (policy.readOnly) throw new Error('connection_read_only')
    if (policy.revision !== payload.policyRevision || Date.parse(payload.expiresAt) < Date.now())
  ...
  async executePlan(scope: StudioScope, id: string) {
    ...
    const policy = await this.policy(scope, plan.dataSourceId!)
    if (policy.readOnly || policy.revision !== payload.policyRevision || Date.parse(payload.expiresAt) < Date.now())
      throw new Error('plan_policy_expired')
...
const requireSourceEditPermission = () => {
  try {
    if (!RequestContext.hasPermission(PermissionsEnum.DATA_SOURCE_EDIT))
      throw new Error('data_source_edit_permission_required')
  } catch (error) {
    if (error instanceof Error && error.name === 'JsonWebTokenError') {
      console.warn('[db-studio] DATA_SOURCE_EDIT check skipped (OIDC RS256 vs HS256 verify):', error.message)
      return
    }
    throw error
  }
