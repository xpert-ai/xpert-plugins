...
// Build supplies the pinned font; installed previews never download or use system fonts.
const bundledCjkFontFile = join(
  dirname(requireFromHere.resolve('@xpert-ai/plugin-excalidraw/package.json')),
  'dist/assets/fonts/NotoSansCJKsc-Regular.otf'
)
  ...
  validate(resolved: ResolvedDiagram): DiagramValidationReport {
    ...
    for (const group of resolved.groups) {
      if (
        group.x < 0 ||
        group.y < 0 ||
        group.x + group.width > canvas.width ||
        group.y + group.height > canvas.height
      ) {
        issues.push(
          issue('canvas.group_out_of_bounds', 'error', `Group "${group.label}" is outside the canvas.`, [group.id])
        )
      }
    }
    for (const node of resolved.nodes) {
      if (
        node.x < canvas.padding ||
        node.y < canvas.padding ||
        node.x + node.width > canvas.width - canvas.padding ||
        node.y + node.height > canvas.height - canvas.padding
      ) {
        issues.push(
          issue('canvas.node_out_of_bounds', 'error', `Node "${node.label}" is outside the canvas safe area.`, [
            node.id
          ])
        )
      }
    ...
    for (let i = 0; i < resolved.nodes.length; i += 1) {
      for (let j = i + 1; j < resolved.nodes.length; j += 1) {
        if (nodeIntersects(resolved.nodes[i], resolved.nodes[j], 8)) {
          issues.push(
            issue('layout.node_overlap', 'error', 'Diagram nodes overlap.', [
              resolved.nodes[i].id,
              resolved.nodes[j].id
            ])
          )
    ...
    for (const edge of resolved.edges) {
      for (const node of resolved.nodes) {
        ...
        if (polylineHitsNode(edge.points, node, 4)) {
          issues.push(
            issue('routing.edge_node_collision', 'error', `Edge "${edge.id}" crosses node "${node.id}".`, [
              edge.id,
              node.id
            ])
          )
      ...
      if (edge.label && estimatedTextWidth(edge.label) > 180) {
        issues.push(
          issue('text.edge_label_long', 'warning', `Edge label "${edge.label}" is too long for reliable routing.`, [
            edge.id
          ])
        )
      }
      if (!nodeMap.has(edge.source.nodeId) || !nodeMap.has(edge.target.nodeId)) {
        issues.push(
          issue('reference.edge_endpoint_missing', 'error', `Edge "${edge.id}" has a missing endpoint.`, [edge.id])
        )
      }
    }
    const labels = resolved.edges
    ...
    for (const label of labels) {
      if (label.left < 0 || label.top < 0 || label.right > canvas.width || label.bottom > canvas.height) {
        issues.push(
          issue(
            'text.edge_label_out_of_bounds',
            'warning',
            `Edge label "${label.edge.label}" is near or outside the canvas boundary.`,
            [label.edge.id]
          )
        )
      }
      for (const node of resolved.nodes) {
        if (
          ...
          issues.push(
            issue(
              'text.edge_label_node_collision',
              'warning',
              `Edge label "${label.edge.label}" overlaps node "${node.id}".`,
              [label.edge.id, node.id]
            )
          )
        }
      }
    ...
    for (let i = 0; i < labels.length; i += 1) {
      for (let j = i + 1; j < labels.length; j += 1) {
        if (rectanglesIntersect(labels[i], labels[j])) {
          issues.push(
            issue('text.edge_label_collision', 'warning', 'Edge labels overlap.', [
              labels[i].edge.id,
              labels[j].edge.id
            ])
          )
    ...
    if (crossings > Math.max(2, resolved.edges.length / 3)) {
      issues.push(
        issue(
          'routing.excessive_crossings',
          'warning',
          `Diagram contains ${crossings} edge crossings.`,
...
export class DiagramCompilerService {
  constructor(private readonly layout: DiagramLayoutService, private readonly validation: DiagramValidationService) {}
...
export class DiagramPreviewService {
  async createPreview(
    ...
    const base = `files/excalidraw/diagrams/${safePath(input.drawingId)}/quality/${safePath(
      input.qualityRunId
    )}/preview-${input.attempt}`
    ...
    const [svg, png] = await Promise.all([
      workspaceFiles.writeRuntimeBuffer({
        path: `${base}.svg`,
        originalName: `preview-${input.attempt}.svg`,
        mimeType: 'image/svg+xml',
        buffer: svgBuffer
      }),
      workspaceFiles.writeRuntimeBuffer({
        path: `${base}.png`,
        originalName: `preview-${input.attempt}.png`,
        mimeType: 'image/png',
        buffer: pngBuffer
      })
    ])
    return {
      svg: {
        ...
        workspacePath: svg.workspacePath,
        fileRef: svg.reference,
        size: svg.size ?? svgBuffer.length
      },
      png: {
        filePath: png.filePath,
        workspacePath: png.workspacePath,
        fileRef: png.reference,
        size: png.size ?? pngBuffer.length
      }
    }
  }
}
...
function compileElements(resolved: ResolvedDiagram, palette: Palette) {
  ...
  for (const group of resolved.groups) {
    elements.push(
      baseElement(`diagram-group-${group.id}`, 'rectangle', group.x, group.y, group.width, group.height, {
    ...
    elements.push(
      textElement(
        `diagram-group-label-${group.id}`,
        group.label,
        group.x + 16,
        group.y + 12,
        Math.max(120, group.width - 32),
        26,
        palette.muted,
        roughness,
        'left'
  ...
  for (const node of resolved.nodes) {
    const type =
      node.kind === 'decision'
        ? 'diamond'
        : ['user', 'actor', 'graph-store'].includes(node.kind)
        ? 'ellipse'
        : 'rectangle'
    const colors = nodeColors(node, palette)
    elements.push(
      baseElement(`diagram-node-${node.id}`, type, node.x, node.y, node.width, node.height, {
    ...
    if (node.description) {
      elements.push(
        textElement(
          `diagram-node-description-${node.id}`,
          node.description,
          node.x + 12,
          node.y + node.height - 26,
          node.width - 24,
          20,
          palette.muted,
          roughness,
  ...
  for (const annotation of resolved.ir.annotations) {
    const position = annotation.position ?? {
      x: resolved.ir.canvas.padding,
      y: resolved.ir.canvas.height - resolved.ir.canvas.padding - 34
    }
    elements.push(
      textElement(
        `diagram-annotation-${annotation.id}`,
        annotation.text,
        position.x,
        position.y,
        260,
  ...
  resolved.ir.legend.forEach((item, index) => {
    ...
    elements.push({
      ...baseElement(`diagram-legend-line-${index}-${item.flow}`, 'line', x, y + 10, 36, 1, {
        strokeColor: palette.flows[item.flow],
        backgroundColor: 'transparent',
        roughness,
    ...
    elements.push(
      textElement(
        `diagram-legend-label-${index}-${item.flow}`,
        item.label,
        x + 46,
        y,
        164,
...
function renderSvg(resolved: ResolvedDiagram, palette: Palette) {
  const markers = Object.entries(palette.flows)
    .map(
      ([flow, color]) =>
    ...
    .join('')
  const groupSvg = resolved.groups
    .map(
      (group) =>
        `<g><rect x="${group.x}" y="${group.y}" width="${group.width}" height="${group.height}" rx="16" fill="${
          palette.groupFill
        }" stroke="${palette.groupStroke}" stroke-dasharray="7 5"/><text x="${group.x + 16}" y="${
          group.y + 24
        }" class="group">${escapeXml(group.label)}</text></g>`
    )
    .join('')
  const lifelineSvg =
    resolved.ir.layout.strategy === 'sequence'
      ? resolved.nodes
          .map(
            (node) =>
              `<path d="M ${node.x + node.width / 2} ${node.y + node.height} L ${node.x + node.width / 2} ${
                resolved.ir.canvas.height - resolved.ir.canvas.padding
              }" fill="none" stroke="${palette.groupStroke}" stroke-width="1.5" stroke-dasharray="6 5"/>`
          )
          .join('')
      : ''
  const edgeSvg = resolved.edges
    .map((edge) => {
      ...
      const label = edge.label
        ? (() => {
            const p = longestSegmentMidpoint(edge.points)
            return `<text x="${p.x}" y="${p.y - 8}" text-anchor="middle" class="edge-label">${escapeXml(
              edge.label
            )}</text>`
          })()
        : ''
      return `<g><path d="${d}" fill="none" stroke="${
        palette.flows[edge.flow]
      }" stroke-width="2.2"${dash}${marker}/>${label}</g>`
    })
    .join('')
  const nodeSvg = resolved.nodes
    .map((node) => {
      const colors = nodeColors(node, palette)
      const label = `<text x="${node.x + node.width / 2}" y="${
        node.y + node.height / 2 + 5
      }" text-anchor="middle" class="node-label">${escapeXml(node.label)}</text>`
      if (node.kind === 'decision') {
        const cx = node.x + node.width / 2
        const cy = node.y + node.height / 2
        return `<g><path d="M ${cx} ${node.y} L ${node.x + node.width} ${cy} L ${cx} ${node.y + node.height} L ${
          node.x
        } ${cy} Z" fill="${colors.fill}" stroke="${colors.stroke}" stroke-width="2"/>${label}</g>`
      }
      if (['user', 'actor', 'graph-store'].includes(node.kind)) {
        return `<g><ellipse cx="${node.x + node.width / 2}" cy="${node.y + node.height / 2}" rx="${
          node.width / 2
        }" ry="${node.height / 2}" fill="${colors.fill}" stroke="${colors.stroke}" stroke-width="2"/>${label}</g>`
      }
    ...
    .join('')
  const annotationSvg = resolved.ir.annotations
    .map((annotation) => {
      const p = annotation.position ?? {
        x: resolved.ir.canvas.padding,
        y: resolved.ir.canvas.height - resolved.ir.canvas.padding
      }
      return `<text x="${p.x}" y="${p.y}" class="annotation">${escapeXml(annotation.text)}</text>`
    })
    .join('')
  const legendTop = resolved.ir.canvas.height - resolved.ir.canvas.padding - resolved.ir.legend.length * 28
  const legendSvg = resolved.ir.legend
    .map((item, index) => {
      const x = resolved.ir.canvas.width - resolved.ir.canvas.padding - 210
      const y = legendTop + index * 28
      const dash = item.flow === 'async' || item.flow === 'write' ? ' stroke-dasharray="6 4"' : ''
      return `<g><path d="M ${x} ${y + 10} L ${x + 36} ${y + 10}" stroke="${
        palette.flows[item.flow]
      }" stroke-width="2"${dash}/><text x="${x + 46}" y="${y + 14}" class="annotation">${escapeXml(
        item.label
      )}</text></g>`
    })
    .join('')
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${resolved.ir.canvas.width} ${
    resolved.ir.canvas.height
  }" width="${resolved.ir.canvas.width}" height="${
    resolved.ir.canvas.height
  }"><defs>${markers}<style>text{font-family:"Noto Sans CJK SC",Arial,"PingFang SC","Microsoft YaHei",sans-serif}.title{font-size:28px;font-weight:700;fill:${
    palette.text
  }}.subtitle{font-size:14px;fill:${palette.muted}}.group{font-size:13px;font-weight:700;fill:${
    palette.muted
  }}.node-label{font-size:17px;font-weight:600;fill:${palette.text}}.edge-label,.annotation{font-size:12px;fill:${
    palette.muted
  };paint-order:stroke;stroke:${
    palette.canvas
  };stroke-width:5px;stroke-linejoin:round}</style></defs><rect width="100%" height="100%" fill="${
    palette.canvas
  }"/><text x="${resolved.ir.canvas.padding}" y="${resolved.ir.canvas.padding + 24}" class="title">${escapeXml(
    resolved.ir.title
  )}</text>${
    resolved.ir.subtitle
      ? `<text x="${resolved.ir.canvas.padding}" y="${resolved.ir.canvas.padding + 48}" class="subtitle">${escapeXml(
          resolved.ir.subtitle
        )}</text>`
      : ''
  }${groupSvg}${lifelineSvg}${edgeSvg}${nodeSvg}${annotationSvg}${legendSvg}</svg>`
}
...
function paletteFor(ir: DiagramIR): Palette {
  ...
  return {
    ...
    flows: semantic
      ...
      : (Object.fromEntries(
          ['primary', 'control', 'read', 'write', 'async', 'transform', 'feedback', 'neutral'].map((key) => [
            key,
            dark ? '#cbd5e1' : '#475569'
          ])
        ) as Record<DiagramFlow, string>)
  }
}
...
function nodeColors(node: ResolvedDiagramNode, palette: Palette) {
  ...
  if (['memory', 'database', 'vector-store', 'graph-store'].includes(node.kind))
    return { fill: palette.nodeFill, stroke: palette.flows.read }
...
function issue(
  code: string,
  severity: DiagramQualityIssue['severity'],
  message: string,
  targetIds: string[]
): DiagramQualityIssue {
...
function longestSegmentMidpoint(points: DiagramPoint[]) {
  const pair = segments(points).sort(([a1, a2], [b1, b2]) => distance(b1, b2) - distance(a1, a2))[0] ?? [
    points[0],
    points[0]
  ]
