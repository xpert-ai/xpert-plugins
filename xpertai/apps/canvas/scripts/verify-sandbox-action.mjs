...
if (!files.some((file) => file.relativePath === 'renderer.js') || !files.some((file) => file.relativePath === 'renderer.css')) throw new Error('Canvas browser renderer bundle is incomplete.')
if (files.some((file) => file.relativePath.startsWith('node_modules/'))) throw new Error('Sandbox Action must not package node_modules.')
if (files.some((file) => file.relativePath.includes('playwright-core'))) throw new Error('playwright-core must be supplied by Browser Runtime.')
