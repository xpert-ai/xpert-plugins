...
describe('canvas assistant template', () => {
  it('declares view-image and Seedream dependencies for visual generation workflows', () => {
    ...
    expect(template.dslContent).toContain('createGeoShapes')
    expect(template.dslContent).toContain('canvas_patch_records.workflow')
    expect(template.dslContent).toContain('workflow.mode=replace_page')
    expect(template.dslContent).toContain('Use createArrowShapes only for actual')
    expect(template.dslContent).not.toContain('hash: canvas-agent-v3-simplified-shapes')
    expect(template.dslContent).not.toContain('canvas_publish_artifact_link')
    expect(template.dslContent).toContain('hash: canvas-agent-v5-semantic-workflow')
