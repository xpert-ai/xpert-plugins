...
const sourceRoot = path.join(packageRoot, 'sandbox-actions', 'storyboard-render')
...
await writeFile(
  ...
  `${JSON.stringify(
    {
      name: 'story-studio.storyboard-render',
...
async function copyPackageClosure(packageName, resolver, resolverRoot, includeDependencies = true) {
  ...
  const unresolved = await resolvePackageJson(packageName, resolver, resolverRoot)
  if (!unresolved) throw new Error(`Sandbox Action dependency is missing: ${packageName}`)
  const packageJsonPath = await realpath(unresolved)
  const packageJson = JSON.parse(await readFile(packageJsonPath, 'utf8'))
  const previous = copiedPackages.get(packageName)
  if (previous && previous !== packageJson.version)
    throw new Error(`Sandbox Action dependency ${packageName} has conflicting versions.`)
...
function shouldCopyPackageEntry(source) {
  ...
  if (['node_modules', '.DS_Store', '.git', 'test', 'tests'].includes(name)) return false
