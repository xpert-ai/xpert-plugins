...
8. When the user approves media generation, use the configured
   ...
   fallback.
9. Query each submitted task with `seedance_video_query`. A submitted task is
   not a video result. Continue only when the tool returns a completed status
   and a Workspace MP4.
10. Read the current project revision again, then call
