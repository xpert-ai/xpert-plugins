          ...
          <div className="grid gap-5 p-5">
            <section className="grid grid-cols-[1.3fr_0.7fr] gap-4">
              <figure className="relative overflow-hidden rounded-xl border border-studio-line bg-black shadow-lg">
                {previewVideoUrl ? <video key={previewCandidate?.id} data-testid="director-main-video-preview" className="aspect-video h-full w-full bg-black object-contain" src={previewVideoUrl} controls crossOrigin="use-credentials" playsInline preload="metadata" autoPlay={isPlaying} onLoadedMetadata={(event) => primeVideoPreview(event.currentTarget)} onLoadedData={(event) => primeVideoPreview(event.currentTarget)} onPlay={() => setIsPlaying(true)} onPause={() => setIsPlaying(false)} /> : null}
        ...
        {shot ? <div className="grid gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-studio-muted">{t('director.storyboard.settings')}</p><h3 className="mt-1 font-display text-xl font-bold">{shot.title}</h3></div>
          <section className="grid gap-3 rounded-lg border border-studio-line bg-studio-canvas p-3">
            ...
            <button type="button" className="flex min-h-12 items-center gap-3 rounded-md border border-dashed border-studio-line bg-studio-paper px-3 py-2 text-left hover:border-studio-brass" onClick={() => shotReferenceUploadRef.current?.click()}>
              {temporaryReference && candidateImage(temporaryReference) ? <img className="size-10 rounded bg-studio-canvas object-contain" crossOrigin="use-credentials" src={candidateImage(temporaryReference) ?? ''} alt="" /> : <span className="grid size-10 place-items-center rounded bg-studio-panel text-studio-muted"><Upload className="size-4" aria-hidden="true" /></span>}
...
function TakeCard(props: { index: number; task?: VideoGenerationTask; candidate?: Candidate; selected: boolean; automatic: boolean; previewed: boolean; onPreview: () => void; onSelect: () => void; onCancel: () => void; onRetry: () => void; t: DirectorTranslator }) {
  ...
  return <article className={`overflow-hidden rounded-lg border bg-studio-canvas ${adopted ? 'border-2 border-studio-brass' : props.previewed ? 'border-studio-ink ring-2 ring-studio-ink/15' : 'border-studio-line'}`}>
    <button type="button" className="relative block w-full overflow-hidden bg-black text-white disabled:cursor-default" disabled={!props.candidate} aria-label={props.t('director.storyboard.candidateLabel', { number: props.index + 1 })} onClick={props.onPreview}>
      {videoUrl ? <video key={props.candidate?.id} data-testid={`director-take-video-${props.index + 1}`} className="aspect-video w-full bg-black object-contain" src={videoUrl} crossOrigin="use-credentials" muted playsInline preload="metadata" onLoadedMetadata={(event) => primeVideoPreview(event.currentTarget)} onLoadedData={(event) => primeVideoPreview(event.currentTarget)} /> : <span className="grid aspect-video w-full place-items-center bg-studio-ink/90 px-4 text-center"><span><Play className="mx-auto size-6 opacity-45" aria-hidden="true" /><span className="mt-2 block text-[11px] text-white/70">{props.task ? videoTaskStatusLabel(props.task.status, props.t) : props.t('director.storyboard.notGenerated')}</span></span></span>}
...
function ReferenceAssetCard(props: { asset: Asset; selectedCandidateIds: string[]; t: DirectorTranslator; onEdit: () => void; onUpload: () => void }) {
  ...
  return <article className="group relative grid min-w-0 grid-cols-[40px_minmax(0,1fr)] items-center gap-2 rounded-md border border-studio-line bg-studio-paper p-2">
    {image ? <span className="relative block size-10"><img className="size-10 rounded bg-studio-canvas object-contain" crossOrigin="use-credentials" src={image} alt={props.asset.name} />{selectedImages.length > 1 ? <span className="absolute -bottom-1 -right-1 grid size-4 place-items-center rounded-full bg-studio-brass text-[8px] font-bold text-white">{selectedImages.length}</span> : null}</span> : <span className="grid size-10 place-items-center rounded bg-studio-panel text-studio-muted"><ImageIcon className="size-4" aria-hidden="true" /></span>}
