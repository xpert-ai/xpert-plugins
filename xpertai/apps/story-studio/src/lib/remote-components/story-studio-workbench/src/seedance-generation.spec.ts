...
describe('Seedance synchronized-audio generation request', () => {
  ...
  it('builds a status-only follow-up that cannot resubmit generation', () => {
    ...
    expect(message).toContain('currentBaseRevision=8')
    expect(message).toContain('returned revision for the next attachment')
    expect(message).not.toContain('refresh the project summary')
