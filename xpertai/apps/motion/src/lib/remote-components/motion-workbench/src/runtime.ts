import { motionWorkbenchDebug, setMotionDebugHostConfig } from './debug-logger'
...
export function getErrorMessage(error: unknown): string {
  return typeof error === 'object' && error && 'message' in error && error.message
...
export function startRemoteBridge(setContext: (context: RemoteBridgeContext) => void, handleHostEvent: (event: RemotePayloadValue | undefined) => void) {
  ...
  function applyRemoteTheme(theme: RemotePayloadValue | undefined) {
    ;(window as RemoteUIWindow).XpertRemoteUI?.applyTheme?.(theme)
  }
  ...
  window.addEventListener('message', (event) => {
    ...
    if (message.type === 'init') {
      ...
      setMotionDebugHostConfig(message.debug)
      const nextContext: RemoteBridgeContext = {
      ...
      setContext(nextContext)
      motionWorkbenchDebug.info('bridge.init', {
    ...
    if (message.type === 'hostEvent') {
      handleHostEvent(extractHostEventFromMessage(message))
      return
    }
...
function extractHostEventFromMessage(message: RemoteMessage): RemotePayloadValue | undefined {
  if (message.event !== undefined) return message.event
