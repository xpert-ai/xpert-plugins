...
description: |
  A heart/favorite icon that animates on toggle — Lottie JSON, portable across web & app. A Lottie (JSON) animation — portable motion that plays identically on web and app via
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Use lottie-favorite: load animation.json with lottie-web on the icon, loop it, and guard
    prefers-reduced-motion (autoplay:false).
---

# Lottie · Favorite

A heart/favorite icon that animates on toggle — Lottie JSON, portable across web & app. Lottie JSON from [spemer/lottie-animations-json](https://github.com/spemer/lottie-animations-json)
