...
description: |
  An animated pagination dots indicator — Lottie JSON. A Lottie (JSON) animation — portable motion that plays identically on web and app via
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Use lottie-pagination: load animation.json with lottie-web on the icon, loop it, and guard
