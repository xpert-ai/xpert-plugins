...
name: lottie-tab
description: |
  An animated bottom-tab indicator — Lottie JSON. A Lottie (JSON) animation — portable motion that plays identically on web and app via
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Use lottie-tab: load animation.json with lottie-web on the button, loop it, and guard
    prefers-reduced-motion (autoplay:false).
---

# Lottie · Tab

An animated bottom-tab indicator — Lottie JSON. Lottie JSON from [spemer/lottie-animations-json](https://github.com/spemer/lottie-animations-json)
