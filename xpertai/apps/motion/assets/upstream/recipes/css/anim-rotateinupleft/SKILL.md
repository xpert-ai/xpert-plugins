---
name: anim-rotateinupleft
description: |
  Rotate In Up Left — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply rotateInUpLeft (Animate.css) to the element as a entrance animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Rotate In Up Left

Rotate In Up Left — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `rotateInUpLeft.css`. 2. Add class `rotateInUpLeft` to the element + set `animation-duration`.
