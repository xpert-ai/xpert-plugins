---
name: anim-lightspeedinleft
description: |
  Light Speed In Left — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply lightSpeedInLeft (Animate.css) to the element as a entrance animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Light Speed In Left

Light Speed In Left — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `lightSpeedInLeft.css`. 2. Add class `lightSpeedInLeft` to the element + set `animation-duration`.
