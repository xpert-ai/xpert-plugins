---
name: anim-fadeinleftbig
description: |
  Fade In Left Big — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply fadeInLeftBig (Animate.css) to the element as a entrance animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Fade In Left Big

Fade In Left Big — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `fadeInLeftBig.css`. 2. Add class `fadeInLeftBig` to the element + set `animation-duration`.
