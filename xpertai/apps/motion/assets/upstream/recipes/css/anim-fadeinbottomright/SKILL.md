---
name: anim-fadeinbottomright
description: |
  Fade In Bottom Right — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply fadeInBottomRight (Animate.css) to the element as a entrance animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Fade In Bottom Right

Fade In Bottom Right — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `fadeInBottomRight.css`. 2. Add class `fadeInBottomRight` to the element + set `animation-duration`.
