...
name: anim-hinge
description: |
  Hinge — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply hinge (Animate.css) to the element as a exit animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Hinge

Hinge — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `hinge.css`. 2. Add class `hinge` to the element + set `animation-duration`.
