...
description: |
  Fade In Right Big — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply fadeInRightBig (Animate.css) to the element as a entrance animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Fade In Right Big

Fade In Right Big — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `fadeInRightBig.css`. 2. Add class `fadeInRightBig` to the element + set `animation-duration`.
