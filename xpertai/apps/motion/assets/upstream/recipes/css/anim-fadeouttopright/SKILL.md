---
name: anim-fadeouttopright
description: |
  Fade Out Top Right — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply fadeOutTopRight (Animate.css) to the element as a exit animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Fade Out Top Right

Fade Out Top Right — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `fadeOutTopRight.css`. 2. Add class `fadeOutTopRight` to the element + set `animation-duration`.
