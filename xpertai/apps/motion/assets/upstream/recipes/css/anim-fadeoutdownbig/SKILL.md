---
name: anim-fadeoutdownbig
description: |
  Fade Out Down Big — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply fadeOutDownBig (Animate.css) to the element as a exit animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Fade Out Down Big

Fade Out Down Big — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `fadeOutDownBig.css`. 2. Add class `fadeOutDownBig` to the element + set `animation-duration`.
