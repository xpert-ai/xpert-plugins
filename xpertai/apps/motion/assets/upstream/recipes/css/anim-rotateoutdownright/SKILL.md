---
name: anim-rotateoutdownright
description: |
  Rotate Out Down Right — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply rotateOutDownRight (Animate.css) to the element as a exit animation, restrained, with a
