---
name: anim-bounceinleft
description: |
  Bounce In Left — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply bounceInLeft (Animate.css) to the element as a entrance animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Bounce In Left

Bounce In Left — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `bounceInLeft.css`. 2. Add class `bounceInLeft` to the element + set `animation-duration`.
