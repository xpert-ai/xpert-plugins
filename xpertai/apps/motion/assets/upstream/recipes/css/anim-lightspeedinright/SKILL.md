---
name: anim-lightspeedinright
description: |
  Light Speed In Right — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply lightSpeedInRight (Animate.css) to the element as a entrance animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Light Speed In Right

Light Speed In Right — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `lightSpeedInRight.css`. 2. Add class `lightSpeedInRight` to the element + set `animation-duration`.
