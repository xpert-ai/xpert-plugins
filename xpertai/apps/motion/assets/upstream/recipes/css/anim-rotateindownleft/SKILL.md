...
description: |
  Rotate In Down Left — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply rotateInDownLeft (Animate.css) to the element as a entrance animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Rotate In Down Left

Rotate In Down Left — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `rotateInDownLeft.css`. 2. Add class `rotateInDownLeft` to the element + set `animation-duration`.
