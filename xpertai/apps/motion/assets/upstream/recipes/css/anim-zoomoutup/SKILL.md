...
name: anim-zoomoutup
description: |
  Zoom Out Up — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply zoomOutUp (Animate.css) to the element as a exit animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Zoom Out Up

Zoom Out Up — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `zoomOutUp.css`. 2. Add class `zoomOutUp` to the element + set `animation-duration`.
