---
name: anim-slideindown
description: |
  Slide In Down — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply slideInDown (Animate.css) to the element as a entrance animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Slide In Down

Slide In Down — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `slideInDown.css`. 2. Add class `slideInDown` to the element + set `animation-duration`.
