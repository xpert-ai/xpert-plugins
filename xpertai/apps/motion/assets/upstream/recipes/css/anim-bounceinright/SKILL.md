---
name: anim-bounceinright
description: |
  Bounce In Right — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply bounceInRight (Animate.css) to the element as a entrance animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Bounce In Right

Bounce In Right — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `bounceInRight.css`. 2. Add class `bounceInRight` to the element + set `animation-duration`.
