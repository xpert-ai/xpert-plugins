---
name: anim-bounceinup
description: |
  Bounce In Up — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply bounceInUp (Animate.css) to the element as a entrance animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Bounce In Up

Bounce In Up — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `bounceInUp.css`. 2. Add class `bounceInUp` to the element + set `animation-duration`.
