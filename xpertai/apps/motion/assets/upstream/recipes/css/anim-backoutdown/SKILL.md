---
name: anim-backoutdown
description: |
  Back Out Down — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply backOutDown (Animate.css) to the element as a exit animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Back Out Down

Back Out Down — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `backOutDown.css`. 2. Add class `backOutDown` to the element + set `animation-duration`.
