...
name: anim-heartbeat
description: |
  Heart Beat — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply heartBeat (Animate.css) to the element as a emphasis animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Heart Beat

Heart Beat — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `heartBeat.css`. 2. Add class `heartBeat` to the element + set `animation-duration`.
