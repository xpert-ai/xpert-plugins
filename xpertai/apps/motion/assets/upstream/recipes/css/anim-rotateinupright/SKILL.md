---
name: anim-rotateinupright
description: |
  Rotate In Up Right — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply rotateInUpRight (Animate.css) to the element as a entrance animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Rotate In Up Right

Rotate In Up Right — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `rotateInUpRight.css`. 2. Add class `rotateInUpRight` to the element + set `animation-duration`.
