...
description: |
  Fade In Bottom Left — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply fadeInBottomLeft (Animate.css) to the element as a entrance animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Fade In Bottom Left

Fade In Bottom Left — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `fadeInBottomLeft.css`. 2. Add class `fadeInBottomLeft` to the element + set `animation-duration`.
