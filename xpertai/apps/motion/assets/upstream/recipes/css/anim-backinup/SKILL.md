...
name: anim-backinup
description: |
  Back In Up — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply backInUp (Animate.css) to the element as a entrance animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Back In Up

Back In Up — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `backInUp.css`. 2. Add class `backInUp` to the element + set `animation-duration`.
