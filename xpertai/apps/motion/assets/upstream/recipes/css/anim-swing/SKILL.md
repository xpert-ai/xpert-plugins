...
name: anim-swing
description: |
  Swing — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply swing (Animate.css) to the element as a emphasis animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Swing

Swing — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `swing.css`. 2. Add class `swing` to the element + set `animation-duration`.
