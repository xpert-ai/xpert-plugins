...
name: anim-shakey
description: |
  Shake Y — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply shakeY (Animate.css) to the element as a emphasis animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Shake Y

Shake Y — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `shakeY.css`. 2. Add class `shakeY` to the element + set `animation-duration`.
