---
name: anim-rotateoutupleft
description: |
  Rotate Out Up Left — a drop-in CSS keyframe animation from Animate.css (Hippocratic-2.1, © Daniel Eden).
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Apply rotateOutUpLeft (Animate.css) to the element as a exit animation, restrained, with a
    prefers-reduced-motion fallback. Attribute Animate.css.
---

# Animate · Rotate Out Up Left

Rotate Out Up Left — CSS keyframe animation from [Animate.css](https://github.com/animate-css/animate.css)
...
## How to apply
1. Copy `rotateOutUpLeft.css`. 2. Add class `rotateOutUpLeft` to the element + set `animation-duration`.
