...
description: |
  Words cascade into place — a kinetic title. A bold, canvas-based effect from Open Design's html-ppt fx pack — for slides and launch
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Use fx-word-cascade as a text effect on a slide or hero: call HPX['word-cascade'](el) on a positioned dark
    container; keep to one bold effect per screen, with a reduced-motion guard.
---

# Word Cascade

Words cascade into place — a kinetic title. From [Open Design](https://github.com/nexu-io/open-design)'s html-ppt fx pack (Apache-2.0).
...
## How to apply
1. Copy `word-cascade.js` and `_util.js` and link them.
2. Add `data-fx="word-cascade"` to a positioned container; call `HPX['word-cascade'](el)` (guard prefers-reduced-motion).
