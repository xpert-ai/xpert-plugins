...
name: fx-confetti
description: |
  Confetti fires across the frame — for wins and reveals. A bold, canvas-based effect from Open Design's html-ppt fx pack — for slides and launch
...
od:
  ...
  design_system:
    requires: false
  example_prompt: |
    Use fx-confetti as a section effect on a slide or hero: call HPX['confetti-cannon'](el) on a positioned dark
    container; keep to one bold effect per screen, with a reduced-motion guard.
---

# Confetti Cannon

Confetti fires across the frame — for wins and reveals. From [Open Design](https://github.com/nexu-io/open-design)'s html-ppt fx pack (Apache-2.0).
