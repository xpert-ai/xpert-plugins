...
async function main() {
  ...
  const reportFile = reportIndex >= 0 ? path.resolve(CALLER_CWD, argv[reportIndex + 1]) : null;
  const positional = argv.filter((arg, index) => !arg.startsWith('--') && argv[index - 1] !== '--title' && argv[index - 1] !== '--report');
  ...
  try {
    ...
    if (pdfMode) {
      ...
      writeReport(reportFile, { format: 'pdf', pages: result.pages, warnings: result.warnings || [] });
    } else {
...
function printUsage() {
  console.error('Usage:');
  console.error('  node scripts/export-pptx.mjs <deck-ppt-dir> [out-file.pptx] [--pdf] [--title <text>] [--report <file.json>]');
