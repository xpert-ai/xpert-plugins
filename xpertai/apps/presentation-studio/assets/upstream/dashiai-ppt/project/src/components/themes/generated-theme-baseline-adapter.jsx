...
function readableText(background,definition) {
  return [definition.tokens.foreground,definition.tokens.background,'#101217','#ffffff']
    .sort((a,b)=>contrastRatio(background,b)-contrastRatio(background,a))[0];
}

function bridgeDialect(definition) {
  return definition.profile?.ornament==='halo-orb'?'halo':'spooky';
}

function dialectBridgeCss(definition) {
  const {tokens:t,key}=definition;
  const dialect=bridgeDialect(definition);
  const root=`.${key}-baseline[data-dialect="${dialect}"]`;
  if(dialect==='halo') return `${root} :is([class*="card"],[class*="panel"],[class*="tile"]){border-radius:18px!important;background:color-mix(in srgb,${t.background} 82%,${t.accent})!important;box-shadow:0 0 34px color-mix(in srgb,${t.accent} 26%,transparent)!important;}${root} .pulse-meter__row--body,${root} .pulse-score__row{min-height:0!important;}${root} .pulse-meter__cell{padding:12px 20px!important;}${root} .pulse-meter__k{font-size:28px!important;}${root} .pulse-meter__num{font-size:40px!important;}${root} .pulse-score__h{padding:10px 14px!important;}${root} .pulse-score__name{gap:3px!important;padding:0 14px!important;}${root} .pulse-score__name-zh{font-size:28px!important;}${root} .pulse-score__cell,${root} .pulse-score__ov{margin:4px!important;min-height:0!important;}${root} .pulse-score__cell b{font-size:38px!important;}${root} .pulse-score__ov b{font-size:42px!important;}${root}:before{content:"";position:absolute;right:54px;top:72px;width:300px;height:300px;border:4px solid ${t.secondary};border-radius:50%;box-shadow:0 0 70px ${t.accent};opacity:.18;pointer-events:none;z-index:1;}`;
  return `${root} :is([class*="card"],[class*="panel"],[class*="tile"]){border-radius:34px 12px 42px 16px!important;border:3px solid ${t.accent}!important;}${root} .dk-glass-dark p{font-size:21px!important;line-height:1.32!important;overflow-wrap:anywhere!important;}${root}:before{content:"";position:absolute;left:0;top:58px;width:330px;height:250px;background:radial-gradient(circle at 0 0,transparent 0 34px,${t.foreground} 35px 37px,transparent 38px 70px,${t.foreground} 71px 73px,transparent 74px);opacity:.14;pointer-events:none;z-index:1;}`;
}
...
function themeBridgeCss(definition) {
  ...
  const accentText=readableText(t.accent,definition);
  const secondaryText=readableText(t.secondary,definition);
  const accentInk=contrastRatio(t.accent,t.background)>=3?t.accent:t.foreground;
  const surface=`color-mix(in srgb,${t.background} 92%,${t.foreground})`;
...
.${key}-baseline :is([class*="card"],[class*="panel"],[class*="tile"],[class*="cell"]){background:${surface}!important;border-color:${t.secondary}!important;box-shadow:none!important;}
.${key}-baseline :is(div,section,main,article,aside,header,footer,h1,h2,h3,h4,h5,p,li,td,th,span,strong,small,label,em){color:${t.foreground}!important;text-shadow:none!important;}
.${key}-baseline :is([class*="value"],[class*="metric"],[class*="number"],[class*="kicker"],[class*="eyebrow"]){color:${accentInk}!important;-webkit-text-fill-color:${accentInk}!important;}
.${key}-baseline :is(h1,h2,h3) :is(span,strong,em),.${key}-baseline :is([class*="title"],[class*="headline"]) :is(span,strong,em){color:${accentInk}!important;-webkit-text-fill-color:${accentInk}!important;}
...
`:'';
  return `
.${key}-baseline{--dashi-target-bg:${t.background};--dashi-target-fg:${t.foreground};--dashi-target-accent:${t.accent};--dashi-target-secondary:${t.secondary};--dashi-accent-ink:${accentInk};--dashi-accent-text:${accentText};--dashi-secondary-text:${secondaryText};width:100%;height:100%;position:relative;overflow:hidden;background:${t.background};color:${t.foreground};}
...
.${key}-baseline :is(div,section,main,article,aside,header,footer,h1,h2,h3,h4,h5,p,li,td,th,span,strong,small,label,em,[class*="title"],[class*="headline"],[class*="display"],[class*="body"],[class*="copy"],[class*="note"]){color:${t.foreground}!important;}
...
.${key}-baseline :is([class*="kicker"],[class*="eyebrow"],[class*="metric"],[class*="value"],[class*="accent"]){color:${accentInk}!important;-webkit-text-fill-color:${accentInk}!important;}
.${key}-baseline :is([class*="card"],[class*="panel"],[class*="tile"]){background-color:${surface}!important;color:${t.foreground}!important;border-color:color-mix(in srgb,${t.accent} 46%,transparent)!important;}
.${key}-baseline :is([class*="active"],[class*="highlight"]){background-color:${t.accent}!important;color:${accentText}!important;-webkit-text-fill-color:${accentText}!important;}
.${key}-baseline :is([class*="active"],[class*="highlight"]) :is(h1,h2,h3,h4,h5,p,span,strong,small,label,li){color:${accentText}!important;-webkit-text-fill-color:${accentText}!important;}
.${key}-baseline :is([class*="secondary"]){color:${secondaryText}!important;}
    ...
    return {
      ...
      Component:props=>{
        ...
        const content=kind==='owned'
          ...
          : <><style>{themeBridgeCss(definition)}</style><div className={`${definition.key}-baseline`} data-dialect={bridgeDialect(definition)}><Component {...defaultProps} {...props}/></div></>;
