    ...
    return {
      ...
      tools: [
        ...
        tool((input) => compact(this.service.addSlide(scope, input)), {
          name: 'presentation_add_slide', description: toolDescription(currentDeckHint, 'Add one slide to an existing deck after inspecting its layout contract. Workbench-created decks may already contain __studioShell placeholder slides; this tool replaces one placeholder on each call until all requested pages contain authored content. Construct nested arrays from authoringContract.arrayItemContracts exactly: array items may contain only allowedKeys, and top-level fields such as desc or en must not be copied into array items unless explicitly allowed. Each active slide must use a unique layout. Prefer the current deckId from context when the user asks to continue or modify the current presentation.'), schema: addSlideSchema, verboseParsingErrors: true
        }),
