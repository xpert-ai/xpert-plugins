...
- **安全只读分享** —— 发布自包含 Viewer，支持公开、组织和工作区范围，以及固定当前修订或始终最新两种策略。
...
- 基于平台 Artifacts 的发布能力，以及插件自带的无网络只读 Viewer
...
4. 在重要审阅节点手动保存版本。
5. 将文档标记为草稿、已审阅或已归档。
6. 分享固定修订或始终最新的只读链接，或将所需设计格式导出到 Workspace Files。
...
插件运行时要求 Xpert 宿主提供 Workspace Files，以及 `platform.collaboration` 和 `platform.artifacts` 能力。
