...
let scale=1,x=0,y=0,drag=null,fallbackFullscreen=false;
const active=()=>document.querySelector('.page.active .stage');
const clampScale=value=>Math.max(.05,Math.min(8,value));
const draw=()=>{const stage=active();if(stage)stage.style.transform='translate('+x+'px,'+y+'px) scale('+scale+')'};
const dimensions=()=>{const svg=active()?.querySelector('svg');if(!svg)return null;const viewBox=svg.viewBox?.baseVal;return{width:viewBox?.width||Number(svg.getAttribute('width'))||svg.getBoundingClientRect().width||1,height:viewBox?.height||Number(svg.getAttribute('height'))||svg.getBoundingClientRect().height||1}};
const center=size=>{x=(viewport.clientWidth-size.width*scale)/2;y=(viewport.clientHeight-size.height*scale)/2;draw()};
const fitPage=()=>{const size=dimensions();if(!size)return;const width=Math.max(1,viewport.clientWidth-64);const height=Math.max(1,viewport.clientHeight-64);scale=clampScale(Math.min(width/size.width,height/size.height));center(size)};
const fitWidth=()=>{const size=dimensions();if(!size)return;const availableWidth=Math.max(1,viewport.clientWidth-24);scale=clampScale(availableWidth/size.width);x=(viewport.clientWidth-size.width*scale)/2;const scaledHeight=size.height*scale;y=scaledHeight<=viewport.clientHeight-24?(viewport.clientHeight-scaledHeight)/2:12;draw()};
const fit=()=>{if(mobileQuery.matches){fitWidth()}else{fitPage()}};
const reset=()=>{const size=dimensions();if(!size)return;scale=1;center(size)};
const zoom=(factor,cx=viewport.clientWidth/2,cy=viewport.clientHeight/2)=>{const next=clampScale(scale*factor);x=cx-(cx-x)*(next/scale);y=cy-(cy-y)*(next/scale);scale=next;draw()};
const syncFullscreen=()=>{const enabled=Boolean(document.fullscreenElement)||fallbackFullscreen;document.body.classList.toggle('viewer-fullscreen',enabled);if(fullscreenButton){const label=enabled?'Exit fullscreen':'Enter fullscreen';fullscreenButton.setAttribute('aria-label',label);fullscreenButton.setAttribute('title',label)}requestAnimationFrame(fit)};
const toggleFullscreen=async()=>{if(document.fullscreenElement){await document.exitFullscreen?.();return}if(fallbackFullscreen){fallbackFullscreen=false;syncFullscreen();return}try{if(document.documentElement.requestFullscreen){await document.documentElement.requestFullscreen();return}}catch{}fallbackFullscreen=true;syncFullscreen()};
buttons.forEach(button=>button.addEventListener('click',()=>{const id=button.dataset.page;buttons.forEach(item=>item.classList.toggle('active',item===button));pages.forEach(page=>page.classList.toggle('active',page.dataset.page===id));requestAnimationFrame(fit)}));
document.querySelector('[data-action="in"]')?.addEventListener('click',()=>zoom(1.2));
document.querySelector('[data-action="out"]')?.addEventListener('click',()=>zoom(1/1.2));
...
document.addEventListener('fullscreenchange',()=>{fallbackFullscreen=false;syncFullscreen()});
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&fallbackFullscreen){fallbackFullscreen=false;syncFullscreen()}});
viewport.addEventListener('wheel',event=>{event.preventDefault();const rect=viewport.getBoundingClientRect();zoom(event.deltaY<0?1.12:1/1.12,event.clientX-rect.left,event.clientY-rect.top)},{passive:false});
viewport.addEventListener('pointerdown',event=>{if(event.button!==0)return;drag={px:event.clientX,py:event.clientY,x,y};viewport.setPointerCapture(event.pointerId);viewport.classList.add('dragging')});
viewport.addEventListener('pointermove',event=>{if(!drag)return;x=drag.x+event.clientX-drag.px;y=drag.y+event.clientY-drag.py;draw()});
