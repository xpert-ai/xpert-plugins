;(function () {
  ...
  function isDeploymentPreviewAction(actionKey) {
    return actionKey === 'create_site' || actionKey === 'deploy_version' || actionKey === 'publish_artifact_link'
  }
  ...
  function isDeploymentPreviewEvent(value) {
    return (
      ...
      Boolean(readString(value, 'url') || readString(value, 'deploymentUrl'))
    )
  }

  function normalizeDeploymentPreviewEvent(value) {
    const url = readString(value, 'url') || readString(value, 'deploymentUrl') || readString(value, 'displayUrl')
    if (!url) {
      return null
    }
      ...
      return h('main', { className: 'content', key: 'detail' }, [
        h('section', { className: 'detail-grid', key: 'detail-grid' }, [
          ...
          tableSection(t('labels.deployments'), [t('labels.status'), t('labels.access'), t('labels.url')], deployments, (item) => [
            ...
            h('div', { className: 'deployment-link-actions' }, [
              item.deploymentUrl
                ? h('a', { href: item.deploymentUrl, target: '_blank', rel: 'noreferrer', key: 'url' }, item.deploymentUrl)
                : h('span', { className: 'muted', key: 'empty' }, '-'),
              item.accessMode === 'public_link'
                ? h(
                    'button',
                    {
                      type: 'button',
                      ...
                      disabled: !!busy,
                      key: 'revoke',
                      onClick: () => runAction('revoke_artifact_link', item.id, { deploymentId: item.id })
                    },
                    busy === 'revoke_artifact_link' ? t('buttons.revokingLink') : t('buttons.revokeLink')
                  )
                : h(
                    'button',
                    {
                      ...
                      key: 'publish',
                      onClick: () =>
                        runAction('publish_artifact_link', item.id, {
                          deploymentId: item.id,
                          userConfirmedPublicLink: true
                        })
                    },
                    busy === 'publish_artifact_link' ? t('buttons.publishingLink') : t('buttons.publishLink')
                  )
            ])
          ], t),
  ...
  function shareScopeLabel(locale, project) {
    ...
    if (project && project.audience === 'public_link') {
      return accessLabel(locale, 'public_link')
    }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      * { box-sizing: border-box; }
      ...
      .deployment-link-actions { display: flex; align-items: center; gap: 8px; min-width: 0; }
      .deployment-link-actions > a { min-width: 0; overflow-wrap: anywhere; }
      .link-action { min-height: 28px; white-space: nowrap; font-size: 12px; }
      .link-action.danger { color: var(--sites-danger); border-color: color-mix(in srgb, var(--sites-danger) 35%, var(--sites-input)); }
