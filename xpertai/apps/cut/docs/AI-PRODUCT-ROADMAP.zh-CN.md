...
| MP4 导出 | 浏览器 | 浏览器一键导出；另有 Managed Queue + Sandbox Job 后台 H.264/AAC 导出 | Cut 已超过基线；自定义字体与更广编解码矩阵待扩展 |
...
| 项目持久化与版本冲突 | 本地项目为主 | 租户/组织隔离、revision、日志 | Cut 领先 |
...
2. 字幕草稿提交已具备项目 revision 与草稿 revision 双重保护，但项目 CAS 与草稿最终状态更新尚未放入同一数据库事务；需要在后续事务化基础设施中消除极小的中断窗口。
...
- 新增 `cut_commit_caption_drafts`：同一项目 revision 下一次提交 1–4 个语言草稿为独立字幕轨，避免逐个提交造成 revision 过期，支持双语/多语言字幕同时烧录。
...
- 正式成片顺序固定为：转写与媒体分析 → 审阅并应用口播清理 → 添加封面 → 按剪口/封面偏移生成字幕 → 翻译并批量提交语言轨 → finalize → 选择导出通道 → 容器元数据与抽帧验收。Sandbox Job 的媒体输入总量上限为 350 MiB；不超过上限时可用 Headless MP4，超过上限时 Workbench 禁用后台按钮并明确提示改用浏览器“导出 MP4”或较小的 Workspace 代理素材，避免提交必然失败的队列任务。
...
- 所有项目字段通过 Agent get/save 往返后保持等价。
...
- `local`：浏览器内 Whisper，强调隐私和零服务端配置。
- `server`：服务端 STT Provider，适合长视频、后台运行和可扩展模型。
...
- 注册 `cut.render-mp4@1.0.0` Sandbox Action，固定 `browser/playwright-1.61/v1` Runtime Profile；Action Bundle 使用确定性 tree SHA-256，`playwright-core` 由平台 Browser Runtime 提供而不重复打包。
- Runner 只接收结构化项目快照与 `/workspace/input/media` 中经 size/SHA-256 校验的 portable files；本地 loopback server 支持视频 range request，拒绝目录穿越、非规则文件和越界媒体引用。
...
- `CutAnalysisJob` 暴露 stage/progress/failureCode/sandboxJobId/resultExportId；可取消 Managed Queue 与活动 Sandbox Job，支持三次指数退避、可重试错误判定、幂等 Sandbox 复用和中断后结果对账。
- `CutExport` 保存 source revision、analysis job、checksum、renderer、runtime/action evidence 与报告文件引用；Workbench 显示 capability 健康、后台任务阶段、取消入口并轮询完成产物。
...
- **M1 Schema Safety（2026-07-16 完成）**：middleware 已直接复用领域层 `CutProjectDocument` Schema；高级字段往返测试已覆盖 transform/effects/mask/transition/audio/text/bookmarks；现有 mutation 已改为紧凑定位结果。
...
- **M7 Production（2026-07-16 完成）**：新增 `cut.render-mp4@1.0.0` Sandbox Action、`CutRenderService` 与 `CutRenderProcessor`。后台导出冻结 source revision 和渲染文档 checksum，媒体只通过 tenant-scoped portable Workspace Files 引用进入沙箱；支持最多 5 个画幅/文本变量/媒体映射变体、整批预校验与入队失败补偿、Sandbox/Queue 双层取消、三次指数退避、失败码、资源上限、报告和可追踪 `CutExport`。Workbench 可见 runtime health、stage/progress、取消与完成关联；Agent prompt/skill 已切换到 `cut_start_headless_export` + `cut_get_analysis_job` 的无人值守流程。
...
- 全量验证：18 个测试套件、68 项测试、正式 build/prepack、MCP 协议级 package smoke、插件生命周期加载/卸载，以及包含真实 WAV 本地证据分析、提案 diff/只读预览/逐项审阅/原子应用/安全回滚、Whisper 审阅草稿、字幕提交、Agent 目标化刷新、脏编辑保护、后台渲染健康/排队/完成关联和 900 帧音视频浏览器 MP4 导出的 19 项 Workbench 浏览器 gate 全部通过；浏览器导出产物为 682,876 字节且包含显式音轨。Sandbox Action 的真实本地 Chrome 冒烟另生成 18 帧、11,961 字节且同时包含 video/audio track 的 MP4；Bundle hash/结构验证通过。额外的真实网络烟测已使用 Hugging Face JFK 样本、`Xenova/whisper-tiny` Q4 与内置 WASM 成功生成字幕草稿。
