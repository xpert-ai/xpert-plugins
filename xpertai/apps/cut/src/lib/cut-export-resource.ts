import type { McpResourceTemplateDefinition, ResourceReadContext } from '@xpert-ai/plugin-sdk'
import { z } from 'zod/v3'
import type { CutService } from './cut.service.js'

export const CUT_EXPORT_RESOURCE_OPTIONS: Omit<McpResourceTemplateDefinition, 'read'> = {
  key: 'cut_get_export',
  uriTemplate: 'cut://projects/{projectId}/exports/{exportId}',
  title: 'Read completed Cut export',
  description:
    'Read a completed export and its authorized portable file reference. Download its filePath through the publication binary files endpoint.',
...
export function cutExportResource(cut: Pick<CutService, 'resolveExportFile'>): McpResourceTemplateDefinition {
  return { ...CUT_EXPORT_RESOURCE_OPTIONS, read: (args, context) => readCutExport(cut, args, context) }
}

export async function readCutExport(
  cut: Pick<CutService, 'resolveExportFile'>,
  args: Record<string, string>,
  context: ResourceReadContext
) {
  const { projectId, exportId } = z
    .object({ projectId: z.string().uuid(), exportId: z.string().uuid() })
    .strict()
    .parse(args)
