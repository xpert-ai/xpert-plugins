...
import {
  Badge, Button, Input, Progress, ResizableHandle, ResizablePanel, ResizablePanelGroup, ScrollArea, Slider,
...
import {
  ...
  Film, Image, Layers3, Magnet, Maximize2, Music2, PanelLeftClose, PanelRightClose, Pause, Play, Plus, Redo2,
...
import { canExportCutMp4, exportCutMp4 } from './cut-exporter'
...
function App() {
  ...
  const queueHeadlessExport = async () => {
    const project = data.detail?.item
    ...
    try {
      await executeAction('cut_start_headless_export', project.id, {
        ...
        changeSummary: 'Queued a revision-bound Cut headless MP4 export from Workbench.'
      })
      await load(project.id, true)
  ...
  const exportMp4 = async () => {
    ...
    setExportProgress(0)
    try {
      if (!await canExportCutMp4(documentDraft.settings.width, documentDraft.settings.height)) throw new Error(t('exportUnsupported'))
      const blob = await exportCutMp4(exportCanvasRef.current, documentDraft, setExportProgress)
      const file = new File([blob], `${safeFileName(data.detail.item.title)}.mp4`, { type: 'video/mp4' })
      await executeFileAction('cut_save_export_file', data.detail.item.id, {
        projectId: data.detail.item.id, changeSummary: t('exportedSummary')
      }, file)
      await load(data.detail.item.id, true)
      notify('success', t('exportSuccess'))
    } catch (error) { notify('error', errorText(error)) } finally { setSaving(false); setExportProgress(null) }
  }
  ...
  return <div className="cut-app">
    ...
    <ResizablePanelGroup orientation="horizontal" className="cut-editor-grid">
      ...
      <aside className="cut-library-panel">
        <Tabs defaultValue="media" className="cut-library-tabs">
          ...
          <div className="cut-library-content">
            ...
            <TabsContent value="captions" className="cut-library-pane"><div className="caption-workflow">
              ...
              <div className="analysis-job-list">{data.analysisJobs.map((job) => <div key={job.id} className={`analysis-job ${job.status}`}>
                <div><strong>{job.type === 'render' ? `MP4 · ${job.variantName ?? 'default'}` : `${job.executionMode === 'server' ? 'STT' : job.executionMode.toUpperCase()} · ${(job.language ?? 'und').toUpperCase()}`}</strong><small>{job.stage ?? job.status} · {job.progress}%{job.resultExportId ? ` · ${t('headlessSaved')}` : ''}</small></div>
                {job.errorMessage && <span title={job.errorMessage}>{job.errorMessage}</span>}
...
function coerceViewData(value: RemoteValue | null): CutViewData {
  ...
  return {
    ...
    analysisJobs: Array.isArray(value.analysisJobs) ? value.analysisJobs as CutViewData['analysisJobs'] : [],
