...
function hostHtml(useRealWhisper) {
  ...
  function workbenchData(analysisJobs=state.analysisJobs){return {projects:{items:[{...state.item}],total:1,page:1,pageSize:20},detail:detail(),captionDrafts:structuredClone(state.captionDrafts),analysisJobs:structuredClone(analysisJobs),mediaSegments:structuredClone(state.mediaSegments),editProposals:structuredClone(state.editProposals),renderCapability:{available:true,backend:'sandbox-job',action:'cut.render-mp4',actionVersion:'1.1.6',runtimeProfile:'browser/video-playwright-1.61/v1',workerCount:1,limits:{maxVariants:5,maxDurationSeconds:600,maxFrames:18000,maxWidth:3840,maxHeight:2160,maxFps:60,maxMediaBytes:4294967296}}}}
  function viewData(){
