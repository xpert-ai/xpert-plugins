    ...
    async (input) => {
      try {
        ...
        if (fileUrl) {
          ...
          if (!resp.ok) {
            return `Error: Failed to download PDF from URL: ${resp.statusText}`
          }
        ...
        let markdown = `# PDF Converted to Markdown\\n\\n` + `> Source File: ${fileName}\\n\\n` + `> Pages: ${pageCount}\\n\\n`

        const images: { fileName: string; filePath: string; fileUrl?: string; mimeType: string; page: number }[] = []

        const renderScale = typeof scale === 'number' && scale > 0 ? scale : 2.0
        ...
        for (let i = 0; i < pageCount; i++) {
          ...
          images.push({
            ...
            fileUrl: baseUrl ? new URL(groupName + '/' + imgFileName, baseUrl).href : undefined,
            mimeType: 'image/png',
            page: i + 1,
          })
        }
        ...
        const markdownInfo = {
          ...
          filePath: mdFullPath,
          fileUrl: baseUrl ? new URL(groupName + '/' + mdFileName, baseUrl).href : undefined,
        ...
        return 'Error converting PDF: ' + (e?.message || String(e))
      }
    },
    {
      name: 'pdf_to_markdown',
      description: 'Convert a PDF file into a markdown file with extracted text and rendered page images. Returns markdown and images file list.',
      schema: z.object({
        fileName: z.string().optional().nullable(),
        filePath: z.string().optional().nullable(),
        fileUrl: z.string().optional().nullable(),
        content: z.union([z.string(), z.instanceof(Buffer), z.instanceof(Uint8Array)]).optional().nullable(),
        scale: z.number().optional().describe('Rendering scale for images, default 2.0')
      })
    }
  )
}
