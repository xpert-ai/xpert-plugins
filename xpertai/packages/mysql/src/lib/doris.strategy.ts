...
import { Connection, createConnection, Pool } from 'mysql'; // The mysql2 package cannot connect to Doris.
...
export class DorisRunner extends MySQLRunner<DorisAdapterOptions> {
  ...
  override jdbcUrl(schema?: string) {
    return `jdbc:mysql://${this.options.host}:${
      this.options.port || DORIS_DEFAULT_PORT
    }/${schema}?user=${encodeURIComponent(
  ...
  override async import(
    ...
    try {
      ...
      if (mergeType === 'DELETE') {
        ...
        if (!keys.length) {
          throw new Error(
            `Olap table should use key fields for 'DISTRIBUTED BY HASH'`
          );
        }
        ...
        const createTableStatement = `CREATE TABLE IF NOT EXISTS \`${name}\` (${columns
          .map(
            (col) =>
              `\`${col.fieldName}\` ${typeToDorisDB(
                col.type,
                col.length,
                col.fraction
              )}`
          )
          .join(', ')}) UNIQUE KEY (${keys.join(
          ','
        )}) COMMENT "${name}" DISTRIBUTED BY HASH(${keys.join(
          ','
