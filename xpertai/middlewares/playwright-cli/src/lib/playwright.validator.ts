...
@Injectable()
export class PlaywrightCLISkillValidator {
  ...
  handle(event: XpertDraftValidateEvent) {
    ...
    agentNodes.forEach((agentNode) => {
      ...
      middlewareEntities
        .filter(({ entity }) => entity.provider === PLAYWRIGHT_CLI_SKILL_MIDDLEWARE_NAME)
        .forEach(({ node, entity }) => {
          if (!sandboxEnabled) {
            items.push({
              node: node.key,
              ...
              level: 'warning',
              message: {
                en_US: 'PlaywrightCLISkill requires the agent sandbox feature to be enabled.',
                zh_Hans: 'PlaywrightCLISkill 需要先启用智能体的 sandbox 功能。'
              }
            })
          }
          ...
          if (!hasSandboxShell) {
            items.push({
              ...
              level: 'warning',
              message: {
                en_US: 'PlaywrightCLISkill should be paired with the SandboxShell middleware on the same agent.',
                zh_Hans: 'PlaywrightCLISkill 需要与同一智能体上的 SandboxShell 中间件配合使用。'
              }
            })
          }
        })
