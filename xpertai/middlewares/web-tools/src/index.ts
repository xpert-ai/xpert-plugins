...
import { WebToolsPlugin } from './lib/web-tools.module.js';
...
const plugin: XpertPlugin<z.infer<typeof ConfigSchema>> = {
  meta: {
    ...
    displayName: 'Web Tools Middleware',
    description: 'A middleware that provides web fetch and web search tools for agents. Enables fetching web page content (as markdown, text, or HTML) and searching the internet via Exa MCP.',
    keywords: ['web', 'fetch', 'search', 'middleware'],
    author: 'XpertAI Team',
  },
  config: {
    schema: ConfigSchema,
  ...
  register(ctx) {
    ctx.logger.log('register web tools middleware plugin');
    return { module: WebToolsPlugin, global: true };
  },
  async onStart(ctx) {
    ctx.logger.log('web tools middleware plugin started');
  },
  async onStop(ctx) {
    ctx.logger.log('web tools middleware plugin stopped');
  },
};
