...
export const editingPerformSchema = z
  .object({
    ...
    operations: z
      .array(
        z
          .object({
            type: z.enum(['insert_text', 'replace_text', 'delete_element', 'set_property']),
            targetId: boundedId,
  ...
  .strict()
export const editingCommitSchema = z
  .object({ designId: boundedId, transactionId: boundedId, operationId, confirmation })
  .strict()
export const editingCancelSchema = z.object({ designId: boundedId, transactionId: boundedId }).strict()
export const exportSchema = z
  .object({ designId: boundedId, format: z.enum(['png', 'jpg', 'jpeg', 'pdf', 'pptx']), operationId, confirmation })
  .strict()
export const importSchema = z
  .object({
    url: z.string().trim().url().max(2_000).refine(isSafeImportUrl, 'URL must be a public HTTPS URL'),
    operationId,
    confirmation
  })
  .strict()
