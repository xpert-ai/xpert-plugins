...
export type DesignSummary = { id: string; title: string | null; type: string | null; status: string | null; updatedAt: string | null; hasThumbnail: boolean }
...
export function mapDesignList(value: CanvaPayload, page: number, pageSize: number) {
  ...
  return { items, page, pageSize, hasMore: Boolean(nextCursor) || items.length === pageSize, nextCursor: nextCursor ?? null }
}
...
export function mapPages(value: CanvaPayload) {
  ...
  return { items: rawItems.filter(isRecord).slice(0, CANVA_RESPONSE_MAX_ITEMS).map((page) => ({
...
export function mapContent(value: CanvaPayload) {
  ...
  return { designId: stringValue(value.design_id ?? value.designId) ?? null, text, elements: mapElements(value.elements) }
}
  ...
  const rawItems = Array.isArray(designCompositions)
    ...
    : Array.isArray(value.candidates)
      ...
      : Array.isArray(value.items)
        ? value.items
        : []
  const items = rawItems.filter(isRecord).slice(0, 20).map((candidate, index) => ({
    index: index + 1,
...
export function mapFormats(value: CanvaPayload) {
  ...
  return { formats: formats.filter(isRecord).slice(0, 30).map((format) => ({
...
export function mapJob(value: CanvaPayload) {
  return { jobId: stringValue(value.job_id ?? value.jobId ?? value.id) ?? null, status: boundedString(value.status) ?? 'unknown', progress: integerValue(value.progress), failureCode: boundedString(value.failure_code ?? value.error_code), nextAction: boundedString(value.next_action ?? value.nextAction) }
}

export function readRecord(value: unknown): CanvaPayload | null { return isRecord(value) ? value : null }
export function stringValue(value: unknown) { return typeof value === 'string' && value.trim() ? value.trim().slice(0, CANVA_RESPONSE_MAX_TEXT) : undefined }
function boundedString(value: unknown) { return stringValue(value) ?? null }
function integerValue(value: unknown) { return typeof value === 'number' && Number.isSafeInteger(value) ? value : null }
function stringArray(value: unknown) { return Array.isArray(value) ? value.filter((item): item is string => typeof item === 'string').map((item) => item.slice(0, 160)) : [] }
function isRecord(value: unknown): value is CanvaPayload { return typeof value === 'object' && value !== null && !Array.isArray(value) }
function mapElements(value: unknown) { return Array.isArray(value) ? value.filter(isRecord).slice(0, 50).map((element) => ({ id: stringValue(element.id) ?? null, type: boundedString(element.type), text: boundedString(element.text) })) : [] }
function hasCandidatePreview(candidate: CanvaPayload) {
  if (typeof candidate.url === 'string' || typeof candidate.preview_url === 'string' || typeof candidate.thumbnail_url === 'string') return true
  const thumbnail = readRecord(candidate.thumbnail)
  if (typeof thumbnail?.url === 'string') return true
  return Array.isArray(candidate.thumbnails) && candidate.thumbnails.some((thumbnail) => isRecord(thumbnail) && typeof thumbnail.url === 'string')
}
function safeCanvaUrl(value: unknown) {
  ...
  try {
    ...
    const isCanvaHost = host === 'canva.com' || host.endsWith('.canva.com') || host === 'canva.cn' || host.endsWith('.canva.cn') || host === 'khsj.cn' || host.endsWith('.khsj.cn')
