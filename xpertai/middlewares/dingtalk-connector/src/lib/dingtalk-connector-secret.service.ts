...
@Injectable()
export class DingTalkConnectorSecretService {
  constructor(private readonly configService: ConfigService) {}

  encrypt(value: unknown): string {
    const secret = requireText(value, 'DingTalk OAuth Client Secret is required.')
  ...
  decrypt(value: unknown): string {
    const encrypted = requireText(value, 'DingTalk OAuth Client Secret is required.')
  ...
  private key(): Buffer {
    ...
    if (!configured || configured === DEFAULT_KEY) {
      throw new Error('SECRETS_ENCRYPTION_KEY must be configured to decrypt DingTalk OAuth credentials.')
    }
