...
import {
  QQ_MAIL_PROTOCOL_DEFAULT_SEARCH_LIMIT,
  QQ_MAIL_PROTOCOL_MAX_ATTACHMENT_BYTES,
  QQ_MAIL_PROTOCOL_MAX_BODY_BYTES,
  QQ_MAIL_PROTOCOL_MAX_BODY_CHARS,
  QQ_MAIL_PROTOCOL_MAX_FOLDER_COUNT,
  QQ_MAIL_PROTOCOL_MAX_SEARCH_LIMIT,
  QQ_MAIL_PROTOCOL_MAX_SEARCH_MATCHES,
...
@Injectable()
export class QqMailProtocolService {
  constructor(
  ...
  async verifyCredential(credential: QqMailProtocolCredential): Promise<void> {
  ...
  async listFolders(credential: QqMailProtocolCredential, input: ListMailFoldersInput) {
    const limit = Math.min(input.limit ?? 100, QQ_MAIL_PROTOCOL_MAX_FOLDER_COUNT)
  ...
  async searchEmails(credential: QqMailProtocolCredential, input: SearchEmailsInput): Promise<SearchEmailsResult> {
    const folder = input.folder?.trim() || 'INBOX'
    const limit = Math.min(input.limit ?? QQ_MAIL_PROTOCOL_DEFAULT_SEARCH_LIMIT, QQ_MAIL_PROTOCOL_MAX_SEARCH_LIMIT)
    ...
    return this.withImap(credential, async (client) => {
      ...
      if (cursor?.folder !== undefined && cursor.folder !== folder) {
        throw new QqMailProtocolError('MAIL_REFERENCE_INVALID', 'The search cursor belongs to another mail folder.')
      }
      if (cursor && cursor.uidValidity !== uidValidity) {
        throw new QqMailProtocolError(
          'MAIL_REFERENCE_STALE',
          'The mail folder changed and the search must be restarted.'
        )
      }
      ...
      const criteria = buildSearchCriteria(input, cursor?.beforeUid)
      const matches = (await client.search(criteria, { uid: true })) || []
      const orderedUids = matches.slice(-QQ_MAIL_PROTOCOL_MAX_SEARCH_MATCHES).sort((left, right) => right - left)
      const selectedUids = orderedUids.slice(0, limit)
      ...
      const items = selectedUids
        ...
        .filter((message): message is FetchMessageObject => !!message)
        .map((message) => this.toSummary(folder, uidValidity, message))
      const hasMore = orderedUids.length > limit
      const lastUid = selectedUids.at(-1)
  ...
  async getEmail(credential: QqMailProtocolCredential, messageRef: string): Promise<MailDetailDto> {
  ...
  async downloadAttachment(
    ...
    return this.withReferencedMessage(credential, reference, true, async (client, message) => {
      ...
      if (!attachment) {
        throw new QqMailProtocolError('MAIL_ATTACHMENT_NOT_FOUND', 'The requested attachment was not found.')
      }
      if (attachment.size && attachment.size > QQ_MAIL_PROTOCOL_MAX_ATTACHMENT_BYTES) {
        throw new QqMailProtocolError(
          'MAIL_ATTACHMENT_TOO_LARGE',
          `The attachment exceeds the ${QQ_MAIL_PROTOCOL_MAX_ATTACHMENT_BYTES}-byte download limit.`
        )
      }

      const downloaded = await client.download(reference.uid, attachmentId, {
        uid: true,
        maxBytes: QQ_MAIL_PROTOCOL_MAX_ATTACHMENT_BYTES + 1
      })
      const buffer = await streamToBuffer(downloaded.content)
      if (buffer.length > QQ_MAIL_PROTOCOL_MAX_ATTACHMENT_BYTES) {
        throw new QqMailProtocolError(
          'MAIL_ATTACHMENT_TOO_LARGE',
          `The attachment exceeds the ${QQ_MAIL_PROTOCOL_MAX_ATTACHMENT_BYTES}-byte download limit.`
        )
  ...
  async sendEmail(
    ...
    for (const descriptor of input.attachments ?? []) {
      ...
      if (!file.buffer.length || totalAttachmentBytes > QQ_MAIL_PROTOCOL_MAX_TOTAL_ATTACHMENT_BYTES) {
        throw new QqMailProtocolError(
          'MAIL_ATTACHMENT_TOO_LARGE',
          `Email attachments must be non-empty and total no more than ${QQ_MAIL_PROTOCOL_MAX_TOTAL_ATTACHMENT_BYTES} bytes.`
        )
      }
      attachments.push({
    ...
    try {
      ...
      if (!accepted.length) {
        throw new QqMailProtocolError('MAIL_SEND_REJECTED', 'The SMTP server did not accept any recipients.')
      }
      return {
  ...
  private async withReferencedMessage<T>(
    ...
    return this.withImap(credential, async (client) => {
      ...
      if (mailbox.uidValidity.toString() !== reference.uidValidity) {
        throw new QqMailProtocolError('MAIL_REFERENCE_STALE', 'The mail folder changed and this reference is stale.')
      }
      ...
      if (!message) {
        throw new QqMailProtocolError('MAIL_MESSAGE_NOT_FOUND', 'The referenced email no longer exists.')
      }
...
function buildSearchCriteria(input: SearchEmailsInput, beforeUid?: number): SearchObject {
  ...
  if (rangeEnd <= since) {
    throw new QqMailProtocolError('MAIL_QUERY_INVALID', 'The search before date must be after the since date.')
  }
  if (rangeEnd.getTime() - since.getTime() > 366 * 24 * 60 * 60 * 1_000) {
    throw new QqMailProtocolError('MAIL_QUERY_INVALID', 'The search date range cannot exceed 366 days.')
  }
  const criteria: SearchObject = {
    ...
    ...(input.query
      ? input.searchIn === 'SEARCH_IN_SUBJECT'
        ? { subject: input.query }
        : input.searchIn === 'SEARCH_IN_CONTENT'
        ? { body: input.query }
        : { text: input.query }
      : {}),
...
async function downloadPreferredBody(
  ...
  const download = await client.download(uid, part, { uid: true, maxBytes: QQ_MAIL_PROTOCOL_MAX_BODY_BYTES })
  ...
  return {
    text: normalized.slice(0, QQ_MAIL_PROTOCOL_MAX_BODY_CHARS),
    truncated: buffer.length >= QQ_MAIL_PROTOCOL_MAX_BODY_BYTES || normalized.length > QQ_MAIL_PROTOCOL_MAX_BODY_CHARS
  }
}
