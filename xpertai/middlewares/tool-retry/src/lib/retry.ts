...
export const retryBaseSchemaObject = z.object({
...
export const validateRetryMatchers = (
  data: z.infer<typeof retryBaseSchemaObject>,
  ctx: z.RefinementCtx
) => {
  const hasMatchers =
  ...
  if (!data.retryAllErrors && !hasMatchers) {
    ctx.addIssue({
      ...
      path: ['retryAllErrors'],
      message: 'When retryAllErrors is false, at least one retry matcher must be configured.',
    })
  }
}

export const retryBaseSchema = retryBaseSchemaObject.superRefine(validateRetryMatchers)
