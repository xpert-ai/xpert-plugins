...
describe('MinerUClient', () => {
  ...
  it('should upload local files through the official signed-upload endpoint', async () => {
    const { instance: configService } = createConfigServiceMock();
    const client = new MinerUClient(configService, { integration: integrationOptions });
    const postSpy = jest.spyOn(axios, 'post').mockResolvedValue({
      data: {
        ...
        msg: 'ok',
        data: { batch_id: 'batch-upload', file_urls: ['https://upload.test/file'] },
      },
      status: 200,
      statusText: 'OK',
      headers: {},
    ...
    await expect(
      client.createUploadBatch({
        files: [{ name: 'document.pdf', buffer, dataId: 'doc.part-0001', isOcr: true }],
        enableFormula: true,
        enableTable: true,
        language: 'ch',
        modelVersion: 'vlm',
      })
    ).resolves.toEqual({ batchId: 'batch-upload' });

    expect(postSpy).toHaveBeenCalledWith(
      'https://custom-mineru.test/api/v4/file-urls/batch',
      {
        files: [{ name: 'document.pdf', data_id: 'doc.part-0001', is_ocr: true }],
        enable_formula: true,
        enable_table: true,
        language: 'ch',
        model_version: 'vlm',
  ...
  it('should wait for all uploaded files and surface official file failures', async () => {
    const { instance: configService } = createConfigServiceMock();
    const client = new MinerUClient(configService, { integration: integrationOptions });
    jest.spyOn(client, 'getBatchResult').mockResolvedValueOnce({
      batch_id: 'batch-1',
      extract_result: [{ file_name: 'document.pdf', state: 'done', full_zip_url: 'https://cdn/result.zip' }],
    });
    await expect(client.waitForBatch('batch-1', 1, 1000, 0)).resolves.toEqual([
      expect.objectContaining({ state: 'done', full_zip_url: 'https://cdn/result.zip' }),
    ]);

    jest.spyOn(client, 'getBatchResult').mockResolvedValueOnce({
      batch_id: 'batch-2',
      extract_result: [
        { file_name: 'bad.pdf', state: 'failed', err_code: -60006, err_msg: 'page count exceeds limit' },
      ],
    });
    await expect(client.waitForBatch('batch-2', 1, 1000, 0)).rejects.toThrow(
      'page count exceeds limit [code=-60006, batchId=batch-2, fileName=bad.pdf]'
    );
  });
  ...
  it('should poll until task provides a finished result', async () => {
    ...
    const finalResult = { state: 'done', full_zip_url: 'https://cdn/result.zip' };
  ...
  const itUsesLiveSelfHosted = process.env[ENV_MINERU_API_BASE_URL] ? it : it.skip;

  itUsesLiveSelfHosted('should upload file and cache result for self-hosted deployments', async () => {
    const { instance: configService } = createConfigServiceMock();
  ...
  it('should retry self-hosted parse with singular file field when required', async () => {
    ...
    const client = new MinerUClient(configService, {
      ...
      fileSystem: {
        readFile: (filePath: string) => new Promise<Buffer>((resolve, reject) => {
  ...
  it('should require a base URL for self-hosted mode', () => {
    const { instance: configService } = createConfigServiceMock({
      [ENV_MINERU_SERVER_TYPE]: 'self-hosted',
    });

    expect(() => new MinerUClient(configService)).toThrow(
      'MinerU self-hosted mode requires apiUrl to be configured'
    );
  });
