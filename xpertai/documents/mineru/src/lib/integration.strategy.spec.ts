...
describe('MinerUIntegrationStrategy', () => {
  ...
  it('owns parse options and separates official and self-hosted fields', () => {
    const schema = strategy.meta.schema
    expect(schema['x-ui']).toMatchObject({ cols: 3 })
    expect(schema.properties.modelVersion).toMatchObject({
      default: 'vlm',
      'x-ui': { visibleWhen: { name: 'serverType', value: 'official' } }
    })
    expect(schema.properties.selfHostedBackend).toMatchObject({
      default: 'pipeline',
      'x-ui': { visibleWhen: { name: 'serverType', value: 'self-hosted' } }
    })
    expect(schema.properties.parseMethod.enum).toEqual(['auto', 'ocr', 'txt'])
    for (const field of ['enableFormula', 'enableTable', 'isOcr']) {
      expect(schema.properties[field]).toMatchObject({ default: true, 'x-ui': { component: 'checkbox', span: 1 } })
    }
    expect(schema.properties).not.toHaveProperty('preserveRawOutput')
  })

  it('should throw error for unimplemented execute', async () => {
    await expect(strategy.execute({} as any, {} as any)).rejects.toThrow('does not expose executable actions')
  })
  ...
  itValidatesLiveConfig('should validate config successfully', async () => {
    await expect(strategy.validateConfig(minerUOptions)).resolves.toBeUndefined()
  })
})
