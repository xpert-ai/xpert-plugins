import { type IIntegration, type JsonSchemaUIExtensions, TIntegrationProvider } from '@xpert-ai/contracts'
...
import { minerUParseOptionsSchema } from './parse-options.schema.js'
import { icon, MinerU, MinerUIntegrationOptions } from './types.js'

const minerUIntegrationSchema: TIntegrationProvider['schema'] & { 'x-ui': JsonSchemaUIExtensions } = {
  type: 'object',
  secret: ['apiKey'],
  'x-ui': { cols: 3 },
  properties: {
    ...
    apiUrl: {
      ...
      description: {
        en_US:
          'Official default: https://mineru.net/api/v4. For self-hosted mode, enter mineru-api or mineru-router base URL.',
        ja_JP: 'MinerUサーバのBase URLを入力してください',
        zh_Hans: '官方默认 https://mineru.net/api/v4；自托管模式请输入 mineru-api 或 mineru-router 的服务地址。'
      },
      'x-ui': { span: 3, component: 'textInput' }
    },
    apiKey: {
      type: 'string',
      title: {
      ...
      'x-ui': {
        ...
        maskSymbol: '*',
        persist: true
      } satisfies ISchemaSecretField & { span: number }
    },
    ...minerUParseOptionsSchema.properties,
    uploadMode: {
      type: 'string',
      title: {
        en_US: 'Source Mode',
      ...
      description: {
        en_US:
          'Auto uploads workspace files through MinerU signed URLs and only falls back to a public URL when no local file is available.',
    ...
    pollIntervalSeconds: {
      ...
      default: 5,
      'x-ui': { span: 3, component: 'numberInput', visibleWhen: { name: 'serverType', value: 'official' } }
    },
    taskTimeoutSeconds: {
      type: 'number',
      title: { en_US: 'Task Timeout (seconds)', zh_Hans: '任务超时（秒）' },
      default: 1800,
      'x-ui': { span: 3, component: 'numberInput' }
    },
...
export class MinerUIntegrationStrategy implements IntegrationStrategy<MinerUIntegrationOptions> {
  readonly meta: TIntegrationProvider = {
    ...
    schema: minerUIntegrationSchema,
    features: [],
  ...
  async validateConfig(config: MinerUIntegrationOptions): Promise<void> {
    ...
    validateMinerUParseOptions(resolveMinerUParseOptions({}, config), mineruClient.serverType)
