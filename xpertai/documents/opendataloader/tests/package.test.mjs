...
test('installable package includes its Sandbox Action declaration and every referenced bundle file', async () => {
  ...
  assert.equal(action.name, 'opendataloader.convert')
  assert.equal(action.version, '1.2.1')
  ...
  for (const required of [
    ...
    'dist/sandbox-actions/convert/bundle/ocr.mjs',
    'dist/_assets/icon.webp'
  ]) {
...
test('plugin card and parser metadata use the bundled official artwork without a network request', async () => {
  const bytes = await readFile(new URL('dist/_assets/icon.webp', root))
  const expected = { type: 'image', value: `data:image/webp;base64,${bytes.toString('base64')}` }
  assert.deepEqual(plugin.meta.icon, expected)
  assert.deepEqual(new OpenDataLoaderTransformerStrategy({}).meta.icon, expected)
  assert.deepEqual(bytes, await readFile(new URL('src/_assets/icon.webp', root)))
})
