...
async function fixture(t) {
  const root = await mkdtemp(path.join(tmpdir(), 'anydoc-retry-'))
...
for (const code of ['RESOURCE_LIMIT', 'RUNTIME_INVALID', 'EMPTY_TEXT', 'UNSUPPORTED_ENCODING']) {
  test(`a ${code} failure is retryable after recovery and only success is reused`, async (t) => {
    ...
    await assert.rejects(converter.convert('x', 'pdf', options), (error) => error.message === 'ANYDOC_' + code)
...
test('a failed Job retains its bounded parser error code and affected pages', async (t) => {
  ...
  await assert.rejects(converter.convert('x', 'pdf', options), (error) => {
    assert.equal(error.message, 'ANYDOC_NEEDS_OCR')
...
test('unrelated or malformed Runtime errors are preserved', async (t) => {
  ...
  for (const message of [
    ...
    'ANYDOC_CONVERSION_ERROR: not-json',
    'ANYDOC_CONVERSION_ERROR: {"ok":false,"code":"UNTRUSTED_CODE"}',
    'ANYDOC_CONVERSION_ERROR: {"ok":false,"code":"NEEDS_OCR","pages":[-1]}'
  ]) {
...
test('Action patch revision avoids previously cached conversion failures', () => {
  assert.equal(ACTION_VERSION, '1.0.3')
})
