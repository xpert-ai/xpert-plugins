...
export type AnyDocFileScope = WorkspaceFileScope & { organizationId?: string | null }
...
@Injectable()
export class AnyDocSandboxConverter {
  constructor(
    @Optional()
    ...
    if (!health.available)
      throw new ServiceUnavailableException(
        health.reason === 'ACTION_MISSING'
          ...
          : `AnyDoc Sandbox Runtime is unavailable (${health.reason ?? 'RUNTIME_UNBOUND'}). ${
  ...
  async convert(
    ...
    if (!FILE_TYPES.includes(extension)) throw new Error(`AnyDoc does not support this file type: ${extension}`)
    ...
    if (!source.buffer.length) throw new Error('ANYDOC_EMPTY_FILE')
    if (source.buffer.length > 100 * 1024 * 1024) throw new Error('ANYDOC_INPUT_TOO_LARGE')
    ...
    try {
      ...
      if (content.buffer.length > 128 * 1024 * 1024) throw new Error('ANYDOC_OUTPUT_TOO_LARGE')
      ...
      if (converted.ok === false) throw Object.assign(new Error(`ANYDOC_${converted.code}`), { pages: converted.pages })
      ...
      for (const asset of converted.assets) {
        ...
        if (decodedSize > 128 * 1024 * 1024) throw new Error('ANYDOC_OUTPUT_TOO_LARGE')
      }
      if (converted.pages) {
      ...
      if (!converted.markdown.trim()) throw new Error('ANYDOC_EMPTY_TEXT')
  ...
  private runtime() {
    ...
    if (!jobs || !files)
      throw new ServiceUnavailableException(
        'AnyDoc requires platform Sandbox Jobs and Workspace Files. Update and restart the API.'
      )
    return { jobs, files }
  }
}
...
function rethrowConversionFailure(error: unknown): never {
  if (isSandboxJobRuntimeError(error)) {
    ...
    if (failure) throw Object.assign(new Error(`ANYDOC_${failure.code}`), { pages: failure.pages })
  }
  throw error
}
