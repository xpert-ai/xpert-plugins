export type QqMailProtocolErrorCode =
...
export class QqMailProtocolError extends Error {
  constructor(readonly code: QqMailProtocolErrorCode, message: string, options?: ErrorOptions) {
    super(`[${code}] ${message}`, options)
    this.name = 'QqMailProtocolError'
  }
}

export function normalizeMailConnectionError(error: unknown, protocol: 'imap' | 'smtp'): QqMailProtocolError {
  ...
  if (
    ...
    return new QqMailProtocolError(
      'MAIL_AUTH_FAILED',
  ...
  return new QqMailProtocolError(
    ...
    protocol === 'imap'
      ? 'IMAP access is unavailable. Enable IMAP/SMTP in QQ Mail and create a client authorization code.'
      : 'SMTP access is unavailable. Enable IMAP/SMTP in QQ Mail and create a client authorization code.',
    error instanceof Error ? { cause: error } : undefined
  )
}
...
export function normalizeMailSendError(error: unknown): QqMailProtocolError {
