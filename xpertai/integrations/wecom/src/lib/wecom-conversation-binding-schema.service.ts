...
const USER_ID_INDEX = 'plugin_wecom_conversation_binding_user_id_idx'

@Injectable()
export class WeComConversationBindingSchemaService {
  private readonly logger = new Logger(WeComConversationBindingSchemaService.name)
  ...
  async ensureSchema(): Promise<void> {
    ...
    try {
      ...
      if (!table) {
        this.logger.warn(
          `[wecom-bind] Table "${WeComConversationBindingEntity.tableName}" is missing, skip schema ensure.`
        )
        return
      }
      ...
      if (actions.length) {
        this.logger.log(`[wecom-bind] Ensured schema: ${actions.join(', ')}`)
      }
    } finally {
  ...
  private async loadTable(queryRunner: QueryRunner): Promise<Table | undefined> {
    return (await queryRunner.getTable(WeComConversationBindingEntity.tableName)) ?? undefined
  }
  ...
  private async dropUniqueSemantics(
    ...
    for (const index of table.indices) {
      ...
      await queryRunner.dropIndex(WeComConversationBindingEntity.tableName, index)
    ...
    for (const unique of table.uniques) {
      ...
      await queryRunner.dropUniqueConstraint(WeComConversationBindingEntity.tableName, unique)
