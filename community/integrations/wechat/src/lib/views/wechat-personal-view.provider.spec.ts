...
import { WechatPersonalViewProvider } from './wechat-personal-view.provider.js'
...
describe('WechatPersonalViewProvider', () => {
    ...
    const manifests = await provider.getViewManifests(
      {
        ...
        hostSnapshot: {
          ...
          type: WECHAT_PERSONAL_PROVIDER_KEY
        }
      } as any,
      'detail.main_tabs'
    )

    expect(manifests.map((manifest) => manifest.key)).toEqual([WECHAT_PERSONAL_VIEW_KEY])
  ...
  it('keeps feature activation for agent workbench manifests', async () => {
    ...
    expect(manifests.map((manifest) => manifest.key)).toEqual([WECHAT_PERSONAL_VIEW_KEY])
    expect(manifests[0].activation?.requiredFeatures).toEqual([WECHAT_PERSONAL_FEATURE])
  })
