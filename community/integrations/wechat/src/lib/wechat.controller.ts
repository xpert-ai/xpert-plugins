...
type WechatWebhookPrincipalContext = {
  user: IApiPrincipal
...
export class WechatController {
  ...
  constructor(
    private readonly conversation: WechatConversationService,
  ...
  async webhook(
    ...
    const ctx: TChatEventContext<TIntegrationWechatOptions> = {
      integration,
  ...
  async getIntegrationSelectOptions(@Query('keyword') keyword?: string) {
    const options = await this.listWechatSelectOptions()
  ...
  private async readWechatIntegration(
    integrationId: string
  ): Promise<IIntegration<TIntegrationWechatOptions>> {
    ...
    if (!integration) {
      throw new BadRequestException(
        `Integration ${integrationId} not found. Please save the WeChat integration first.`
      )
    }
    if (integration.provider !== WECHAT_PROVIDER_KEY) {
      throw new BadRequestException(
        `Integration ${integrationId} is not provider '${WECHAT_PROVIDER_KEY}'`
      )
    }
    return integration
  }

  private async listWechatSelectOptions(): Promise<
  ...
  > {
    ...
    return (result.items ?? [])
      .filter((integration) => integration?.provider === WECHAT_PROVIDER_KEY)
      .map((integration) => ({
  ...
  ): WechatWebhookPrincipalContext {
    const user = req.user as IApiPrincipal | undefined
    if (!user?.apiKey) {
      throw new BadRequestException('WeChat webhook principal is required')
    }
