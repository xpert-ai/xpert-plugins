...
import {
  ...
  type WechatPersonalInboundVoiceRef
} from './types.js'
...
export interface WechatPersonalDownloadImageInput {
  uuid: string
  contactId: string
  newMsgId: string
  msgContent: string
...
export type WechatPersonalDownloadVoiceInput = WechatPersonalInboundVoiceRef
...
export type WechatPersonalDownloadedImageFile = WechatPersonalInboundFile &
  Required<
    Pick<
...
export interface WechatPersonalDownloadImageResult {
  success: boolean
  file?: WechatPersonalDownloadedImageFile
  error?: string
  raw?: unknown
}

export interface WechatPersonalDownloadedVoiceFile {
  data: Buffer
  mimeType: 'audio/wav'
  originalName: string
  fileKey: string
...
export interface WechatPersonalDownloadVoiceResult {
  success: boolean
  audio?: WechatPersonalDownloadedVoiceFile
  error?: string
  raw?: unknown
}

type WechatPersonalHistoryMessageRecord = Record<string, unknown>
...
export class WechatPersonalClient {
  ...
  async downloadImage(
    integration: IIntegration<TIntegrationWechatPersonalOptions>,
    input: WechatPersonalDownloadImageInput
  ): Promise<WechatPersonalDownloadImageResult> {
  ...
  async downloadVoice(
    integration: IIntegration<TIntegrationWechatPersonalOptions>,
    input: WechatPersonalDownloadVoiceInput
  ): Promise<WechatPersonalDownloadVoiceResult> {
  ...
  private shouldRetryVoiceDownloadWithHistory(result: WechatPersonalDownloadVoiceResult): boolean {
    if (result.success) {
      return false
    }
  ...
  private buildVoiceDownloadBody(input: WechatPersonalDownloadVoiceInput): Record<string, unknown> {
  ...
  private resolveDownloadedImage(
    ...
    input: WechatPersonalDownloadImageInput,
    preferHd: boolean
  ): WechatPersonalDownloadImageResult {
  ...
  private async downloadMediaChunks(
    integration: IIntegration<TIntegrationWechatPersonalOptions>,
    input: Pick<WechatPersonalDownloadVoiceInput, 'uuid' | 'newMsgId'>,
    variant: string
  ...
  private async resolveHistoryVoiceDownloadInput(
    integration: IIntegration<TIntegrationWechatPersonalOptions>,
    input: WechatPersonalDownloadVoiceInput
  ): Promise<WechatPersonalDownloadVoiceInput | null> {
  ...
  private findMatchingHistoryVoice(
    items: unknown[],
    input: WechatPersonalDownloadVoiceInput
  ): WechatPersonalHistoryMessageRecord | null {
    const voiceItems = items
      .map((item) => (item && typeof item === 'object' && !Array.isArray(item) ? (item as WechatPersonalHistoryMessageRecord) : null))
      .filter((item): item is WechatPersonalHistoryMessageRecord => {
  ...
  private buildVoiceDownloadInputFromHistory(
    input: WechatPersonalDownloadVoiceInput,
    item: WechatPersonalHistoryMessageRecord
  ): WechatPersonalDownloadVoiceInput {
  ...
  private isDifferentVoiceDownloadInput(
    left: WechatPersonalDownloadVoiceInput,
    right: WechatPersonalDownloadVoiceInput
  ): boolean {
    return (
  ...
  private historyNewMsgId(item: WechatPersonalHistoryMessageRecord): string {
    return normalizeString(item.newmsgid || item.newMsgId || item.Newmsgid || item.new_msg_id || item.newMsgID)
  }

  private historyContent(item: WechatPersonalHistoryMessageRecord): string {
    return (
  ...
  private withVoiceDownloadDiagnostics(error: string, input: WechatPersonalDownloadVoiceInput): string {
    const diagnostics = this.describeVoiceDownloadInput(input)
    return diagnostics ? `${error}; voiceDownloadInput{${diagnostics}}` : error
  }

  private describeVoiceDownloadInput(input: WechatPersonalDownloadVoiceInput): string {
