...
import {
  decodeWechatPersonalTunnelFrames,
  encodeWechatPersonalTunnelMessage,
  WechatPersonalTunnelBrokerService,
  WechatPersonalTunnelMessage
...
function createService(overrides: Record<string, unknown> = {}) {
  return new WechatPersonalTunnelBrokerService({
    config: {
...
function attachMemoryClient(service: WechatPersonalTunnelBrokerService) {
  const transport = new MemoryTunnelTransport()
  service.attachTransport(transport as any, { expectedClientId: 'client-1' })
  return transport
}

function readWrites(transport: MemoryTunnelTransport): WechatPersonalTunnelMessage[] {
  if (!transport.writes.length) {
    return []
  }
  return decodeWechatPersonalTunnelFrames(Buffer.concat(transport.writes)).messages
}

describe('WechatPersonalTunnelBrokerService', () => {
  let service: WechatPersonalTunnelBrokerService | null = null
  ...
  it('encodes and decodes wx2.0 length-prefixed JSON frames', () => {
    const frame = encodeWechatPersonalTunnelMessage({
      type: 'register',
      id: 'client-1',
      name: 'local'
    })
    const decoded = decodeWechatPersonalTunnelFrames(frame)
  ...
  it('rejects registration without client id', async () => {
    ...
    transport.receive(encodeWechatPersonalTunnelMessage({ type: 'register' }))
  ...
  it('times out pending HTTP RPC requests', async () => {
    ...
    transport.receive(encodeWechatPersonalTunnelMessage({ type: 'register', id: 'client-1' }))
  ...
  it('rejects websocket bridge connections whose register id does not match the path client id', async () => {
    ...
    transport.receive(encodeWechatPersonalTunnelMessage({ type: 'register', id: 'client-other' }))

    const ack = decodeWechatPersonalTunnelFrames(Buffer.concat(transport.writes)).messages[0]
    expect(ack).toEqual(
      expect.objectContaining({
        type: 'register_ack',
        ok: false
