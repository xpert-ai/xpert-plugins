...
export type WechatPersonalOutgoingContentPart =
  | {
      type: 'text'
      content: string
    }
...
export function parseWechatPersonalOutgoingContent(value: unknown): WechatPersonalOutgoingContentPart[] {
  ...
  const parts: WechatPersonalOutgoingContentPart[] = []
  let cursor = 0
...
function pushFormattedText(
  parts: WechatPersonalOutgoingContentPart[],
  rawText: string
): WechatPersonalOutgoingContentPart[] {
  const content = formatWechatPersonalOutgoingText(rawText)
  if (content) {
    parts.push({
      type: 'text',
      content
