...
import { WechatPersonalInboundQueueProcessor } from './wechat-personal-inbound-queue.processor.js'

describe('WechatPersonalInboundQueueProcessor', () => {
  it('allows multiple conversation keys to run while per-key Redis locks serialize aggregation', () => {
    const source = readFileSync(join(process.cwd(), 'src/lib/workflow/wechat-personal-inbound-queue.processor.ts'), 'utf8')
  ...
  it('delegates aggregate jobs to the trigger strategy', async () => {
    ...
    const processor = new WechatPersonalInboundQueueProcessor(triggerStrategy as any)
    const job = {
      name: WECHAT_PERSONAL_INBOUND_AGGREGATE_JOB,
      data: {
  ...
  it('delegates flush jobs to the trigger strategy', async () => {
    ...
    const processor = new WechatPersonalInboundQueueProcessor(triggerStrategy as any)
    const job = {
      name: WECHAT_PERSONAL_INBOUND_FLUSH_JOB,
      data: {
        aggregateKey: 'aggregate-1',
        version: 2
      }
