...
describe('WechatPersonalTriggerAggregationService', () => {
  ...
  function createService(redis = createRedis()) {
    ...
    const service = new WechatPersonalTriggerAggregationService(queue as any)
    return { service, queue, redis }
  }

  function createState(overrides: Partial<WechatPersonalTriggerAggregationState> = {}): WechatPersonalTriggerAggregationState {
    return {
  ...
  it('adds aggregate and flush jobs with plugin-prefixed ids', async () => {
    ...
    expect(queue.add).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        jobId: expect.stringMatching(/^plugin_wechat_personal_inbound_aggregate-/),
        attempts: 5
      })
    )
    expect(queue.add).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        jobId: expect.stringMatching(/^plugin_wechat_personal_inbound_flush-/),
        delay: 5000,
        attempts: 3
      })
    )
  ...
  it('stores aggregation state under plugin-prefixed Redis keys', async () => {
    ...
    expect(redis.set).toHaveBeenCalledWith(
      `plugin_wechat_personal:trigger:aggregate:${state.aggregateKey}`,
      JSON.stringify(state),
      'PX',
      60000
    )
    expect(redis.get).toHaveBeenCalledWith(`plugin_wechat_personal:trigger:aggregate:${state.aggregateKey}`)
    expect(redis.del).toHaveBeenCalledWith(`plugin_wechat_personal:trigger:aggregate:${state.aggregateKey}`)
  })
  ...
  it('runs callbacks while holding the per-aggregate Redis lock', async () => {
    ...
    expect(redis.set).toHaveBeenCalledWith(
      'plugin_wechat_personal:lock:inbound:aggregate-1',
      expect.any(String),
      'PX',
      3000,
      'NX'
    ...
    expect(redis.eval).toHaveBeenCalledWith(expect.stringContaining("redis.call('get'"), 1, 'plugin_wechat_personal:lock:inbound:aggregate-1', expect.any(String))
  })
