...
export class WechatWebsocketTunnelService implements OnGatewayConnection, OnGatewayDisconnect {
  private readonly logger = new Logger(WechatWebsocketTunnelService.name)
  private readonly transports = new WeakMap<Socket, SocketIoTunnelTransport>()

  constructor(private readonly broker: WechatTunnelBrokerService) {}
  ...
  handleConnection(client: Socket): void {
    ...
    this.logger.log(
      `[wechat-tunnel-ws] socket.io connected ${transport.remoteAddress} clientId=${expectedClientId || '<register>'}`
    )
  }
  ...
  private extractClientId(client: Socket): string | undefined {
    ...
    if (namespaceName.startsWith(WECHAT_TUNNEL_NAMESPACE_PREFIX)) {
      return decodeURIComponent(namespaceName.slice(WECHAT_TUNNEL_NAMESPACE_PREFIX.length))
    }
...
class SocketIoTunnelTransport extends EventEmitter implements WechatTunnelTransport {
