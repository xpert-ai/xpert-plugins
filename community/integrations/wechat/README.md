...
| 出站队列 | 否 | 开启 | 使用 Redis BullMQ 排队发送文本回复，支持延迟、限流、锁、取消、重试和暂停。 |
...
默认情况下，`sendTextByIntegrationId()` 不再直接调用 wx2.0。它会先格式化文本，创建一条 outbound message log，状态为 `queued`，然后投递 BullMQ job 到 Redis 队列：
...
真正的 wx2.0 `sendtext` 调用只发生在 BullMQ processor 中。发送成功后，message log 会更新为 `sent`，写入 wx2.0 `messageId` 和 `sentAt`；失败会按 BullMQ attempts/backoff 重试，最终失败会更新为 `failed`，并按失败保护规则暂停账号。
