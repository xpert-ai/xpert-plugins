...
const required = [
  "dist/lib/remote-components/factory-assistant-profile/app.js",
  "dist/lib/remote-components/factory-assistant-profile/app.css",
  "dist/lib/migrations/assistant-profile.migration.js",
  "dist/index.js",
  "dist/index.d.ts",
  ...
  "dist/lib/factory-mcp-apps.js",
  "dist/lib/factory-mcp-apps.d.ts",
  "dist/mcp-apps/dashboard/index.html",
  "dist/mcp-apps/case-summary/index.html",
...
const [
  ...
  readFile(resolve("dist/lib/factory-mcp-apps.js"), "utf8"),
  readFile(resolve("dist/mcp-apps/dashboard/index.html"), "utf8"),
  readFile(resolve("dist/mcp-apps/case-summary/index.html"), "utf8"),
]);

if (sourceYaml !== builtYaml) throw new Error("Built Assistant YAML is stale.");
if (sourceManagerYaml !== builtManagerYaml)
  throw new Error("Built Manager Assistant YAML is stale.");
const runtimeLevel = packageJson.xpert?.plugin?.level;
const runtimeNamespace = packageJson.xpert?.plugin?.artifactNamespace;
if (
  bundleJson.name !== packageJson.name ||
  bundleJson.version !== packageJson.version
) {
...
if (
  packageJson.peerDependencies?.["@xpert-ai/contracts"] !== "^3.18.0" ||
  packageJson.peerDependencies?.["@xpert-ai/plugin-sdk"] !== "^3.18.1" ||
  packageJson.devDependencies?.["@xpert-ai/contracts"] !== "3.18.0" ||
  packageJson.devDependencies?.["@xpert-ai/plugin-sdk"] !== "3.18.1"
) {
  throw new Error("Host-native MCP SDK dependency versions are inconsistent.");
}

const xpertTarget = bundleJson.targetAppMeta?.xpert;
const mcpContributions =
  xpertTarget?.marketplace?.contents?.filter((item) => item.type === "mcp") ??
  [];
const mcpProviders = new Map(
  mcpContributions.map((item) => [item.name, item.metadata?.provider])
);
if (
...
if (
  ...
  !builtConstants.includes(
    "FACTORY_INSIGHTS_TOOLSET_PROVIDER_KEY = 'factory_ops_insights'"
  ) ||
  !builtConstants.includes(
    "FACTORY_MCP_CAPABILITY = 'factory-operations-mcp'"
  ) ||
  !builtConstants.includes(
    "FACTORY_INSIGHTS_MCP_CAPABILITY = 'factory-operations-insights-mcp'"
  ) ||
  ...
  !builtTools.includes("FACTORY_INSIGHTS_MCP_APPS") ||
  !builtApps.includes("factory_operations_dashboard_app") ||
  !builtApps.includes("factory_case_summary_app")
) {
  throw new Error("Factory Operations native MCP runtime build is stale.");
}

for (const [name, html] of [
  ["dashboard", dashboardAppHtml],
  ["case summary", caseSummaryAppHtml],
]) {
  if (
    !html.includes("ui/initialize") ||
    !html.includes("ui/notifications/tool-result") ||
    !html.includes("ui/notifications/size-changed") ||
    !html.includes("--mcp-app-color-background") ||
    html.includes("XPERT_MCP_API_KEY")
  ) {
    throw new Error(`Built ${name} MCP App is incomplete or unsafe.`);
  }
}

const expectedScreenshots = ["./assets/screenshot-01.jpg"];
const appContribution = bundleJson.targetAppMeta?.[
  "data-xpert"
]?.marketplace?.contents?.find(
...
if (
  !existsSync(resolve(expectedScreenshots[0])) ||
  !builtIndex.includes(expectedScreenshots[0])
) {
  throw new Error(
    "Application screenshot is missing or stale in the runtime build."
  );
}
