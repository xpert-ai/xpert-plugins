...
describe("Factory Operations decorated business tools", () => {
  it("declares all 12 tools exactly once and preserves all 10 Middleware keys", () => {
    ...
    const names = descriptors.flatMap((descriptor) =>
      descriptor.tools.map((tool) => tool.options.name)
    );
    const middlewareProviders = descriptors.flatMap(
      (descriptor) =>
        descriptor.options.middlewares?.map(({ provider }) => provider) ?? []
    );
    ...
    expect(insights.options.provider).toBe(
      FACTORY_INSIGHTS_TOOLSET_PROVIDER_KEY
    );
    expect(insights.options.componentKey).toBe(
      FACTORY_INSIGHTS_TOOLSET_COMPONENT_KEY
    );
    ...
    expect(new Set(middlewareProviders)).toEqual(
      new Set(Object.values(FACTORY_MIDDLEWARE))
    );
  });
  ...
  it("derives two governed MCP providers and empty legacy BuiltinToolset surfaces", async () => {
    ...
    const definitions = toolsets.flatMap(
      (toolset) => toolset.getMcpCapabilityDefinitions()?.tools ?? []
    );
    ...
    expect(toolsets[0].getMcpCapabilityDefinitions()?.apps).toBeUndefined();
    expect(
      toolsets[1].getMcpCapabilityDefinitions()?.apps?.map(({ key }) => key)
    ).toEqual([
      FACTORY_DASHBOARD_MCP_APP_KEY,
      FACTORY_CASE_SUMMARY_MCP_APP_KEY,
    ]);
    for (const definition of definitions) {
      ...
      expect(definition.visibility).toEqual(
        definition.name === FACTORY_TOOL.dashboard ||
          definition.name === FACTORY_TOOL.caseSummary
          ? ["model", "app"]
          : ["model"]
      );
    ...
    expect(
      definitions.find(({ name }) => name === FACTORY_TOOL.dashboard)?.app
    ).toEqual({
      resourceKey: FACTORY_DASHBOARD_MCP_APP_KEY,
    });
    expect(
      definitions.find(({ name }) => name === FACTORY_TOOL.caseSummary)?.app
    ).toEqual({
      resourceKey: FACTORY_CASE_SUMMARY_MCP_APP_KEY,
    });
    expect(definitions.filter(({ app }) => app)).toHaveLength(2);
    expect(
      definitions.filter(({ name }) =>
        FACTORY_MUTATION_TOOL_NAMES.includes(name as never)
      )
    ).toHaveLength(7);
  });

  it("keeps both MCP App frontends protocol-native, localized, and host-themed", async () => {
    const [dashboardSource, summarySource, bridgeSource, styles] =
      await Promise.all([
        readFile(
          new URL("../mcp-apps/dashboard/main.ts", import.meta.url),
          "utf8"
        ),
        readFile(
          new URL("../mcp-apps/case-summary/main.ts", import.meta.url),
          "utf8"
        ),
        readFile(
          new URL("../mcp-apps/shared/bridge.ts", import.meta.url),
          "utf8"
        ),
        readFile(new URL("../mcp-apps/styles.css", import.meta.url), "utf8"),
      ]);

    for (const source of [dashboardSource, summarySource]) {
      expect(source).toContain("MESSAGES");
      expect(source).not.toMatch(/tenantId|organizationId|XPERT_MCP_API_KEY/);
    }
    expect(bridgeSource).toContain("ui/initialize");
    expect(bridgeSource).toContain("ui/notifications/tool-result");
    expect(bridgeSource).toContain("ui/notifications/size-changed");
    expect(styles).toContain("--mcp-app-color-background");
    expect(styles).toContain("--mcp-app-color-primary");
  });
  ...
  it("uses invocation scope, validates strict input, and returns compact MCP structuredContent", async () => {
    ...
    const search = toolset
      ...
      ?.tools?.find(({ name }) => name === FACTORY_TOOL.casesSearch);

    await expect(
      search?.execute(
        { organizationId: "model-controlled" },
        executionContext()
      )
    ).rejects.toThrow();
    ...
    ).rejects.toThrow("require an organization-scoped invocation context");
    ...
    expect(JSON.stringify(result)).not.toMatch(
      /internalPath|analysisFacts|timeline|tenant-runtime/
    );
  ...
  it("derives Agent tools that return JSON text and adds mutation hooks only to Middleware", async () => {
    ...
    const middleware = await coordination.createMiddleware(
      {},
      middlewareContext()
    );
    ...
    const output = JSON.parse(
      String(await search.invoke({ search: "M-07", page: 1, pageSize: 1 }))
    );
    ...
    const mutation = new DecoratedAgentMiddlewareStrategy(
      provider,
      descriptor,
      FACTORY_MIDDLEWARE.triage
    );
    const mutationMiddleware = await mutation.createMiddleware(
      {},
      middlewareContext()
    );
  ...
  it("discovers MCP from runtime decorators without a manifest toolsets declaration", async () => {
    const manifest = JSON.parse(
      await readFile(
        new URL("../../.xpertai-plugin/plugin.json", import.meta.url),
        "utf8"
      )
    );
    const runtimeMcp =
      plugin.meta.targetAppMeta?.xpert?.marketplace?.contents.filter(
        (item) => item.type === "mcp"
      );
    const manifestMcp =
      manifest.targetAppMeta.xpert.marketplace.contents.filter(
        (item: { type?: string }) => item.type === "mcp"
      );

    expect(manifest.toolsets).toBeUndefined();
    expect(plugin.meta.targetAppMeta?.xpert?.capabilities).toContain(
      FACTORY_MCP_CAPABILITY
    );
    expect(plugin.meta.targetAppMeta?.xpert?.capabilities).toContain(
      FACTORY_INSIGHTS_MCP_CAPABILITY
    ...
    expect(runtimeMcp?.map((item) => [item.name, item.metadata])).toEqual([
      [
        FACTORY_TOOLSET_COMPONENT_KEY,
        { protocol: "native", provider: FACTORY_TOOLSET_PROVIDER_KEY },
      ],
      [
        FACTORY_INSIGHTS_TOOLSET_COMPONENT_KEY,
        { protocol: "native", provider: FACTORY_INSIGHTS_TOOLSET_PROVIDER_KEY },
      ],
    ]);
    expect(
      manifestMcp.map((item: { name: string; metadata: object }) => [
        item.name,
        item.metadata,
      ])
    ).toEqual(runtimeMcp?.map((item) => [item.name, item.metadata]));
  });
});
...
function createProvider() {
  ...
  const wrap = vi.fn(
    () => async (request: unknown, handler: (value: unknown) => unknown) =>
      handler(request)
  );
