...
const REQUIRED_CONTEXT = ['tenant', 'organization', 'principal', 'execution'] as const
const READ_BEHAVIOR = {
  risk: 'read',
  sideEffect: 'none',
  idempotency: 'safe'
...
@XpertToolProvider({
  ...
  description: 'Organization-scoped, read-only Factory Case search, progress, dashboard, and execution insights.',
  instructions:
  ...
  middlewares: [
    {
      ...
      meta: middlewareMeta(FACTORY_MIDDLEWARE.coordination, 'Factory case coordination', '工厂事件协调', [
    ...
    },
    {
      ...
      meta: middlewareMeta(FACTORY_MIDDLEWARE.monitoring, 'Factory operations monitoring', '工厂运营监控', [
    ...
    },
    {
      ...
      meta: middlewareMeta(FACTORY_MIDDLEWARE.execution, 'Recovery execution status', '恢复执行状态', [
...
export class FactoryOperationsInsightsTools {
  ...
  async searchCases(input: SearchFactoryCasesInput, context: XpertBusinessToolContext) {
    const result = await this.service.listCases(scopeFromContext(context), input)
    return {
  ...
  getCaseSummary(input: GetFactoryCaseInput, context: XpertBusinessToolContext) {
  ...
  @XpertTool({
    ...
    description: 'Read compact progress for one exact Factory Case without returning all evidence and action detail.',
    inputSchema: getFactoryCaseSchema,
  ...
  async getCaseProgress(input: GetFactoryCaseInput, context: XpertBusinessToolContext) {
    const summary = await this.service.getCaseSummary(scopeFromContext(context), input)
    return {
      caseId: summary.id,
      revision: summary.revision,
      status: summary.status,
  ...
  async getDashboard(_input: Record<string, never>, context: XpertBusinessToolContext) {
    const dashboard = await this.service.getManagementDashboard(scopeFromContext(context))
    return {
  ...
  getExecutionStatus(input: GetFactoryCaseInput, context: XpertBusinessToolContext) {
...
@XpertToolProvider({
  ...
  description: 'Organization-scoped, governed Factory Case recovery mutation capabilities.',
  instructions:
  ...
  middlewares: [
    {
      ...
      meta: middlewareMeta(FACTORY_MIDDLEWARE.triage, 'Factory event triage', '工厂异常研判', [FACTORY_FEATURE.triage])
    },
    {
      provider: FACTORY_MIDDLEWARE.equipment,
      meta: middlewareMeta(FACTORY_MIDDLEWARE.equipment, 'Equipment diagnostics', '设备诊断', [
        FACTORY_FEATURE.equipment
      ])
    },
    {
      provider: FACTORY_MIDDLEWARE.quality,
      meta: middlewareMeta(FACTORY_MIDDLEWARE.quality, 'Quality impact', '质量影响', [FACTORY_FEATURE.quality])
    },
    {
      provider: FACTORY_MIDDLEWARE.production,
      meta: middlewareMeta(FACTORY_MIDDLEWARE.production, 'Production impact', '生产与订单影响', [
        FACTORY_FEATURE.production
      ])
    },
    {
      provider: FACTORY_MIDDLEWARE.resources,
      meta: middlewareMeta(FACTORY_MIDDLEWARE.resources, 'Resource readiness', '备件与维修资源', [
        FACTORY_FEATURE.resources
      ])
    },
    {
      provider: FACTORY_MIDDLEWARE.planning,
      meta: middlewareMeta(FACTORY_MIDDLEWARE.planning, 'Recovery planning', '恢复方案规划', [FACTORY_FEATURE.planning])
    },
    {
      provider: FACTORY_MIDDLEWARE.verification,
      meta: middlewareMeta(FACTORY_MIDDLEWARE.verification, 'Recovery verification', '恢复验证', [
...
export class FactoryOperationsTools implements XpertToolProviderInstance {
  ...
  async recordTriage(input: RecordTriageInput, context: XpertBusinessToolContext) {
    return (await this.service.recordTriage(scopeFromContext(context), input)).receipt
  }
  ...
  async recordEquipment(input: RecordEquipmentFindingInput, context: XpertBusinessToolContext) {
    return (await this.service.recordEquipmentFinding(scopeFromContext(context), input)).receipt
  }
  ...
  async recordQuality(input: RecordQualityFindingInput, context: XpertBusinessToolContext) {
    return (await this.service.recordQualityFinding(scopeFromContext(context), input)).receipt
  }
  ...
  async recordProduction(input: RecordProductionFindingInput, context: XpertBusinessToolContext) {
    return (await this.service.recordProductionFinding(scopeFromContext(context), input)).receipt
  }

  @XpertTool({
    name: FACTORY_TOOL.resources,
    title: 'Record resource readiness',
    description: 'Record one spare-part and maintenance-resource readiness assessment with WMS/CMMS evidence.',
  ...
  async recordResources(input: RecordResourceFindingInput, context: XpertBusinessToolContext) {
    return (await this.service.recordResourceFinding(scopeFromContext(context), input)).receipt
  }
  ...
  async generatePlan(input: GenerateRecoveryPlanInput, context: XpertBusinessToolContext) {
    return (await this.service.generateRecoveryPlan(scopeFromContext(context), input)).receipt
  }
  ...
  async verifyRecovery(input: VerifyRecoveryInput, context: XpertBusinessToolContext) {
    return (await this.service.verifyRecovery(scopeFromContext(context), input)).receipt
  }

  getMiddlewareExtensions(provider: string, _options: unknown, _context: IAgentMiddlewareContext) {
    const toolName = MUTATION_TOOL_BY_MIDDLEWARE.get(provider)
    return toolName ? { wrapToolCall: this.events.wrap([toolName], provider) } : {}
  }
}
...
function middlewareMeta(name: string, en_US: string, zh_Hans: string, features: string[]): TAgentMiddlewareMeta {
  return {
    name,
    label: { en_US, zh_Hans },
    description: {
...
function scopeFromContext(context: XpertBusinessToolContext): FactoryScope {
  if (!context.organizationId) {
    throw new Error('Factory Operations tools require an organization-scoped invocation context.')
  }
  return {
