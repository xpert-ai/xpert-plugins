...
describe('AgriServiceViewProvider', () => {
  const context = {
    hostType: 'agent',
    hostId: 'assistant-1',
    tenantId: 'tenant-1',
  ...
  function createProvider() {
    return new AgriServiceViewProvider({
  ...
  it('registers one Agri Service Workorder workbench manifest for the fixed slot', () => {
    const provider = createProvider()
    const manifests = provider.getViewManifests(context, 'agent.workbench.fixed')

    expect(manifests.map((item) => item.key)).toEqual([AGRI_SERVICE_WORKBENCH_VIEW_KEY])
    expect(manifests[0].title.zh_Hans).toBe('农服工单工作台')
    expect(manifests[0].clientCommands?.map((item) => item.key)).toEqual(['assistant.chat.send_message'])
    expect(manifests[0].hostEvents?.subscriptions?.[0].filter?.toolNames).toContain(AGRI_SERVICE_SAVE_TOOL_NAME)
  ...
  it('routes view actions to service methods', async () => {
    const provider = createProvider()
    const result = await provider.executeViewAction(context, AGRI_SERVICE_WORKBENCH_VIEW_KEY, 'mark_processed', {
      targetId: 'wo-1',
      input: {
  ...
  it('prepares report entry form submissions as assistant messages instead of saving directly', async () => {
    const provider = createProvider()
    const result = await provider.executeViewAction(context, AGRI_SERVICE_WORKBENCH_VIEW_KEY, 'prepare_report_chat_message', {
      input: {
    ...
    expect(result.data).toMatchObject({
      ...
      payload: {
        text: expect.stringContaining('agri_service_save_generated_work_order')
      }
    })
  })
  ...
  it('instructs the agent to infer service scope from natural language instead of requiring manual scope hints', async () => {
    ...
    expect(text).toContain('多个候选同等合理或当前服务范围完全未覆盖时，不要调用 agri_service_save_generated_work_order')
  ...
  it('prepares uploaded service data as an assistant import task without saving directly', async () => {
    ...
    expect(result).toMatchObject({
      ...
      data: {
        commandKey: 'assistant.chat.send_message',
        payload: {
          text: expect.stringContaining('agri_service_import_service_data')
        },
        importDraftId: 'import-draft-1',
        summary: {
          customers: 1,
  ...
  it('routes supplement draft view action to the service', async () => {
    const provider = createProvider()
    const result = await provider.executeViewAction(context, AGRI_SERVICE_WORKBENCH_VIEW_KEY, 'prepare_supplement_draft', {
      targetId: 'wo-1',
      input: {
