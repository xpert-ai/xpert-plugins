...
import type {
  AgriServiceGeneratedWorkOrderInput,
  AgriServiceLogAction,
  AgriServiceProcessingResult,
  AgriServiceSearchInput,
  AgriServiceServiceDataImportDraft,
  AgriServiceServiceDataImportInput,
  AgriServiceServiceDataPayload,
  AgriServiceServiceDataRecord,
  AgriServiceServiceDataSummary,
  AgriServiceScope,
  AgriServiceSimilarWorkOrderSummary,
  AgriServiceSupplementDraftInput,
  AgriServiceUpdateInput,
  AgriServiceWorkOrderStatus
} from './types'

const EDITABLE_STATUSES: AgriServiceWorkOrderStatus[] = ['pending_confirmation', 'needs_supplement']
const SIMILAR_LOOKBACK_DAYS = 7
const DEMO_OPERATOR_ID = 'agri-service-workorder-demo'
...
interface StoredServiceDataImportDraft extends AgriServiceServiceDataImportDraft {
  scopeKey: string
  createdAt: number
}
...
const SERVICE_DATA_SHEET_KEYS: Record<string, keyof AgriServiceServiceDataPayload> = {
  customers: 'customers',
  customer: 'customers',
  '客户': 'customers',
  '客户表': 'customers',
...
type DemoWorkOrder = Partial<AgriServiceWorkOrder> & {
  workOrderNo: string
  status: AgriServiceWorkOrderStatus
  sourceType: NonNullable<AgriServiceWorkOrder['sourceType']>
...
@Injectable()
export class AgriServiceService {
  constructor(
    @InjectRepository(AgriServiceWorkOrder)
    private readonly workOrderRepository: Repository<AgriServiceWorkOrder>,
    @InjectRepository(AgriServiceWorkOrderLog)
    private readonly logRepository: Repository<AgriServiceWorkOrderLog>,
    @InjectRepository(AgriServiceServiceData)
    private readonly serviceDataRepository: Repository<AgriServiceServiceData>,
    private readonly catalogService: AgriServiceMockCatalogService
  ) {}

  async saveGeneratedWorkOrder(input: AgriServiceGeneratedWorkOrderInput, scope: AgriServiceScope) {
    ...
    return this.workOrderRepository.manager.transaction(async (manager) => {
      const workOrderRepository = manager.getRepository(AgriServiceWorkOrder)
      const logRepository = manager.getRepository(AgriServiceWorkOrderLog)
      ...
      await this.writeLogWithRepository(logRepository, scope, workOrder, 'ai_generated', {
        remark: 'AI generated Agri Service Workorder work order.',
  ...
  async searchWorkOrders(scope: AgriServiceScope, input: AgriServiceSearchInput = {}) {
    await this.ensureDemoWorkOrders(scope)
  ...
  async getWorkOrderDetail(scope: AgriServiceScope, workOrderId: string) {
  ...
  async getWorkOrderDetailForAgent(scope: AgriServiceScope, workOrderId: string) {
    const { workOrder, logs } = await this.getWorkOrderDetail(scope, workOrderId)
    return toWorkOrderDetailItem(workOrder, logs)
  }

  async updateWorkOrder(scope: AgriServiceScope, workOrderId: string, input: AgriServiceUpdateInput) {
    const workOrder = await this.getScopedWorkOrder(scope, workOrderId)
    ...
    await this.writeLog(scope, updated, 'field_updated', {
      ...
      remark: 'Human updated Agri Service Workorder work order fields.'
    })
    return updated
  }

  async markNeedsSupplement(scope: AgriServiceScope, workOrderId: string, input: { reason?: string; remark?: string }) {
  ...
  async saveSupplement(scope: AgriServiceScope, workOrderId: string, input: AgriServiceUpdateInput & { remark?: string }) {
    const workOrder = await this.getScopedWorkOrder(scope, workOrderId)
  ...
  async prepareSupplementDraft(scope: AgriServiceScope, workOrderId: string, input: AgriServiceSupplementDraftInput) {
    ...
    await this.writeLog(scope, updated, 'supplement_draft_prepared', {
      remark: supplementContent ?? draft.rationale ?? 'AI prepared Agri Service Workorder supplement draft.',
      snapshot: draft
    })
    return updated
  }

  async confirmProcessing(scope: AgriServiceScope, workOrderId: string, input: AgriServiceUpdateInput) {
    const workOrder = await this.getScopedWorkOrder(scope, workOrderId)
    if (workOrder.status !== 'pending_confirmation') {
  ...
  async markProcessed(
    scope: AgriServiceScope,
    workOrderId: string,
    input: { processingResult?: AgriServiceProcessingResult; processingSummary?: string }
  ) {
  ...
  async rejectAndClose(scope: AgriServiceScope, workOrderId: string, input: { reason?: string }) {
  ...
  private async normalizeGeneratedWorkOrderInput(
    scope: AgriServiceScope,
    input: AgriServiceGeneratedWorkOrderInput
  ): Promise<AgriServiceGeneratedWorkOrderInput> {
  ...
  async getCurrentCatalog(scope: AgriServiceScope) {
    const latest = await this.findLatestServiceData(scope)
  ...
  async importServiceData(scope: AgriServiceScope, input: AgriServiceServiceDataImportInput) {
  ...
  async prepareServiceDataImportDraft(scope: AgriServiceScope, input: {
    fileName?: string
    mimeType?: string
    size?: number
    buffer: Buffer
  }): Promise<AgriServiceServiceDataImportDraft> {
    ...
    const draft: AgriServiceServiceDataImportDraft = {
      importDraftId,
      fileName,
  ...
  private async ensureDemoWorkOrders(scope: AgriServiceScope) {
  ...
  private async findSimilarWorkOrders(
    scope: AgriServiceScope,
    input: AgriServiceGeneratedWorkOrderInput
  ): Promise<AgriServiceSimilarWorkOrderSummary[]> {
  ...
  private async findScopedWorkOrders(scope: AgriServiceScope) {
  ...
  private async getScopedWorkOrder(scope: AgriServiceScope, workOrderId: string) {
    ...
    if (!workOrder) {
      throw new NotFoundException(`Agri Service Workorder work order '${workOrderId}' was not found`)
    }
    return workOrder
  }

  private scopeWhere(scope: AgriServiceScope) {
    return {
      tenantId: scope.tenantId,
      organizationId: scope.organizationId ?? undefined
    }
  ...
  private async findLatestServiceData(scope: AgriServiceScope) {
  ...
  private consumeServiceDataImportDraft(scope: AgriServiceScope, importDraftId: string) {
    this.pruneServiceDataImportDrafts()
  ...
  private assertEditable(workOrder: AgriServiceWorkOrder) {
  ...
  private async writeLogWithRepository(
    repository: Repository<AgriServiceWorkOrderLog>,
    scope: AgriServiceScope,
    workOrder: AgriServiceWorkOrder,
    action: AgriServiceLogAction,
    input: {
...
function normalizeCompletenessTips(input: AgriServiceGeneratedWorkOrderInput) {
...
function buildTitle(input: AgriServiceGeneratedWorkOrderInput) {
...
function resolveServiceDataScopeFields(
  serviceData: AgriServiceServiceDataPayload | undefined,
  input: AgriServiceGeneratedWorkOrderInput
) {
  if (!serviceData) {
    return null
...
function serviceDataDeviceMatches(
  record: AgriServiceServiceDataRecord,
  input: AgriServiceGeneratedWorkOrderInput,
  searchText: string
) {
  const candidates = [
...
function findLinkedLocation(
  ...
  device: AgriServiceServiceDataRecord | undefined
) {
  if (!device) {
    return undefined
  }
...
function findRecordByCode(records: AgriServiceServiceDataRecord[], code: string | undefined) {
...
function findUniqueServiceDataRecord(
  records: AgriServiceServiceDataRecord[],
  predicate: (record: AgriServiceServiceDataRecord) => boolean
) {
  const matches = records.filter(predicate)
  return matches.length === 1 ? matches[0] : undefined
}

function recordDisplayName(record: AgriServiceServiceDataRecord | undefined) {
  if (!record) {
    return undefined
  }
...
function pickUpdateFields(input: AgriServiceUpdateInput) {
...
function buildSupplementDraft(input: AgriServiceSupplementDraftInput) {
  const merged = {
    ...(input.draft ?? {}),
    ...input
  }
...
function parseServiceDataFile(fileName: string, mimeType: string | undefined, buffer: Buffer): AgriServiceServiceDataPayload {
  const lowerName = fileName.toLowerCase()
  const lowerType = (mimeType || '').toLowerCase()
  if (lowerName.endsWith('.json') || lowerType.includes('json')) {
    return JSON.parse(buffer.toString('utf8')) as AgriServiceServiceDataPayload
  }
...
function parseWorkbook(buffer: Buffer): AgriServiceServiceDataPayload {
  const workbook = XLSX.read(buffer, { type: 'buffer' })
  const payload: AgriServiceServiceDataPayload = {}
  for (const sheetName of workbook.SheetNames) {
    const key = resolveServiceDataKey(sheetName)
    if (!key) {
      continue
    }
    payload[key] = XLSX.utils.sheet_to_json(workbook.Sheets[sheetName], { defval: '' }) as AgriServiceServiceDataRecord[]
  }
  return payload
}

function parseCsv(content: string): AgriServiceServiceDataRecord[] {
  const rows = content
    .split(/\r?\n/)
    .map((row) => row.trim())
    .filter(Boolean)
...
function serviceDataDraftScopeKey(scope: AgriServiceScope) {
  return [scope.tenantId, scope.organizationId ?? ''].join(':')
}

function normalizeServiceDataPayload(input: AgriServiceServiceDataPayload | null | undefined): AgriServiceServiceDataPayload {
  const source = input && typeof input === 'object' ? input : {}
  return Object.fromEntries(
    SERVICE_DATA_KEYS.map((key) => [key, normalizeRecordArray(source[key])]).filter(([, value]) => (value as unknown[]).length)
  ) as AgriServiceServiceDataPayload
}

function normalizeRecordArray(value: unknown): AgriServiceServiceDataRecord[] {
...
function buildServiceDataSummary(serviceData: AgriServiceServiceDataPayload): AgriServiceServiceDataSummary {
  return {
...
function hasAnyServiceData(summary: AgriServiceServiceDataSummary) {
  return Object.values(summary).some((value) => value > 0)
}

function serviceDataToCatalog(serviceData: AgriServiceServiceDataPayload) {
  const fallback = {
    deviceTypes: [],
    devices: [],
    faultCategories: [],
...
function normalizeCatalogOptions(records: AgriServiceServiceDataRecord[] | undefined) {
...
function resolveServiceDataKey(sheetName: string): keyof AgriServiceServiceDataPayload | null {
...
function assertConfirmProcessingRequiredFields(workOrder: AgriServiceWorkOrder) {
  const missing = [
...
function toWorkOrderSnapshot(workOrder: AgriServiceWorkOrder) {
  return {
    id: workOrder.id,
...
function toSimilarSummary(workOrder: AgriServiceWorkOrder): AgriServiceSimilarWorkOrderSummary {
  return {
...
function buildStats(items: AgriServiceWorkOrder[]) {
...
function toWorkOrderListItem(workOrder: AgriServiceWorkOrder) {
  return {
    id: workOrder.id,
    workOrderNo: workOrder.workOrderNo,
    title: workOrder.title,
...
function toWorkOrderDetailItem(workOrder: AgriServiceWorkOrder, logs: AgriServiceWorkOrderLog[]) {
  return {
    ...workOrder,
    logs
  }
