...
const agri_service_TOOL_NAMES = [
  'agri_service_save_generated_work_order',
  'agri_service_import_service_data',
...
describe('AgriServiceMiddleware', () => {
  function createMiddleware(service: Record<string, jest.Mock>) {
    const middleware = new AgriServiceMiddleware(service as never)
  ...
  it('exposes the Agri Service Workorder business tools', async () => {
    const service = {
    ...
    expect(result.tools.map((item) => item.name)).toEqual(agri_service_TOOL_NAMES)
  })
  ...
  it('invokes search, detail, catalog and supplement draft tools with scoped service methods', async () => {
    ...
    await tools.get('agri_service_get_catalog')?.invoke({})
    await tools.get('agri_service_search_work_orders')?.invoke({ status: 'needs_supplement', search: '空调' })
    await tools.get('agri_service_get_work_order_detail')?.invoke({ workOrderId: 'wo-1' })
    await tools
      .get('agri_service_prepare_supplement_draft')
  ...
  it('imports service data only through the agent middleware tool', async () => {
    ...
    const response = await tools.get('agri_service_import_service_data')?.invoke({
  ...
  it('returns work order status, scope and supplement tips for AI feedback after saving', async () => {
    ...
    const response = await tools.get('agri_service_save_generated_work_order')?.invoke({
