...
describe('Agri Service Workorder remote component', () => {
  function readAppScript() {
    return readFileSync(join(__dirname, 'remote-components', 'agri-service-workorder', 'app.js'), 'utf8')
  }

  function loadAppTestHooks() {
    const script = readAppScript().replace(
      "  const rootElement = document.getElementById('root')",
      "  globalThis.__AgriServiceTestHooks = { isNewServiceData, isNewWorkOrder, isAfterBaseline, buildImportFeedback }\n  const rootElement = document.getElementById('root')"
    )
    const sandbox = {
    ...
    return sandbox.globalThis.__AgriServiceTestHooks as {
  ...
  it('renders the redesigned workbench with the import and service range entry points', () => {
    ...
    expect(script).toContain('农服工单工作台')
  ...
  it('uploads service data through a file action and sends an assistant import task', () => {
    ...
    expect(script).toContain('agri_service_import_service_data')
