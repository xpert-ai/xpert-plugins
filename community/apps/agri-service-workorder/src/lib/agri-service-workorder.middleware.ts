...
import type { AgriServiceGeneratedWorkOrderInput, AgriServiceScope } from './types'
...
const importServiceDataSchema = z.object({
  ...
  serviceData: serviceDataPayloadSchema.optional().describe('Agent-approved structured service data to persist as the current agri-service-workorder catalog. Use only when no importDraftId is available.')
})
...
const workOrderDetailSchema = z.object({
  workOrderId: z.string().min(1).describe('Agri Service Workorder work order id.')
})
...
@Injectable()
@AgentMiddlewareStrategy(AGRI_SERVICE_MIDDLEWARE_NAME)
export class AgriServiceMiddleware implements IAgentMiddlewareStrategy<Record<string, never>> {
  meta: TAgentMiddlewareMeta = {
    name: AGRI_SERVICE_MIDDLEWARE_NAME,
    label: {
      en_US: 'Agri Service Workorder',
      zh_Hans: '农服工单'
    },
    description: {
      en_US: 'Save AI-generated Agri Service Workorder work orders for human review.',
      zh_Hans: '保存 AI 生成的农服工单，供人工审核处理。'
    },
    icon: {
      type: 'svg',
  ...
  constructor(private readonly service: AgriServiceService) {}
  ...
  createMiddleware(_options: Record<string, never>, context: IAgentMiddlewareContext): PromiseOrValue<AgentMiddleware> {
    ...
    const saveGeneratedWorkOrderTool = tool(
      async (input: z.infer<typeof saveGeneratedWorkOrderSchema>) => {
        const payload: AgriServiceGeneratedWorkOrderInput = {
          ...input,
        ...
        return JSON.stringify({
          success: true,
          message: 'Agri Service Workorder work order was generated for human review.',
          data: {
            id: workOrder.id,
      ...
      {
        name: AGRI_SERVICE_SAVE_TOOL_NAME,
        description:
          'Save one AI-generated Agri Service Workorder work order after extracting structured fields from a natural-language repair report. Use exactly once per report. Do not split multiple issues into multiple work orders; set hasMultipleIssues and multipleIssueTip instead.',
    ...
    const getCatalogTool = tool(
      async (_input: z.infer<typeof getCatalogSchema>) => {
        ...
        return JSON.stringify({
          success: true,
          message: 'Agri Service Workorder catalog candidates were returned.',
          data: catalog
        })
      },
      {
        name: AGRI_SERVICE_GET_CATALOG_TOOL_NAME,
        description:
          'Get Agri Service Workorder candidate values for normalizing device types, devices, fault categories, locations, departments, roles, parts, service types and urgency.',
    ...
    const importServiceDataTool = tool(
      async (input: z.infer<typeof importServiceDataSchema>) => {
        ...
        return JSON.stringify({
          success: true,
          message: 'Agri Service Workorder service data was imported by the Agent.',
          data: {
            id: row.id,
      ...
      {
        name: AGRI_SERVICE_IMPORT_SERVICE_DATA_TOOL_NAME,
        description:
          'Persist Agent-approved Agri Service Workorder service data after parsing an uploaded JSON, CSV or Excel file. The workbench upload action only prepares data; this tool is the required write path for customers, projects, locations, devices, departments, roles, personnel, parts and similar cases.',
    ...
    const searchWorkOrdersTool = tool(
      async (input: z.infer<typeof searchWorkOrdersSchema>) => {
        ...
        return JSON.stringify({
          success: true,
          message: 'Agri Service Workorder work orders were searched.',
          data: result
        })
      },
      {
        name: AGRI_SERVICE_SEARCH_TOOL_NAME,
        description:
          'Search Agri Service Workorder work orders by status, keyword, device type and urgency. Use this when the user asks about existing maintenance work orders or history.',
    ...
    const getWorkOrderDetailTool = tool(
      async (input: z.infer<typeof workOrderDetailSchema>) => {
        ...
        return JSON.stringify({
          success: true,
          message: 'Agri Service Workorder work order detail was returned.',
          data: detail
        })
      },
      {
        name: AGRI_SERVICE_DETAIL_TOOL_NAME,
        description:
          'Get one Agri Service Workorder work order detail including AI extraction, human confirmation fields, processing fields, supplement draft and operation logs.',
    ...
    const prepareSupplementDraftTool = tool(
      async (input: z.infer<typeof supplementDraftSchema>) => {
        ...
        return JSON.stringify({
          success: true,
          message:
            'Agri Service Workorder supplement draft was saved. Ask the user to open the review desk, one-click fill the draft, and save manually.',
          data: {
            id: workOrder.id,
            workOrderNo: workOrder.workOrderNo,
            status: workOrder.status,
      ...
      {
        name: AGRI_SERVICE_SUPPLEMENT_DRAFT_TOOL_NAME,
        description:
