...
import {
  ...
  AGRI_SERVICE_MIDDLEWARE_TOOL_NAMES,
  AGRI_SERVICE_PLUGIN_NAME,
  AGRI_SERVICE_PROVIDER_KEY,
  AGRI_SERVICE_REMOTE_ENTRY_KEY,
  AGRI_SERVICE_REPORT_VIEW_KEY,
  AGRI_SERVICE_REVIEW_VIEW_KEY,
...
@Injectable()
@ViewExtensionProvider(AGRI_SERVICE_PROVIDER_KEY)
export class AgriServiceViewProvider implements IXpertViewExtensionProvider {
  ...
  getViewManifests(_context: XpertResolvedViewHostContext, slot: string): XpertExtensionViewManifest[] {
    ...
    return [
      {
        key: AGRI_SERVICE_WORKBENCH_VIEW_KEY,
        title: text('Agri Service Workorder Workbench', '农服工单工作台'),
        description: text(
          'Prepare agri catalog candidates, submit AI-assisted farm service requests, review work orders and close the lightweight processing loop.',
          '准备农服候选数据、提交 AI 填报、审核工单并完成轻量处理闭环。'
        ),
        icon: {
          type: 'font',
        ...
        ...(fixed
          ? {
              workbench: {
                ...
                menu: {
                  enabled: true,
                  label: text('Agri Service Workorder', '农服工单'),
                  order: 20,
                  icon: {
                    type: 'font',
                    value: 'ri-customer-service-2-line',
          ...
          : {}),
        source: {
          provider: AGRI_SERVICE_PROVIDER_KEY,
          plugin: AGRI_SERVICE_PLUGIN_NAME
        },
        view: remoteView(),
  ...
  async getRemoteComponentEntry(
    ...
    if (
      component.entry !== AGRI_SERVICE_REMOTE_ENTRY_KEY ||
      !isAgriServiceViewKey(viewKey)
    ) {
      return {
        html: '<!doctype html><html><body>Unsupported Agri Service Workorder component.</body></html>',
        contentType: 'text/html; charset=utf-8'
      }
    }
    const appPath = join(__dirname, 'remote-components', AGRI_SERVICE_REMOTE_ENTRY_KEY, 'app.js')
    ...
    return {
      html: renderRemoteReactIframeHtml({
        title: 'Agri Service Workorder Workbench',
        lang: 'zh-Hans',
        reactUmd,
        reactDomUmd,
        appScript
  ...
  async getViewData(
    ...
    if (!isAgriServiceViewKey(viewKey)) {
      return {}
    }
    return this.service.getViewData(scopeFromContext(context), {
      viewMode: 'review',
      workOrderId: getStringParameter(query.parameters, 'workOrderId'),
      status: getStringParameter(query.parameters, 'status') as AgriServiceWorkOrderStatus | undefined,
  ...
  async executeViewAction(
    ...
    try {
      ...
      if (actionKey === 'prepare_report_chat_message') {
        ...
        if (!originalContent) {
          return failure('Report content is required', '农服需求不能为空')
        }
        const payload = {
        ...
        return {
          success: true,
          message: text('Report message prepared', '农服填报消息已准备'),
          refresh: false,
          data: {
      ...
      if (actionKey === 'refresh') {
        return success('Agri Service Workorder view refreshed', '农服工单视图已刷新')
      }
      if (actionKey === 'update_work_order') {
        ...
        await this.service.markProcessed(scope, workOrderId as string, {
          processingResult: getStringInput(request.input, 'processingResult') as AgriServiceProcessingResult | undefined,
...
function remoteView(): XpertRemoteComponentViewSchema {
  return {
    ...
    component: {
      ...
      entry: AGRI_SERVICE_REMOTE_ENTRY_KEY
    },
    dataSource: {
...
function toolCompletedHostEvents() {
  return {
    subscriptions: [
      {
        key: 'agri-service-workorder-tool-completed',
        event: 'assistant.tool.completed',
        filter: {
          sources: ['chatkit'],
          toolNames: [...AGRI_SERVICE_MIDDLEWARE_TOOL_NAMES]
        },
        action: {
...
function isAgriServiceViewKey(viewKey: string) {
  return (
    viewKey === AGRI_SERVICE_WORKBENCH_VIEW_KEY ||
    viewKey === AGRI_SERVICE_REPORT_VIEW_KEY ||
    viewKey === AGRI_SERVICE_REVIEW_VIEW_KEY
  )
}

function scopeFromContext(context: XpertResolvedViewHostContext): AgriServiceScope {
  return {
...
function buildAssistantMessage(input: Record<string, unknown> | null | undefined) {
  ...
  return [
    '请根据以下自然语言报修内容创建农服工单工单。',
    '你必须先调用 agri_service_get_catalog 获取当前服务范围与候选数据，再识别字段；不要只根据用户手填线索判断。',
    '客户、项目、场所不是用户必填项；它们只是辅助线索，缺失时不能直接要求用户手动选择。',
    '根据自然语言中的客户、项目、场所、楼栋、房间、设备名称、设备编号、故障代码与候选数据自动匹配，优先选择最可信的服务范围、设备和故障类别。',
    '如果能够匹配到唯一或明显最优候选，请调用 agri_service_save_generated_work_order 保存一张待确认农服工单工单。',
    '多个候选同等合理或当前服务范围完全未覆盖时，不要调用 agri_service_save_generated_work_order；请直接向用户说明候选冲突或未覆盖的范围，并提示只需补充最小必要信息。',
...
function buildServiceDataImportAssistantMessage(draft: {
  ...
  return [
    '请解析并导入以下农服工单服务候选数据。',
    '你必须调用 agri_service_import_service_data 工具落库；不要只总结文件内容，也不要声称已导入但未调用工具。',
    ...
    '',
    `工具调用要求：调用 agri_service_import_service_data，参数使用 {"importDraftId":"${draft.importDraftId}","fileName":"${draft.fileName}","importMode":"${draft.importMode}"}。`
  ].join('\n')
}
...
function normalizeReportInput(input: Record<string, unknown> | null | undefined) {
  return {
    ...
    urgency: getStringInput(input, 'urgency') as AgriServiceUrgency | undefined,
    serviceType: getStringInput(input, 'serviceType') as AgriServiceServiceType | undefined,
