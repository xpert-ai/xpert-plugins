;(function () {
  ...
  const IMPORT_TOOL_NAME = 'agri_service_import_service_data'
  ...
  const PROCESSING_RESULT_LABELS = {
    fixed: '已完成服务',
    temporarily_restored: '临时处置',
    unable_to_process: '无法上门'
  }
  ...
  const SAMPLE_REPORT = {
    ...
    siteName: '东河村三组北地',
    reporterName: '李农技',
    reporterDepartment: '乡镇农服站',
    reporterContact: '138****5678',
    deviceType: '水稻',
    deviceName: '东河村三组北地',
    originalContent: '东河村三组王大叔家水稻大约 12 亩，最近叶片发黄有褐斑，怀疑稻瘟病，想尽快安排植保喷施，明天有雨，请尽快联系。',
    title: '水稻疑似稻瘟病植保喷施',
    faultCategory: '稻瘟病',
    faultPhenomenon: '叶片发黄、褐斑',
    location: '东河村三组北地',
    impactScope: '约12亩',
    urgency: 'high',
    serviceType: 'repair',
    needOnsite: 'true',
    aiDiagnosis: '根据叶片褐斑与发黄描述，初步怀疑稻瘟病，需现场复核后确定用药方案。',
    suggestedAction: '建议安排植保员上门查看病斑分布，确认生育期后制定喷施方案，避开明日降雨窗口。',
    recommendedDepartment: '植保服务组',
    recommendedRole: '植保员',
    recommendedDispatchAdvice: '优先安排熟悉水稻病害的植保员，携带常用杀菌剂备选清单。',
    suggestedParts: '三环唑、稻瘟灵',
    completenessTips: '缺少准确亩数与发生起始时间，建议人工补充',
    processingRemark: '农户希望尽快上门，注意明日降雨。'
  }
  ...
  window.addEventListener('message', (event) => {
    ...
    if (message.type === 'init') {
      ...
      window.__AgriServiceSetContext &&
        window.__AgriServiceSetContext({
    ...
    if (message.type === 'hostEvent') {
      window.__AgriServiceHandleHostEvent && window.__AgriServiceHandleHostEvent(message.event)
      return
    }
  ...
  function App() {
    ...
    React.useEffect(() => {
      window.__AgriServiceSetContext = setContext
      window.__AgriServiceHandleHostEvent = () => {
        window.__AgriServiceReload && window.__AgriServiceReload()
      }
      post('ready')
      return () => {
        delete window.__AgriServiceSetContext
        delete window.__AgriServiceHandleHostEvent
        delete window.__AgriServiceReload
      }
    }, [])
    ...
    if (!context) {
      return h('main', { className: 'sm-shell' }, h('div', { className: 'sm-empty' }, '正在初始化农服工单工作台...'))
    }
  ...
  function Workbench({ context }) {
    ...
    React.useEffect(() => {
      window.__AgriServiceReload = () => loadData({ feedbackFromTool: true })
      loadData()
      return () => {
        ...
        try {
          ...
          if (taskType === 'import') {
            ...
            if (isNewServiceData(latestServiceData, base.latestServiceDataId, base.startedAt)) {
              ...
              setNotice('服务数据候选范围已刷新；后续填报会优先从服务数据中自动匹配。')
              aiTaskRef.current = ''
              agentTaskBaselineRef.current = {}
              return
            }
        ...
        if (attempts >= AGENT_RESULT_POLL_MAX_ATTEMPTS) {
          setAiFeedback({
            ...
            detail: taskType === 'import'
              ...
              : '已发送填报任务，但暂未读取到新工单；请查看对话中的 Agent 是否需要补充信息，或是否因服务范围未覆盖/候选冲突而未建单。'
          })
          aiTaskRef.current = ''
          agentTaskBaselineRef.current = {}
          return
    ...
    async function submitReport() {
      ...
      setAiFeedback({
        tone: 'working',
        title: 'AI 正在识别农服需求',
        detail: '已把农服需求描述内容发送给 Agent，Agent 会先读取服务范围，再决定生成待确认/待补充工单或提示补充。'
      })
    ...
    return h('main', { className: 'sm-shell' }, [
      h('header', { className: 'sm-system-header', key: 'header' }, [
        h('div', { className: 'sm-brand', key: 'brand' }, [
          ...
          h('div', { key: 'text' }, [
            h('strong', { key: 'title' }, '农服工单工作台'),
            h('span', { key: 'subtitle' }, 'AI 农服填报、工单审核与派单建议')
          ])
        ]),
    ...
    return h('section', { className: 'sm-home' }, [
      h('section', { className: 'sm-card sm-import-card', key: 'import' }, [
        h('div', { className: 'sm-card-title', key: 'title' }, [
          h('div', { key: 'heading' }, [
            h('h1', { key: 'h1' }, '导入服务数据'),
            h('p', { key: 'p' }, '导入农服候选数据、作物、村组、病虫害与人员候选，为农服工单提供智能支持。')
          ]),
        ...
        h('div', { className: 'sm-kpi-grid', key: 'kpis' }, [
          metricCard('农户 / 服务项目 / 地块', `${props.catalogSummary.customers.length} / ${props.catalogSummary.projects.length} / ${props.catalogSummary.locations.length}`, '待审核 18 项'),
          metricCard('可服务作物', props.catalogSummary.devices.length, `作物 ${props.catalogSummary.deviceTypes.length} 类`),
      ...
      h('div', { className: 'sm-home-panels', key: 'panels' }, [
        h('section', { className: 'sm-card sm-range-card', key: 'range' }, [
          ...
          h('div', { className: 'sm-scope-preview', key: 'preview' }, [
            scopePreviewLine('农户', props.catalogSummary.customers),
      ...
      h('section', { className: 'sm-card sm-create-card', key: 'create' }, [
        h('div', { className: 'sm-card-title', key: 'title' }, [
          h('div', { key: 'heading' }, [
            h('h1', { key: 'h1' }, '智能筛选与创建'),
            h('span', { key: 'span' }, '农服需求描述 + 可选线索')
          ]),
          h('button', { className: 'sm-btn sm-btn-plain', key: 'sample', disabled: props.busy, onClick: props.onFillSample }, '填入示例')
        ]),
        textareaField('农服需求描述', props.form.originalContent, (value) => props.onUpdateForm('originalContent', value), {
          rows: 4,
          maxLength: 500,
          required: true,
        ...
        h('div', { className: 'sm-form-grid sm-form-grid-4', key: 'scope-fields' }, [
          ...
          selectField('作物', props.form.deviceType, props.catalog.deviceTypes || [], (value) => props.onUpdateForm('deviceType', value), { valueKey: 'label' }),
          selectField('地块名称', props.form.deviceName, props.catalog.devices || [], (value) => props.onUpdateForm('deviceName', value), { valueKey: 'label' }),
          textField('登记人', props.form.reporterName, (value) => props.onUpdateForm('reporterName', value)),
      ...
      h('section', { className: 'sm-card sm-list-card', key: 'list' }, [
        ...
        h('div', { className: 'sm-filter-grid', key: 'filters' }, [
          ...
          selectField('作物', props.filters.deviceType, props.catalog.deviceTypes || [], (value) => props.onUpdateFilter('deviceType', value), { emptyLabel: '全部', valueKey: 'label' }),
  ...
  function ServiceScopeScreen({ catalog, catalogSummary, workOrders, onBack }) {
    ...
    return h('section', { className: 'sm-scope-screen' }, [
      ...
      h('div', { className: 'sm-scope-kpis', key: 'kpis' }, [
        metricCard('农户', catalogSummary.customers.length, '共维护农户'),
        metricCard('项目', catalogSummary.projects.length, '项目数据'),
        metricCard('场所', catalogSummary.locations.length, '服务点位'),
        metricCard('设备', catalogSummary.devices.length, '候选地块/作物')
      ]),
      h('div', { className: 'sm-scope-layout', key: 'layout' }, [
        h('section', { className: 'sm-card sm-scope-table-card', key: 'customers' }, [
          ...
          h('table', { className: 'sm-table sm-compact-table', key: 'table' }, [
            h('thead', { key: 'thead' }, h('tr', null, [h('th', { key: 'customer' }, '农户姓名'), h('th', { key: 'project' }, '项目数量'), h('th', { key: 'site' }, '场所数量')])),
      ...
      h('section', { className: 'sm-card sm-catalog-card', key: 'catalog' }, [
        ...
        h('div', { className: 'sm-catalog-grid', key: 'grid' }, [
          catalogBlock('服务作物', catalogSummary.deviceTypes),
          catalogBlock('常见病虫害', (catalog.faultCategories || []).map((item) => item.label)),
  ...
  function DetailScreen(props) {
    ...
    return h('section', { className: 'sm-detail-screen' }, [
      ...
      h('div', { className: 'sm-detail-grid', key: 'grid' }, [
        ...
        detailCard('原始需求内容', h('p', { className: 'sm-original-text' }, order.originalContent || '-'), 'original'),
        detailCard('AI 识别结果', renderAiResult(order), 'ai'),
        detailCard('多问题 / 相似工单提示', renderIssueHints(order), 'hints'),
  ...
  function renderBasicInfo(order) {
    return basicRows([
      ['农户 / 服务项目 / 地块', [order.customerName, order.projectName, order.siteName].filter(Boolean).join(' / ')],
      ['地块名称', order.deviceName || order.deviceType],
  ...
  function renderDispatchAdvice(order, props) {
    return h('div', null, [
      ...
      h('p', { className: 'sm-muted-line', key: 'reason' }, order.recommendedDispatchAdvice || '根据作物、故障分类和候选资源生成推荐。'),
  ...
  function buildImportFeedback(catalog, serviceData) {
    ...
    return {
      tone: 'success',
      title: '服务数据已进入候选范围',
      detail: `已更新 ${summary.customers.length} 个客户、${summary.projects.length} 个项目、${summary.locations.length} 个场所、${summary.devices.length} 台设备${importedAt}，后续填报会优先从这些数据中自动匹配。`
    }
  }
