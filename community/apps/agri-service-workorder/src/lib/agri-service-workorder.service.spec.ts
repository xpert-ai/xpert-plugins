...
import type { AgriServiceScope } from './types'
      ...
      transaction: jest.fn(async (callback: (manager: { getRepository: (entity: unknown) => typeof repository }) => unknown) =>
        callback({
          getRepository: (entity: unknown) =>
            entity === AgriServiceWorkOrderLog ? (logRepositoryRef.current as typeof repository) : repository
        })
      )
    },
...
const logRepositoryRef: { current?: ReturnType<typeof createRepository<AgriServiceWorkOrderLog>> } = {}
const serviceDataRepositoryRef: { current?: ReturnType<typeof createRepository<AgriServiceServiceData>> } = {}

describe('AgriServiceService', () => {
  const scope: AgriServiceScope = {
    tenantId: 'tenant-1',
  ...
  let workOrderRepository: ReturnType<typeof createRepository<AgriServiceWorkOrder>>
  let logRepository: ReturnType<typeof createRepository<AgriServiceWorkOrderLog>>
  let serviceDataRepository: ReturnType<typeof createRepository<AgriServiceServiceData>>
  let service: AgriServiceService

  beforeEach(() => {
    workOrderRepository = createRepository<AgriServiceWorkOrder>()
    logRepository = createRepository<AgriServiceWorkOrderLog>()
    serviceDataRepository = createRepository<AgriServiceServiceData>()
    logRepositoryRef.current = logRepository
    serviceDataRepositoryRef.current = serviceDataRepository
    service = new AgriServiceService(
      workOrderRepository as never,
      logRepository as never,
      serviceDataRepository as never,
      new AgriServiceMockCatalogService()
    )
  })
