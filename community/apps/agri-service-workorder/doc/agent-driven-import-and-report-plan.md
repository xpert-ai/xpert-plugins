...
**Goal:** Make Agri Service Workorder report creation and service-data upload run through the Agent and middleware tools, with plugin persistence only happening inside those tools.
...
**Scope:** Only files under `packages/plugins/agri-service-workorder` are changed.
...
- [ ] Add `agri_service_import_service_data` middleware tool.
