...
import {
  ...
  SUPPLIER_DUE_DILIGENCE_REMOTE_ENTRY_KEY,
...
@Injectable()
@ViewExtensionProvider(SUPPLIER_DUE_DILIGENCE_PROVIDER_KEY)
export class SupplierDueDiligenceViewProvider implements IXpertViewExtensionProvider {
    ...
    return [
      {
        ...
        refreshable: true,
        activation: {
          requiredFeatures: [SUPPLIER_DUE_DILIGENCE_FEATURE]
        },
        ...(isAgentFixedWorkbench
          ? {
              workbench: {
                ...
                menu: {
                  enabled: true,
                  label: text('Supplier Due Diligence', '供应商尽调助手'),
                  order: 30,
                  icon: {
                    type: 'svg',
                    value: PROCUREMENT_ICON,
                    alt: 'Supplier Due Diligence'
                  }
                }
              }
            }
          : {}),
        source: {
          provider: SUPPLIER_DUE_DILIGENCE_PROVIDER_KEY,
          plugin: SUPPLIER_DUE_DILIGENCE_PLUGIN_NAME
        },
        view: {
          ...
          component: {
            isolation: 'iframe',
            entry: SUPPLIER_DUE_DILIGENCE_REMOTE_ENTRY_KEY
          },
          dataSource: {
            mode: 'platform'
          }
  ...
  async getRemoteComponentEntry(
    ...
    if (viewKey !== SUPPLIER_DUE_DILIGENCE_VIEW_KEY || component.entry !== SUPPLIER_DUE_DILIGENCE_REMOTE_ENTRY_KEY) {
      return {
    ...
    const appScript = await readFile(
      join(__dirname, 'remote-components', SUPPLIER_DUE_DILIGENCE_REMOTE_ENTRY_KEY, 'app.js'),
      'utf8'
    )
  ...
  async getViewData(
    ...
    if (viewKey !== SUPPLIER_DUE_DILIGENCE_VIEW_KEY) {
      return {}
    }
  ...
  async executeViewAction(
    ...
    try {
      ...
      if (actionKey === 'refresh') {
        return success('Procurement view refreshed', '供应商尽调视图已刷新')
      }
