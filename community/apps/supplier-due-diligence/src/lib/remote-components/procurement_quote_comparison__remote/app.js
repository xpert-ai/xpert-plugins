;(function () {
    ...
    return h(
      ...
      h(
        ...
        h(
          ...
          h('h2', null, isDetail && data && data.item && data.item.case ? displayText(data.item.case.title) || '采购项目' : '供应商尽调助手'),
  ...
  function renderDetail(detail, xpertId, setXpertId, uploadFile, runAction, busy) {
    ...
    const webEvidence = detail.webEvidence || []
    ...
    return h(
      ...
      h(
        ...
        h(
          ...
          metric('网页证据', webEvidence.length),
      ...
      configuredXpertId
        ? null
        : h('div', { className: 'xui-notice' }, '当前项目还没有绑定供应商尽调 Xpert。请在数字专家工作台中打开，或填写 Xpert ID。'),
      h(
        'section',
        ...
        h(
          ...
          h(
            ...
            panelHeader('公开网页证据', '由网页搜索和抓取工具保存；点击链接可复核原始来源'),
            webEvidence.length
              ? h('ul', { className: 'proc-risk-list' }, webEvidence.map((evidence) => h('li', { key: evidence.id || evidence.sourceUrl }, h('strong', null, displayText(evidence.title)), h('p', null, displayText(evidence.excerpt)), h('a', { href: evidence.sourceUrl, target: '_blank', rel: 'noreferrer' }, displayText(evidence.sourceUrl)))))
              : h('div', { className: 'proc-empty' }, '尚无网页证据。请在 Assistant 对话中发起供应商公开资料核验。')
          ),
          h(
            'section',
            { className: 'proc-panel proc-summary-panel' },
