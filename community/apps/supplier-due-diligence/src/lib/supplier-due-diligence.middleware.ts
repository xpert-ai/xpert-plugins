...
const saveWebEvidenceSchema = z.object({
  caseId: z.string().min(1), supplierName: z.string().min(1).max(200).optional(),
  kind: z.enum(['official_site', 'certification', 'service_terms', 'news', 'risk_signal']),
  sourceUrl: z.string().url().max(2048), title: z.string().min(1).max(500),
  excerpt: z.string().min(1).max(4000), confidence: z.number().min(0).max(1).optional()
}).strict()

@Injectable()
@AgentMiddlewareStrategy(SUPPLIER_DUE_DILIGENCE_MIDDLEWARE_NAME)
export class SupplierDueDiligenceMiddleware implements IAgentMiddlewareStrategy<Record<string, never>> {
  readonly meta: TAgentMiddlewareMeta = {
    name: SUPPLIER_DUE_DILIGENCE_MIDDLEWARE_NAME,
    label: {
      en_US: 'Supplier Due Diligence',
      zh_Hans: '供应商尽调助手'
    },
    icon: {
      type: 'svg',
    ...
    description: {
      ...
      zh_Hans: '为供应商尽调工作台保存需求解析、供应商报价、商品匹配、风险项和推荐结论。'
    },
    features: [SUPPLIER_DUE_DILIGENCE_FEATURE],
    configSchema: {
      type: 'object',
      properties: {}
    }
  ...
  constructor(private readonly service: SupplierDueDiligenceService) {}
  ...
  createMiddleware(_options: Record<string, never>, context: IAgentMiddlewareContext): PromiseOrValue<AgentMiddleware> {
    ...
    return {
      ...
      tools: [
        tool(
          async (input) => JSON.stringify(await this.service.saveSupplierWebEvidence(scope, input)),
          { name: 'supplier_save_web_evidence', description: 'Save one verified public-web evidence item after web_search or web_scrape. Never save an invented source.', schema: saveWebEvidenceSchema, verboseParsingErrors: true }
        ),
        tool(
