...
describe('SupplierDueDiligenceViewProvider', () => {
  const createService = () => ({
  ...
  it('registers a remote workbench view on project detail sections', () => {
    const provider = new SupplierDueDiligenceViewProvider(createService())
    ...
    expect(manifests[0].key).toBe(SUPPLIER_DUE_DILIGENCE_VIEW_KEY)
    expect(manifests[0].key).not.toContain('__')
    expect(manifests[0].source.provider).toBe(SUPPLIER_DUE_DILIGENCE_PROVIDER_KEY)
    expect(manifests[0].activation?.requiredFeatures).toEqual([SUPPLIER_DUE_DILIGENCE_FEATURE])
  ...
  it('uses the current agent host as default procurement Xpert for new cases', async () => {
    ...
    const provider = new SupplierDueDiligenceViewProvider(service)
