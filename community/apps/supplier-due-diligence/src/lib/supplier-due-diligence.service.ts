...
@Injectable()
export class SupplierDueDiligenceService {
  constructor(
    ...
    @InjectRepository(SupplierWebEvidence)
    private readonly webEvidenceRepository: Repository<SupplierWebEvidence>
  ) {}
  ...
  async saveSupplierWebEvidence(scope: ProcurementScope, input: SaveSupplierWebEvidenceInput) {
    await this.requireCase(scope, input.caseId)
    const sourceUrl = normalizeRequired(input.sourceUrl, 'Evidence URL is required.')
    if (!/^https?:\/\//i.test(sourceUrl)) throw new BadRequestException('Evidence URL must use http or https.')
    const evidence = await this.webEvidenceRepository.save(this.webEvidenceRepository.create({
      ...scopedCreate(scope), caseId: input.caseId, supplierName: normalizeOptional(input.supplierName),
      kind: input.kind, sourceUrl, title: normalizeRequired(input.title, 'Evidence title is required.'),
      excerpt: normalizeRequired(input.excerpt, 'Evidence excerpt is required.').slice(0, 4000),
      confidence: Math.max(0, Math.min(1, input.confidence ?? 0.5)), status: 'active'
    }))
    return { evidence: { id: evidence.id, caseId: evidence.caseId, sourceUrl: evidence.sourceUrl, kind: evidence.kind } }
  }
  ...
  private async getCaseDetail(scope: ProcurementScope, caseId: string) {
    ...
    const [documents, parseJobs, requirementItems, supplierQuotes, quoteItems, itemMatches, risks, recommendations, webEvidence] =
      await Promise.all([
        ...
        this.webEvidenceRepository.find({ where: scopedWhere(scope, { caseId }) })
      ])
