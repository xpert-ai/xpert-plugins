...
AI 供应商尽调助手面向企业采购、行政采购、IT 设备采购和财务共享等场景。用户在 Xpert Project 或供应商尽调 Xpert 工作台中上传采购需求单，系统自动创建供应商尽调项目；随后上传多家供应商报价单，由已配置的供应商尽调 Xpert 调用插件工具完成结构化解析、横向比价、风险识别和推荐说明。
...
- 在 Xpert Project 页面 `detail.sections` slot 挂载供应商尽调工作台。
- 上传采购需求单创建采购项目；保留手动新建动作作为后台 action。
- 在采购项目详情页分入口上传采购需求单和供应商报价单。
- 用户手动触发解析，不做上传后自动解析。
- 同一个供应商尽调 Xpert 处理需求解析、报价解析和汇总比价。
...
2. 在 Project 详情页打开「供应商尽调助手」。
3. 点击「上传采购需求单」，系统根据文件名自动创建采购项目。
4. 进入采购项目详情。
5. 批量上传 2-3 份供应商报价单。
7. 填写或选择已挂载本插件 middleware 的供应商尽调 Xpert。
8. 点击「解析采购需求」。
9. 点击「批量解析报价单」。
...
- 解析配置：供应商尽调 Xpert ID、分步解析按钮、一键解析按钮。
- 文件区：采购需求单、供应商报价单。
...
业务隔离以「供应商尽调项目」为边界，同时按 tenant、organization、workspace/project 过滤。

供应商尽调项目不是 Xpert Project 本身，而是插件内的业务 case。Xpert Project 只是页面宿主和工作空间边界。

核心对象：

- `ProcurementComparisonCase`：供应商尽调项目。
