...
```sh
pnpm --filter @xpert-ai/plugin-supplier-due-diligence build
```
...
```sh
pnpm --filter @xpert-ai/plugin-supplier-due-diligence test
```
...
node plugin-dev-harness/dist/index.js \
  --workspace ./community/apps/supplier-due-diligence \
  --plugin @xpert-ai/plugin-supplier-due-diligence
```
