...
const componentName = 'meeting-action-workbench'
...
const result = await viteBuild({
  ...
  build: {
    ...
    lib: {
      ...
      name: 'XpertMeetingActionWorkbench',
      formats: ['iife'],
      fileName: () => 'app.js'
    }
  }
...
const normalizeArtifact = (output) => output.replace(/\r\n/g, '\n').replace(/[\t ]+(?=\n)/g, '')
...
if (!js || !css) throw new Error('Meeting Workbench build did not produce both JavaScript and CSS assets.')

if (process.argv.includes('--check')) {
  let stale = false
  for (const [path, output] of artifacts) {
    const current = existsSync(path) ? await readFile(path, 'utf8') : ''
    if (normalizeArtifact(current) !== output) {
