...
const SALES_ONTOLOGY_MANIFEST: SalesOntologyManifest = {
  ...
  version: {
    ...
    notes:
      "Sales Ontology business decision ontology for data-xpert business_ontology resources.",
  },
  entityTypes: [
    {
      ...
      description:
        "Customer, doctor, hospital, target, compliance alert, or other sales-domain business object.",
  ...
  actionTypes: [
    ...
    ...[
      ...
      [
        "sales_ontology.generate_visit_brief",
        "Generate Visit Brief",
        "LOW",
        false,
      ],
      [
        "sales_ontology.update_access_status",
      ...
      [
        "sales_ontology.update_actual_value",
        "Update Actual Value",
        "MEDIUM",
        true,
      ],
...
const SALES_ONTOLOGY_DOMAIN_ONTOLOGY = {
  RevenueManagement: {
    description:
      "Revenue target management: sales flow, market potential, hospital development, territory performance.",
    objectTypes: [
      "SalesFlow",
      "MarketPotential",
      "HospitalDevelopment",
  ...
  CustomerManagement: {
    description:
      "Customer management: categorization, visit feedback, PDCA plans, one-hospital-one-strategy.",
    objectTypes: [
      "CustomerCategory",
      "VisitFeedback",
      "PDCAPlan",
    ...
    rules: [
      "customer.category.scoring",
      "visit.feedback.followup",
      "pdca.lifecycle",
    ],
  },
  ExpenseManagement: {
    description:
      "Expense governance: budgets, classifications, labor payments, ROI and utilization.",
    objectTypes: [
      "BudgetCategory",
  ...
  MedicalAffairs: {
    description:
      "Medical affairs: real-world studies, academic events, publications and medical evidence.",
    objectTypes: [
      "RWSProject",
      "AcademicEvent",
      "Publication",
  ...
  ComplianceManagement: {
    description:
      "Compliance management: risk alerts, audit findings, policy controls and remediation plans.",
    objectTypes: [
      "ComplianceAlert",
      "AuditFinding",
      "PolicyRule",
...
export class SalesOntologyService {
  ...
  async seedDatabase(
    ...
    const syncMode =
      readString(input["syncMode"]) === "replace_snapshot"
        ? "replace_snapshot"
        : "merge";
    ...
    const ontologyActions = SALES_ONTOLOGY_DEMO_PROPOSALS.map(
      demoProposalToOntologyAction
    );
    const ontology = {
      resourceId,
      syncMode,
      ...
      result: undefined as
        | SalesOntologyBusinessOntologyPublishResponse
        | undefined,
    };

    if (publishOntology) {
      if (this.ontologyClient.isConfigured()) {
        ontology.result = await this.ontologyClient.publish(
          resourceId,
          {
            manifest: SALES_ONTOLOGY_MANIFEST,
    ...
    if (includeInternalRecords) {
      ...
      const savedPerceptions = await this.perceptionRepository.save(
        perceptions
      );
      internalRecords.perceptions = savedPerceptions.length;

      const suggestions = buildSuggestions(seedObjects).map(
        (suggestion, index) =>
          this.suggestionRepository.create({
            id: stableSeedId(
              scope,
              "suggestion",
              seedSuggestionKey(suggestion, index)
            ),
            ...scoped,
      ...
      const notifications = SALES_ONTOLOGY_DEMO_NOTIFICATIONS.map(
        (notification) =>
          this.notificationRepository.create({
            ...
            entityTypeCode: notification.entityExternalKey
      ...
      const reminders = SALES_ONTOLOGY_DEMO_REMINDERS.map((reminder, index) =>
        this.reminderRepository.create({
          ...
          entityTypeCode: reminder.entityExternalKey
            ? "sales_ontology_object"
            : undefined,
          payload: { seedSource: SALES_ONTOLOGY_DEMO_SEED_SOURCE },
        })
      ...
      const effects = SALES_ONTOLOGY_DEMO_SCENARIOS.map((scenario) =>
        this.decisionEffectRepository.create({
          ...
          status: computeMetricStatus(
            scenario.baselineAchievementRate,
            scenario.achievementRate
          ),
          evidence: {
      ...
      const memories = [
        ...
        this.memoryRepository.create({
          ...
          memoryType: "episodic",
          contentText:
            "Sales Ontology demo agents are initialized for perception, reasoning, compliance, action governance, scenario simulation, and learning.",
          metadata: {
            agents: [
              "InsightAgent",
              "ComplianceAgent",
  ...
  async publishSnapshot(
    scope: SalesOntologyScope,
    input: SalesOntologyPublishInput = {}
  ) {
    ...
    const result = await this.ontologyClient.publish(
      resourceId,
      payload,
      scope
    );
  ...
  async getCustomerContext(
    ...
    const objects = await this.queryObjects(scope, input, keyword, limit);
  ...
  async getComplianceRisks(
    ...
    const limit = readLimit(input["limit"], 100);
    const objects = await this.queryObjects(
      scope,
      input,
      readString(input["keyword"]),
      limit
    );
    const risky = objects
      .map((object) => ({
        ...object,
        riskLevel:
          readString(object.attributes["compliance_risk_level"]) ??
          readString(object.properties["complianceRiskLevel"]),
        riskScore:
          readNumber(object.properties["risk_score"]) ??
          readNumber(object.properties["riskScore"]),
      }))
      .filter(
        (object) =>
          object.objectType === "ComplianceAlert" ||
          ["high", "critical"].includes(
            (object.riskLevel ?? "").toLowerCase()
          ) ||
  ...
  async getSalesTargetStatus(
    ...
    const objects = await this.queryObjects(
      ...
      input,
      readString(input["keyword"]),
      readLimit(input["limit"], 200)
    );
    const targets = objects.filter(
      (item) =>
        item.objectType === "SalesTarget" || item.domain === "sales_target"
    );
    const rows = targets.map((target) => {
      const actual =
        readNumber(target.properties["actual"]) ??
        readNumber(target.properties["actual_amount"]) ??
        0;
      const targetValue =
        readNumber(target.properties["target"]) ??
        readNumber(target.properties["target_amount"]) ??
        0;
    ...
    const rates = rows
      .map((item) => item.achievementRate)
      .filter((value): value is number => typeof value === "number");
    return {
      total: rows.length,
      averageAchievementRate: average(rates),
      underperformingCount: rows.filter(
        (item) => (item.achievementRate ?? 1) < 0.8
      ).length,
      items: rows,
    };
  }
  ...
  async runPerception(
    ...
    run.output = {
      ...
      highRiskCount: saved.filter((item) => (item.riskScore ?? 0) >= 0.7)
        .length,
      items: saved,
    };
  ...
  async runReasoning(
    ...
    const reasoningType = normalizeReasoningType(
      readString(input["reasoningType"]) ?? "causal"
    );
    const objects = await this.queryObjects(
      scope,
      input,
  ...
  async generateInsights(
    ...
    run.confidence = average(
      insights
        .map((item) => item.confidence)
        .filter((value): value is number => typeof value === "number")
    );
    await this.decisionRunRepository.save(run);
    await this.publishDecisionArtifacts(
      scope,
      input,
      insights.map(insightToEntity),
      insights.map(insightToRelation).filter(Boolean)
    );
    return run.output;
  }
  ...
  async generateSuggestions(
    ...
    await this.publishDecisionArtifacts(
      scope,
      input,
      saved.map(suggestionToEntity),
      saved.map(suggestionToRelation).filter(Boolean)
    );
    return {
      total: saved.length,
      items: saved,
  ...
  async proposeAction(
    ...
    const proposal = await this.proposalRepository.save(
      this.proposalRepository.create({
        ...
        actionType:
          readString(input["actionType"]) ?? "sales_ontology.propose_action",
        entityTypeCode:
          readString(target?.["entityTypeCode"]) ??
          readString(input["entityTypeCode"]),
        entityExternalKey:
          readString(target?.["externalKey"]) ??
          readString(input["entityExternalKey"]),
        entityName:
          readString(target?.["label"]) ?? readString(input["entityName"]),
        entityObjectType:
          readString(target?.["objectType"]) ??
          readString(input["entityObjectType"]),
        ...
        actionDefinition:
          readRecord(input["actionDefinition"]) ?? inputToRecord(input),
    ...
    await this.publishDecisionArtifacts(
      ...
      [proposalToRelation(proposal)].filter(Boolean),
      [
        {
          actionTypeCode:
            proposal.actionType ?? "sales_ontology.propose_action",
          target:
            proposal.entityTypeCode && proposal.entityExternalKey
              ? {
                  entityTypeCode: proposal.entityTypeCode,
                  externalKey: proposal.entityExternalKey,
                }
              : undefined,
          status: proposal.status,
  ...
  async recordActionResult(
    ...
    if (proposalId) {
      const proposal = await this.proposalRepository.findOne({
        where: scopedWhere(scope, { id: proposalId }),
      });
      if (proposal) {
        proposal.status = status === "failed" ? "failed" : "executed";
        proposal.executionLogs = [
          ...(proposal.executionLogs ?? []),
  ...
  async executeObjectAction(
    scope: SalesOntologyScope,
    input: Record<string, unknown> = {}
  ) {
    const actionName = normalizeActionName(
      readString(input["actionName"]) ??
        readString(input["actionType"]) ??
        "sales_ontology.propose_action"
    );
    const targetExternalKey =
      readString(input["targetExternalKey"]) ??
      readString(input["entityExternalKey"]);
    const targetLabel =
      readString(input["targetLabel"]) ?? readString(input["entityName"]);
    const params =
      readRecord(input["params"]) ?? readRecord(input["parameters"]) ?? {};
    const actionDefinition = getActionDefinition(actionName);

    if (
      actionDefinition?.requiresApproval &&
      input["approved"] !== true &&
      ...
      const proposal = await this.proposeAction(scope, {
        ...
        description:
          ...
          `Approval required before executing ${actionDefinition.name}.`,
        actionType: actionName,
        priority:
          actionDefinition.riskLevel === "HIGH" ||
          actionDefinition.riskLevel === "CRITICAL"
            ? "high"
            : "medium",
        confidence: readNumber(input["confidence"]) ?? 0.78,
        target: {
          entityTypeCode:
            readString(input["targetEntityTypeCode"]) ??
            "sales_ontology_object",
    ...
    const target = await this.resolveTargetObject(
      scope,
      input,
      targetExternalKey
    );
    const preconditionResult = evaluatePreconditions(
      target,
      readArrayOfStrings(input["preconditions"])
    );
    ...
    const sideEffects = await this.executeSideEffects(
      scope,
      target,
      actionName,
      params,
      readArrayOfStrings(input["sideEffects"])
    );
    ...
    const writeBackLogs = await Promise.all(
      writeBackTargets.map((targetSystem) =>
        this.saveExecutionLog(scope, {
          ...
          result: {
            targetSystem,
            message: `Write-back intent to ${targetSystem} logged.`,
          },
        })
      )
    );
    ...
    await this.publishDecisionArtifacts(
      scope,
      input,
      buildActionExecutionEntities(target, actionName, main),
      [],
      [
        {
          actionTypeCode: actionName,
          actionRef: log.id,
          entity: targetExternalKey
            ? {
                entityTypeCode: "sales_ontology_object",
                externalKey: targetExternalKey,
              }
    ...
    const notification = await this.notificationRepository.save(
      this.notificationRepository.create({
        ...
        entityExternalKey:
          ...
          readString(input["targetExternalKey"]),
        entityTypeCode:
          readString(input["entityTypeCode"]) ?? "sales_ontology_object",
  ...
  async recordDecisionEffect(
    scope: SalesOntologyScope,
    input: Record<string, unknown> = {}
  ) {
    const decisionId =
      readString(input["decisionId"]) ??
      readString(input["proposalId"]) ??
      readString(input["entityExternalKey"]);
    if (!decisionId) {
    ...
    const effect = await this.decisionEffectRepository.save(
      this.decisionEffectRepository.create({
        ...
        decisionId,
        decisionType:
          readString(input["decisionType"]) ?? readString(input["actionType"]),
        ...
        status:
          ...
          computeMetricStatus(
            readNumber(input["expectedValue"]),
            readNumber(input["actualValue"])
          ),
  ...
  async getDecisionEffects(
    scope: SalesOntologyScope,
    input: Record<string, unknown> = {}
  ) {
    const decisionId =
      readString(input["decisionId"]) ?? readString(input["proposalId"]);
    const where = decisionId
      ? scopedWhere(scope, { decisionId })
      : scopedWhere(scope);
  ...
  async simulateScenario(
    ...
    const baselineForecast =
      readNumber(input["baselineForecastValue"]) ??
      readNumber(input["forecastValue"]) ??
      0;
    const targetValue = readNumber(input["targetValue"]) ?? baselineForecast;
    const baseDelta = readNumber(input["delta"]) ?? 0;
    const adjustmentFactor = Object.values(params).reduce<number>(
      (factor, value) => {
        const numeric = readNumber(value);
        return numeric === undefined
          ? factor
          : factor * (1 + (numeric / 100) * 0.08);
      },
      1
    );
    const forecastValue = Math.round(
      baselineForecast * (1 + (baseDelta / 100) * adjustmentFactor)
    );
    const achievementRate =
      targetValue > 0
        ? Math.round((forecastValue / targetValue) * 1000) / 10
        : 0;
    const riskLevel =
      achievementRate >= 100
        ? "on_track"
    ...
    const result = {
      ...
      delta:
        Math.round(
          ((forecastValue - baselineForecast) / Math.max(1, baselineForecast)) *
            1000
        ) / 10,
    };
  ...
  async getViewData(
    ...
    const perceptionRecords = perceptions as unknown as Array<
      Record<string, unknown>
    >;
    const suggestionRecords = suggestions as unknown as Array<
      Record<string, unknown>
    >;
    const proposalRecords = proposals as unknown as Array<
      Record<string, unknown>
    >;
    ...
    const notificationRecords = notifications as unknown as Array<
      Record<string, unknown>
    >;
    const reminderRecords = reminders as unknown as Array<
      Record<string, unknown>
    >;
    const scenarioRecords = scenarios as unknown as Array<
      Record<string, unknown>
    >;
    ...
    let neighborhood:
      | SalesOntologyBusinessOntologyNeighborhoodResponse
      | undefined;
    let graphError: string | undefined;
    const ontologyConfigured = this.ontologyClient.isConfigured();
    if (ontologyConfigured) {
      try {
        ontologyObjects = await this.queryObjects(scope, input, search, 120);
        if (graphObjectType) {
          ontologyObjects = ontologyObjects.filter(
            (object) => object.objectType === graphObjectType
          );
        }
      } catch (error) {
      ...
      try {
        ontologyRelations = await this.queryRelations(scope, input, 300);
      } catch (error) {
      ...
      if (selected) {
        try {
          neighborhood = await this.ontologyClient.getNeighborhood(
            this.resourceId(readString(input["resourceId"])),
            selected.entityTypeCode,
            selected.externalKey,
            scope
          );
          ontologyRelations = mergeRelations(
            ontologyRelations,
            extractNeighborhoodRelations(neighborhood)
          );
        } catch (error) {
          graphError = getErrorMessage(error);
        }
    ...
    const insightItems = buildWorkbenchInsightItems(
      perceptionRecords,
      suggestionRecords,
      effectRecords
    );
    ...
    const assistantItems = buildAssistantItems(
      runRecords,
      logRecords,
      notificationRecords,
      reminderRecords
    );
    const toolStatus = buildToolStatus(
      this.getDomainOntology(),
      ontologyConfigured,
      logRecords
    );
    const currentItems = selectWorkbenchItems(viewTab, {
      workspace: buildWorkspaceItems(
        perceptionRecords,
        suggestionRecords,
        proposalRecords,
        notificationRecords,
    ...
    return {
      ...
      summary: {
        ...
        pendingProposals: proposals.filter((item) => item.status === "pending")
          .length,
        highRiskPerceptions: perceptions.filter(
          (item) => (item.riskScore ?? 0) >= 0.7
        ).length,
  ...
  async executeProposal(
    ...
    const execution = await this.executeObjectAction(scope, {
      ...
      params:
        readRecord(actionDefinition?.["params"]) ?? actionDefinition ?? {},
      approved: true,
      sideEffects: readArrayOfStrings(actionDefinition?.["sideEffects"]),
      writeBackTargets: readArrayOfStrings(
        actionDefinition?.["writeBackTargets"]
      ),
    });
    const log = await this.saveExecutionLog(scope, {
      proposalId,
      ...
      result:
        readRecord(input["result"]) ??
        (execution as unknown as Record<string, unknown>),
    });
    if (execution["success"] === false) {
      proposal.status = "failed";
      proposal.errorMessage = readString(
        (execution as Record<string, unknown>)["message"]
      );
    }
    proposal.executionLogs = [
      ...(proposal.executionLogs ?? []),
  ...
  private async queryObjects(
    scope: SalesOntologyScope,
    input: Record<string, unknown>,
    query: string | undefined,
    limit: number
  ) {
    const response = await this.ontologyClient.queryEntities(
      this.resourceId(readString(input["resourceId"])),
      {
        entityTypeCode: "sales_ontology_object",
        query,
  ...
  private async queryRelations(
    ...
    const response = await this.ontologyClient.queryRelations(
      this.resourceId(readString(input["resourceId"])),
      {
  ...
  private async resolveTargetObject(
    scope: SalesOntologyScope,
    input: Record<string, unknown>,
    ...
    if (!targetExternalKey) {
      return {
        ...
        label:
          readString(input["targetLabel"]) ??
          readString(input["entityName"]) ??
          "Unknown target",
  ...
  private async executeSideEffects(
    ...
    const effects = sideEffects?.length
      ? sideEffects
      : defaultSideEffectsForAction(actionName);
    const results = [];
    for (const effect of effects) {
      if (effect.startsWith("notify ")) {
        const notification = await this.sendNotification(scope, {
          ...
          message: `${actionLabel(actionName)} executed for ${
            target.label ?? target.externalKey
          }.`,
          priority:
            actionName.includes("compliance") || actionName.includes("risk")
              ? "high"
              : "medium",
        ...
        results.push({
          effect,
          status: "logged",
          field: effect.replace("update ", "").trim(),
        });
      } else if (effect.startsWith("trigger ")) {
        results.push({
          effect,
          status: "logged",
          engine: effect.replace("trigger ", "").trim(),
        });
      } else {
  ...
  private async publishDecisionArtifacts(
    ...
    return this.ontologyClient.publish(
      this.resourceId(readString(input["resourceId"])),
      {
        manifest: SALES_ONTOLOGY_MANIFEST,
        entities: normalizePublishEntities(
          entities.filter(Boolean) as SalesOntologyEntityInput[]
        ),
        relations: normalizePublishRelations(
          relations.filter(Boolean) as SalesOntologyRelationInput[]
        ),
...
function demoObjectToSummary(
  object: SalesOntologyDemoObject
): SalesOntologyObjectSummary {
  return {
...
function demoRelationToOntologyRelation(
  relation: SalesOntologyDemoRelation
): SalesOntologyRelationInput {
  return {
    relationTypeCode: "sales_ontology_object_link",
    source: {
      entityTypeCode: "sales_ontology_object",
      externalKey: relation.source,
    },
    target: {
      entityTypeCode: "sales_ontology_object",
      externalKey: relation.target,
    },
    attributes: {
...
function demoObjectActionToEntity(
  action: (typeof SALES_ONTOLOGY_DEMO_OBJECT_ACTIONS)[number]
): SalesOntologyEntityInput {
  return {
...
function demoObjectActionToRelation(
  action: (typeof SALES_ONTOLOGY_DEMO_OBJECT_ACTIONS)[number]
): SalesOntologyRelationInput {
  return {
...
function demoProposalToOntologyEntity(
  proposal: SalesOntologyDemoProposal
): SalesOntologyEntityInput {
  return {
...
function demoProposalToOntologyRelation(
  proposal: SalesOntologyDemoProposal
): SalesOntologyRelationInput {
  return {
...
function demoProposalToOntologyAction(
  proposal: SalesOntologyDemoProposal
): SalesOntologyActionInput {
  return {
...
function seedTextEvidence(texts: string[] | undefined) {
  const entries = texts?.length
    ? texts
    : [SALES_ONTOLOGY_DEMO_SEED_SOURCE.title];
...
function seedSuggestionKey(
  suggestion: ReturnType<typeof buildSuggestions>[number],
  index: number
) {
  const target = readRecord(suggestion.targetEntities?.[0]);
  return (
    readString(target?.["externalKey"]) ??
    `${suggestion.type}:${suggestion.title}:${index}`
  );
}
...
type SalesOntologyWorkbenchTab =
  | "workspace"
  | "graph"
  | "insights"
  | "rules"
...
function buildWorkbenchInsightItems(
  ...
  const perceptionItems = perceptions.map((item) => {
    ...
    return {
      id:
        readString(item["id"]) ??
        readString(item["entityExternalKey"]) ??
      ...
      title:
        ...
        readString(item["entityExternalKey"]) ??
        "Sales Ontology perception",
      description:
        readString(readRecord(alerts[0])?.["message"]) ??
        readString(item["state"]) ??
        "Sales Ontology perception result",
      priority:
        riskScore >= 0.75 ? "high" : riskScore >= 0.55 ? "medium" : "low",
  ...
  const suggestionItems = suggestions.map((item) => {
    ...
    return {
      ...
      kind: "suggestion",
      category: inferInsightCategory(
        `${type} ${title} ${readString(item["description"]) ?? ""}`
      ),
      title,
  ...
  const effectItems = effects.map((item) => ({
    ...
    description: `${readString(item["status"]) ?? "unknown"} ${
      readNumber(item["actualValue"]) ?? "-"
    } / ${readNumber(item["expectedValue"]) ?? "-"}`,
...
function inferInsightCategory(text: string) {
  ...
  if (/opportunity|growth|potential|next_best|action/.test(normalized))
    return "opportunity";
...
function buildWorkbenchGraph(input: {
  ...
  if (!input.ontologyConfigured) {
    return {
      ...
      domainGroups: Object.entries(SALES_ONTOLOGY_DOMAIN_ONTOLOGY).map(
        ([key, value]) => ({
          key,
  ...
  const objectExternalKeys = new Set(
    input.ontologyObjects.map((object) => object.externalKey).filter(Boolean)
  );
  ...
  const relations = mergeRelations(
    ontologyRelations,
    demoRelationsForObjects(objectExternalKeys)
  );
  relations.forEach((relation, index) => {
    const sourceId = graphNodeId(
      relation.source.entityTypeCode,
      relation.source.externalKey
    );
    ...
    addNode({
      ...
      label:
        readString(relation.attributes?.["target_label"]) ??
        relation.target.externalKey,
      objectType:
        readString(relation.attributes?.["target_object_type"]) ??
        relation.target.entityTypeCode,
      source: "ontology_relation_endpoint",
    });
    addEdge({
      ...
      target: targetId,
      relationType:
        readString(relation.attributes?.["link_type"]) ??
  ...
  for (const perception of input.perceptions) {
    ...
    const entityTypeCode =
      readString(perception["entityTypeCode"]) ?? "sales_ontology_object";
  ...
  for (const suggestion of input.suggestions) {
    ...
    const targets = Array.isArray(suggestion["targetEntities"])
      ? suggestion["targetEntities"]
      : [];
    ...
    for (const target of targets) {
      ...
      const entityTypeCode =
        readString(record?.["entityTypeCode"]) ?? "sales_ontology_object";
      if (!externalKey) continue;
  ...
  for (const proposal of input.proposals) {
    ...
    const entityTypeCode =
      readString(proposal["entityTypeCode"]) ?? "sales_ontology_object";
    ...
    if (externalKey) {
      ...
      addNode({
        ...
        objectType:
          readString(proposal["entityObjectType"]) ?? "BusinessObject",
...
function relationEdgeId(relation: SalesOntologyRelationInput, index = 0) {
  const relationType =
    readString(relation.attributes?.["link_type"]) ?? relation.relationTypeCode;
  return `${graphNodeId(
    relation.source.entityTypeCode,
    relation.source.externalKey
  )}:${graphNodeId(
...
function demoRelationsForObjects(objectExternalKeys: Set<string>) {
  ...
  return SALES_ONTOLOGY_DEMO_RELATIONS.filter(
    (relation) =>
      objectExternalKeys.has(relation.source) ||
      objectExternalKeys.has(relation.target)
  ).map(demoRelationToOntologyRelation);
}

function extractNeighborhoodRelations(
  neighborhood: SalesOntologyBusinessOntologyNeighborhoodResponse | undefined
) {
  if (!neighborhood) {
    return [];
  }
...
function normalizeRelations(items: unknown[]) {
  return items
    ...
    .filter((relation): relation is SalesOntologyRelationInput =>
      Boolean(relation)
    );
}
...
function normalizeRelationSummary(
  input: unknown
): SalesOntologyRelationInput | undefined {
  const record = readRecord(input) ?? {};
  const attributes = {
    ...(readRecord(record["attributes"]) ??
  ...
  const relationTypeCode =
    readString(record["relationTypeCode"]) ??
    explicitRelationType ??
    "sales_ontology_object_link";
  ...
  return {
    ...
    target: targetRef,
    attributes,
    provenance: readArrayOfRecords(
      record["provenance"]
    ) as SalesOntologyRelationInput["provenance"],
  };
}
...
function normalizeEntityRef(
  ...
  const directRef =
    parseEntityRefValue(record[side]) ??
    parseEntityRefValue(record[`${side}Id`]);
  const entityTypeCode =
...
function buildWorkbenchRuleItems(runs: Array<Record<string, unknown>>) {
  const domainRules = Object.entries(SALES_ONTOLOGY_DOMAIN_ONTOLOGY).flatMap(
    ([domain, value]) =>
  ...
  const reasoningRuns = runs
    ...
    .map((run) => ({
      id:
        readString(run["id"]) ??
        `reasoning:${readString(run["createdAt"]) ?? Date.now()}`,
      kind: "reasoning_run",
      title:
        readString(readRecord(run["output"])?.["reasoningType"]) ?? "reasoning",
      description:
        readString(readRecord(run["output"])?.["hypothesis"]) ??
        readString(run["status"]) ??
        "completed",
      confidence:
        readNumber(run["confidence"]) ??
        readNumber(readRecord(run["output"])?.["confidence"]) ??
        0,
...
function buildAssistantItems(
  ...
  return [
    ...runs
      ...
      .map((run) => ({
        kind: "run",
        title: readString(run["runType"]) ?? "run",
        source: run,
      })),
    ...logs
      .slice(0, 12)
      .map((log) => ({
        kind: "tool_log",
        title:
          readString(log["toolName"]) ??
          readString(log["actionName"]) ??
          "tool",
        source: log,
      })),
    ...notifications
    ...
    ...reminders
      ...
      .map((item) => ({
        kind: "reminder",
        title: readString(item["title"]) ?? "reminder",
        source: item,
      })),
  ];
}
...
function buildToolStatus(
  domain: ReturnType<SalesOntologyService["getDomainOntology"]>,
  ontologyConfigured: boolean,
  logs: Array<Record<string, unknown>>
) {
  const recentTools = new Set(
    logs
      .map(
        (log) => readString(log["toolName"]) ?? readString(log["actionName"])
      )
      .filter(Boolean)
  );
  const items = [
    {
      key: "sales_ontology_publish_business_snapshot",
      title: "Publish ontology snapshot",
      group: "ontology",
    },
    {
      key: "sales_ontology_get_customer_context",
      title: "Read customer context",
      group: "context",
    },
    {
      key: "sales_ontology_run_perception",
      title: "Run perception",
      group: "perception",
    },
    {
      key: "sales_ontology_run_reasoning",
      title: "Run reasoning",
      group: "reasoning",
    },
    {
      key: "sales_ontology_generate_suggestions",
      title: "Generate suggestions",
      group: "planning",
    },
    {
      key: "sales_ontology_propose_action",
      title: "Propose governed action",
      group: "governance",
    },
    {
      key: "sales_ontology_simulate_scenario",
      title: "Simulate scenario",
      group: "scenario",
    },
    {
      key: "sales_ontology_record_decision_effect",
      title: "Record decision effect",
      group: "learning",
    },
  ].map((item) => ({
    ...item,
...
function normalizeProvenanceEntries(
  provenance: SalesOntologyEntityInput["provenance"]
) {
  ...
  const normalized = provenance
    .map((entry, index) => {
      ...
      const ref =
        ...
        `sales-ontology:evidence:${index + 1}`;
      return {
        ref,
...
function normalizeObjectSummary(input: unknown): SalesOntologyObjectSummary {
  ...
  const properties =
    readRecord(attributes["properties"]) ??
    readRecord(record["properties"]) ??
    {};
  return {
    externalKey:
      readString(record["externalKey"]) ??
    ...
    state:
      readString(record["currentStateCode"]) ??
      readString(attributes["status"]),
    objectType:
      readString(attributes["object_type"]) ??
      readString(properties["objectType"]),
    domain:
      readString(attributes["domain"]) ?? readString(properties["domain"]),
    properties,
    attributes,
...
function analyzeObject(object: SalesOntologyObjectSummary) {
  ...
  const prescriptionVolume =
    readNumber(object.properties["prescription_volume"]) ??
    readNumber(object.properties["prescriptionVolume"]);
  if (
    object.objectType === "Doctor" &&
    typeof prescriptionVolume === "number" &&
    prescriptionVolume < 80
  ) {
    riskScore += 0.28;
    churnProbability += 0.22;
    loyaltyScore -= 0.18;
    anomalies.push({
      type: "low_prescription_volume",
      value: prescriptionVolume,
      threshold: 80,
    });
    alerts.push({
      severity: prescriptionVolume < 50 ? "high" : "medium",
      message: "Prescription volume is below target.",
    });
  }

  const lastVisit = readDate(
    object.properties["last_visit_date"] ?? object.properties["lastVisitDate"]
  );
  if (lastVisit && daysBetween(lastVisit, new Date()) > 28) {
    ...
    anomalies.push({
      type: "visit_gap",
      days: daysBetween(lastVisit, new Date()),
      threshold: 28,
    });
  }

  const accessStatus =
    readString(object.properties["access_status"]) ??
    readString(object.properties["accessStatus"]);
  if (
    object.objectType === "Hospital" &&
    ["pending", "restricted", "blocked"].includes(
    ...
    alerts.push({
      severity: "high",
      message: "Hospital access status blocks field execution.",
    });
  }
  ...
  if (["high", "critical"].includes((complianceRisk ?? "").toLowerCase())) {
    ...
    alerts.push({
      ...
      message: "Compliance risk requires review.",
    });
  }
...
function buildReasoningConclusion(
  reasoningType: string,
  context: { total: number; objectTypes: Record<string, number> }
) {
  const dominantType =
    Object.entries(context.objectTypes).sort((a, b) => b[1] - a[1])[0]?.[0] ??
    "business objects";
  return {
...
function buildAdvancedReasoning(
  reasoningType: string,
  objects: SalesOntologyObjectSummary[],
  input: Record<string, unknown>
) {
  ...
  const chain: Record<string, unknown>[] = [
    {
      ...
      result: `Loaded ${objects.length} Sales Ontology ontology objects.`,
    },
  ];
  const riskObjects = objects.filter(
    (object) => objectRiskScore(object) >= 0.55
  );
  ...
  if (reasoningType === "consistency" || reasoningType === "validate") {
    ...
    recommendations.push({
      ...
      priority: issues.length ? "medium" : "low",
    ...
    return reasoningResult(
      ...
      findings,
      recommendations,
      issues.length ? 0.68 : 0.88,
      issues.length
        ? "Ontology consistency issues were found."
        : "Ontology object data is internally consistent."
    );
  }
  ...
  if (reasoningType === "temporal") {
    ...
    return reasoningResult(
      ...
      findings,
      recommendations,
      findings.length ? 0.76 : 0.42,
      "Recent object properties were inspected for trend direction and cadence risk."
    );
  }
  ...
  if (reasoningType === "implicit_relations") {
    const grouped = groupBy(
      objects,
      (object) =>
        `${object.objectType ?? "Unknown"}:${object.domain ?? "global"}`
    );
    for (const [group, members] of Object.entries(grouped)) {
      if (members.length > 1) {
        findings.push({
          group,
          relation: "peer_similarity",
          candidates: members.slice(0, 5).map((item) => item.externalKey),
          confidence: 0.62,
        });
      }
    }
    recommendations.push({
      action: "publish_candidate_object_links",
      priority: findings.length ? "medium" : "low",
    });
    return reasoningResult(
      reasoningType,
      chain,
      findings,
      recommendations,
      findings.length ? 0.66 : 0.35,
      "Objects sharing type/domain were mined as candidate implicit relations."
    );
  }

  if (reasoningType === "abductive") {
    const observation =
      readString(input["observation"]) ??
      readString(input["keyword"]) ??
      "business anomaly";
    ...
    recommendations.push({
      ...
      priority: riskObjects.length ? "high" : "medium",
    });
    return reasoningResult(
      reasoningType,
      chain,
      findings,
      recommendations,
      riskObjects.length ? 0.7 : 0.35,
      "Plausible explanations were generated for observed risk signals."
    );
  }

  if (reasoningType.startsWith("attribution")) {
    const attribution = buildAttribution(
      target ?? riskObjects[0] ?? objects[0]
    );
    ...
    return reasoningResult(
      ...
      recommendations,
      0.74,
      "Metric change was attributed across relationship, access, compliance, and execution factors."
    );
  }
  ...
  if (reasoningType === "analogy") {
    ...
    recommendations.push({
      action: "reuse_successful_playbook",
      priority: analogs.length ? "medium" : "low",
    });
    return reasoningResult(
      reasoningType,
      chain,
      findings,
      recommendations,
      analogs.length ? 0.65 : 0.35,
      "Similar objects were compared to identify reusable sales playbooks."
    );
  }

  if (reasoningType === "counterfactual") {
    const intervention = readRecord(input["intervention"]) ?? {
      visit_frequency: "+1",
      compliance_risk: "reduced",
    };
    ...
    findings.push({
      ...
      baselineRisk,
      simulatedRisk,
      expectedRiskReduction: baselineRisk - simulatedRisk,
    });
    recommendations.push({
      action: "test_counterfactual_with_scenario",
      priority: "medium",
    });
    return reasoningResult(
      reasoningType,
      ...
      recommendations,
      0.62,
      "A counterfactual intervention was simulated against aggregate risk."
    );
  }
  ...
  if (reasoningType === "hierarchical") {
    ...
    findings.push({
      byDomain,
      byType,
      levels: ["domain", "object_type", "object"],
    });
    recommendations.push({
      action: "drill_down_largest_risk_domain",
      priority: "medium",
    });
    return reasoningResult(
      reasoningType,
      ...
      recommendations,
      0.7,
      "Objects were decomposed by domain and type for hierarchical risk review."
    );
  }
  ...
  if (reasoningType === "multi_step") {
    chain.push(
      {
        step: "perceive",
        result: `${riskObjects.length} risky objects detected.`,
      },
      {
        step: "reason",
        result: "Risk drivers were mapped to action candidates.",
      },
    ...
    recommendations.push(
      ...riskObjects
        .slice(0, 5)
        .map((object) => ({
          action: objectActionRecommendation(object),
  ...
  if (reasoningType === "constraint_check") {
    ...
    return reasoningResult(
      ...
      findings,
      recommendations,
      constraints.length ? 0.76 : 0.88,
      constraints.length
        ? "Business constraints have violations."
        : "No major business constraint violations were detected."
    );
  }
  ...
  if (reasoningType === "coordination") {
    const owners = countBy(
      objects,
      (object) =>
        readString(object.attributes["owner_id"]) ??
        readString(object.properties["ownerId"]) ??
        "unassigned"
    );
    findings.push({ owners, unassignedCount: owners["unassigned"] ?? 0 });
    recommendations.push({
      action: "assign_owner_and_follow_up",
      priority: (owners["unassigned"] ?? 0) > 0 ? "high" : "medium",
    });
    return reasoningResult(
      reasoningType,
      chain,
      ...
      recommendations,
      0.68,
      "Ownership and coordination gaps were inspected."
    );
  }
  ...
  for (const object of riskObjects.slice(0, 10)) {
    findings.push({
      entity: object.externalKey,
      riskScore: objectRiskScore(object),
      drivers: abductiveExplanations(object),
    });
  }
  recommendations.push(
    ...riskObjects
...
function consistencyIssues(object: SalesOntologyObjectSummary) {
  ...
  if (!object.externalKey)
    issues.push({ type: "missing_external_key", entity: object.label });
  if (!object.objectType)
    issues.push({ type: "missing_object_type", entity: object.externalKey });
  for (const [key, value] of Object.entries(object.properties)) {
    if (
      typeof value === "number" &&
      value < 0 &&
      /rate|score|achievement|share/i.test(key)
    ) {
      issues.push({
        type: "negative_value",
        entity: object.externalKey,
...
function businessConstraintIssues(object: SalesOntologyObjectSummary) {
  const issues: Record<string, unknown>[] = [];
  const achievement =
    readNumber(object.properties["achievement_rate"]) ??
    readNumber(object.properties["achievementRate"]);
  if (
    object.objectType === "SalesTarget" &&
    typeof achievement === "number" &&
    achievement < 0.7
  ) {
    issues.push({
      type: "target_under_threshold",
      entity: object.externalKey,
      achievement,
    });
  }
  const complianceRisk =
    ...
    readString(object.properties["complianceRiskLevel"]);
  if (
    ["high", "critical"].includes((complianceRisk ?? "").toLowerCase()) &&
    object.state !== "pending" &&
    object.state !== "investigating"
  ) {
    issues.push({
      type: "compliance_not_under_review",
      entity: object.externalKey,
      complianceRisk,
      state: object.state,
    });
...
function inferTemporalTrends(object: SalesOntologyObjectSummary) {
  ...
  for (const key of [
    ...
    const current =
      readNumber(object.properties[key]) ??
      readNumber(object.properties[camelCase(key)]);
    const previous =
      readNumber(object.properties[`previous_${key}`]) ??
      readNumber(object.properties[`previous${capitalize(camelCase(key))}`]);
    if (current !== undefined && previous !== undefined) {
      trends.push({
        ...
        changePercent:
          previous === 0
            ? 0
            : Math.round(((current - previous) / Math.abs(previous)) * 1000) /
              10,
        current,
        previous,
      });
...
function abductiveExplanations(object: SalesOntologyObjectSummary) {
  ...
  if (
    ...
    explanations.push(
      "Prescription volume is low; possible competitor penetration, weak visit cadence, or product perception issue."
    );
  }
  const lastVisit = readDate(
    object.properties["last_visit_date"] ?? object.properties["lastVisitDate"]
  ...
  if (
    ...
    explanations.push(
      "Hospital access or listing status constrains execution."
    );
  }
  const riskLevel =
    readString(object.attributes["compliance_risk_level"]) ??
  ...
  if (["high", "critical"].includes((riskLevel ?? "").toLowerCase())) {
    explanations.push(
      "Compliance risk is high enough to require governance before action."
    );
  }
  return explanations.length
    ? explanations
    : [
        "No dominant explanation; collect more visit, event, and time-series evidence.",
      ];
}
...
function buildAttribution(object: SalesOntologyObjectSummary | undefined) {
  const factors = [
    {
      factor: "relationship_recency",
      contribution: object
        ? abductiveExplanations(object).some((item) =>
            item.includes("Visit gap")
          )
          ? 0.32
          : 0.12
        : 0.1,
    ...
    {
      factor: "commercial_signal",
      contribution: object
        ? Math.min(0.42, objectRiskScore(object) * 0.45)
        : 0.2,
    },
    {
      factor: "access_constraint",
      contribution:
        object &&
        abductiveExplanations(object).some((item) => item.includes("access"))
          ? 0.25
          : 0.08,
    },
    {
      factor: "compliance_control",
      contribution:
        object &&
        abductiveExplanations(object).some((item) =>
...
function findTargetObject(
  ...
  const target =
    ...
    readString(input["entityExternalKey"]);
  return target
    ? objects.find((object) => object.externalKey === target)
    : undefined;
}
...
function findAnalogs(
  ...
  return objects
    .filter(
      (object) =>
        object.externalKey !== target.externalKey &&
        object.objectType === target.objectType
    )
    .map((object) => ({
      externalKey: object.externalKey,
      label: object.label,
      ...
      successfulPattern:
        objectRiskScore(object) < objectRiskScore(target)
          ? "lower_risk_peer"
          : "similar_risk_peer",
    }))
...
function similarityScore(
  a: SalesOntologyObjectSummary,
  b: SalesOntologyObjectSummary
) {
  let score = 0.35;
  if (a.domain && a.domain === b.domain) score += 0.2;
  for (const key of ["specialty", "department", "region", "territory"]) {
    if (a.properties[key] && a.properties[key] === b.properties[key])
      score += 0.12;
  }
...
function objectActionRecommendation(object: SalesOntologyObjectSummary) {
  ...
  if (object.objectType === "Hospital")
    return "sales_ontology.update_access_status";
  if (object.objectType === "SalesTarget")
    return "sales_ontology.update_actual_value";
  if (object.objectType === "ComplianceAlert")
    return "sales_ontology.escalate_alert";
...
function buildInsights(objects: SalesOntologyObjectSummary[]) {
  ...
  for (const object of objects) {
    const influence =
      readNumber(object.properties["influence_score"]) ??
      readNumber(object.properties["influenceScore"]);
    const volume =
      readNumber(object.properties["prescription_volume"]) ??
      readNumber(object.properties["prescriptionVolume"]);
    if (
      ...
      insights.push({
        ...
        description: `${
          object.label ?? object.externalKey
        } has influence signal above threshold while prescription volume is low.`,
        confidence: 0.78,
        priority: "high",
        target: object,
      });
    ...
    if (
      ...
      insights.push({
        ...
        description: `${
          object.label ?? object.externalKey
        } achievement rate is ${Math.round(achievement * 100)}%.`,
        confidence: 0.82,
  ...
  return buildInsights(objects).map((insight) => ({
    ...
    title:
      insight.priority === "critical"
        ? "Escalate and assign recovery owner"
    ...
    suggestedActions: [
      {
        actionType:
          insight.priority === "critical"
            ? "sales_ontology.flag_compliance_risk"
            : "sales_ontology.schedule_visit",
...
function suggestionToEntity(
  suggestion: SalesOntologySuggestion
): SalesOntologyEntityInput {
  return {
    entityTypeCode: "sales_ontology_suggestion",
...
function proposalToEntity(
  proposal: SalesOntologyActionProposal
): SalesOntologyEntityInput {
  return {
    entityTypeCode: "sales_ontology_action_proposal",
...
function proposalToRelation(
  ...
  return {
    ...
    source: {
      entityTypeCode: "sales_ontology_action_proposal",
      externalKey: proposal.id,
    },
    target: {
      entityTypeCode: proposal.entityTypeCode ?? "sales_ontology_object",
      externalKey: proposal.entityExternalKey,
    },
    attributes: { confidence: proposal.confidence },
  };
...
function normalizeActionName(value: string) {
  ...
  return (
    map[raw] ??
    (raw.startsWith("sales_ontology.") ? raw : `sales_ontology.${raw}`)
  );
}

function getActionDefinition(actionName: string) {
  return SALES_ONTOLOGY_MANIFEST.actionTypes.find(
    (item) => item.code === actionName
  );
}
...
function evaluateCondition(
  ...
  for (const operator of ["!=", "==", ">=", ">", " in "]) {
    ...
    if (
      ...
      return { valid: false, reason: `${left.trim()} is ${expected}` };
    if (
      operator === "==" &&
      actual !== undefined &&
      String(actual) !== expected
      ...
      return {
        valid: false,
        reason: `${left.trim()} is ${actual}, not ${expected}`,
      };
    if (
      operator === ">=" &&
      readNumber(actual) !== undefined &&
      readNumber(actual) < Number(expected)
    )
      return { valid: false, reason: `${left.trim()} is below ${expected}` };
    if (
      operator === ">" &&
      readNumber(actual) !== undefined &&
      readNumber(actual) <= Number(expected)
      ...
      return {
        valid: false,
        reason: `${left.trim()} is not greater than ${expected}`,
      };
    if (operator === " in ") {
      const values = right
        .replace(/[[\]]/g, "")
      ...
      if (actual !== undefined && !values.includes(String(actual)))
        return {
          valid: false,
          reason: `${left.trim()} is not in ${values.join(", ")}`,
        };
    }
  }
  return { valid: true };
...
function resolveTargetField(
  ...
  if (field === "lifecycleStage")
    return (
      target.attributes["lifecycle_stage"] ??
      target.properties["lifecycleStage"] ??
      target.state
    );
  if (field === "status")
    return (
      target.state ?? target.attributes["status"] ?? target.properties["status"]
    );
  return (
    target.attributes[field] ??
    target.properties[field] ??
    target.properties[camelCase(field)]
  );
}
...
function executeActionLogic(
  ...
  if (actionName === "sales_ontology.schedule_visit") {
    ...
    result["message"] = `Compliance risk flagged for ${
      target.label ?? target.externalKey
    }.`;
    ...
    const actualValue =
      readNumber(params["actualValue"]) ?? readNumber(params["actual"]);
    const targetValue =
      readNumber(target.properties["target"]) ??
      readNumber(target.properties["target_value"]) ??
      readNumber(params["targetValue"]);
    result["actualValue"] = actualValue;
    result["achievementRate"] =
      actualValue !== undefined && targetValue
    ...
    const current =
      readString(target.attributes["lifecycle_stage"]) ??
      readString(target.properties["lifecycleStage"]) ??
      "new";
    const targetStage =
      readString(params["targetStage"]) ??
      getValidTransitions(target.objectType, current)[0];
    if (
      !targetStage ||
      !validateTransition(target.objectType, current, targetStage)
    ) {
      result["success"] = false;
      result["message"] = `Cannot transition from ${current} to ${
        targetStage ?? "(none)"
      }.`;
    } else {
    ...
    result["status"] =
      readString(params["status"]) ?? readString(params["targetStage"]);
  } else if (actionName === "sales_ontology.invite_to_event") {
    result["eventId"] =
      readString(params["eventId"]) ?? readString(params["event"]);
    result["message"] = `Event invitation logged for ${
      target.label ?? target.externalKey
    }.`;
  } else {
    result[
      "message"
    ] = `${actionName} executed by generic Sales Ontology handler.`;
  }
  return result;
}
...
function defaultSideEffectsForAction(actionName: string) {
  const map: Record<string, string[]> = {
    "sales_ontology.schedule_visit": [
      "create VisitRecord",
      "update doctor.nextRecommendedVisitDate",
      "notify managedBy",
    ],
    "sales_ontology.update_sentiment": ["update doctor.sentiment"],
    "sales_ontology.flag_compliance_risk": [
      "create ComplianceAlert",
      "notify compliance",
    ],
    "sales_ontology.mark_as_at_risk": [
      "update doctor.lifecycleStage",
      "notify managedBy",
    ],
...
function actionSideEffectEntity(
  target: SalesOntologyObjectSummary,
  entityType: string,
  params: Record<string, unknown>
): SalesOntologyEntityInput {
...
function buildActionExecutionEntities(
  target: SalesOntologyObjectSummary,
  actionName: string,
  result: Record<string, unknown>
) {
...
function getValidTransitions(objectType: string | undefined, current: string) {
  ...
  return (
    (objectType ? LIFECYCLE_TRANSITIONS[objectType]?.[current] : undefined) ??
    defaultTransitions[current] ??
    []
  );
}
...
function buildLearningRecommendations(
  ...
  if ((effectRollup.byStatus["below"] ?? 0) > 0) {
    recommendations.push({
      action: "tighten_action_preconditions",
      reason: "Some decisions underperformed expected effects.",
    });
  }
  if ((effectRollup.byStatus["exceeded"] ?? 0) > 0) {
    recommendations.push({
      action: "reuse_high_effect_playbooks",
      reason: "Some decisions exceeded expected effects.",
    });
  }
  const reasoningRuns = runs.filter(
    (run) => run.runType === "reasoning"
  ).length;
  if (reasoningRuns < 3) {
    recommendations.push({
      action: "collect_more_reasoning_runs",
      reason: "Learning quality improves with more reasoning/effect pairs.",
    });
  }
...
function camelCase(value: string) {
  return value.replace(/[_-]+([a-zA-Z0-9])/g, (_, next: string) =>
    next.toUpperCase()
  );
}
