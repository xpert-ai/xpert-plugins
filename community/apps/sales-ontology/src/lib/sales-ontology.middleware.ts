...
const ActorTokenRuntimeCapability = createRuntimeCapability<ActorTokenApi>(
  "platform.actor-token"
);
...
const publishSnapshotSchema = z.object({
  resourceId: z
    ...
    .optional()
    .describe(
      "data-xpert business ontology resource id. Defaults to plugin config."
    ),
  manifest: z
    .any()
    .optional()
    .describe(
      "Optional dynamic ontology manifest. Omit to use the Sales Ontology default manifest."
    ),
  entities: z
    .array(ontologyEntitySchema)
    .default([])
    .describe(
      "Business ontology entities such as doctors, hospitals, targets, events, insights, or proposals."
    ),
  relations: z
    .array(ontologyRelationSchema)
    .default([])
    .describe("Business ontology relations between entities."),
  actions: z
    .array(ontologyActionSchema)
    .default([])
    .describe("Action states or action execution facts to publish."),
  syncMode: z
    .enum(["replace_snapshot", "merge"])
    .optional()
    .describe("Use merge for incremental updates."),
});
...
const proposeActionSchema = z.object({
  ...
  actionType: z
    ...
    .optional()
    .describe(
      "Sales Ontology action type such as sales_ontology.schedule_visit."
    ),
...
const executeObjectActionSchema = z.object({
  ...
  actionName: z
    ...
    .optional()
    .describe(
      "Sales Ontology action, e.g. scheduleVisit or sales_ontology.schedule_visit."
    ),
  ...
  protected createAllTools(
    ...
    const publishSnapshotTool = tool(
      async (input: z.infer<typeof publishSnapshotSchema>) => {
        const result = await this.service.publishSnapshot(
          scope,
          input as SalesOntologyPublishInput
        );
        return JSON.stringify({
          message:
            "Sales Ontology business ontology snapshot was published to data-xpert.",
          result,
        });
      },
      {
    ...
    const customerContextTool = tool(
      async (input: z.infer<typeof queryContextSchema>) =>
        stringifyResult(await this.service.getCustomerContext(scope, input)),
      {
    ...
    const complianceRisksTool = tool(
      async (input: z.infer<typeof queryContextSchema>) =>
        stringifyResult(await this.service.getComplianceRisks(scope, input)),
      {
        name: "sales_ontology_get_compliance_risks",
        description:
          "Find high or critical compliance risk objects from the Sales Ontology business ontology.",
        schema: queryContextSchema,
      }
    );
    ...
    const salesTargetStatusTool = tool(
      async (input: z.infer<typeof queryContextSchema>) =>
        stringifyResult(await this.service.getSalesTargetStatus(scope, input)),
      {
        name: "sales_ontology_get_sales_target_status",
        description:
          "Summarize Sales Ontology sales target achievement and underperforming target objects.",
        schema: queryContextSchema,
      }
    );
    ...
    const perceptionTool = tool(
      async (input: z.infer<typeof queryContextSchema>) =>
        stringifyResult(await this.service.runPerception(scope, input)),
      {
    ...
    const reasoningTool = tool(
      async (input: z.infer<typeof reasoningSchema>) =>
        stringifyResult(await this.service.runReasoning(scope, input)),
      {
    ...
    const insightsTool = tool(
      async (input: z.infer<typeof queryContextSchema>) =>
        stringifyResult(await this.service.generateInsights(scope, input)),
      {
        name: "sales_ontology_generate_insights",
        description:
          "Generate Sales Ontology business insights and publish insight nodes back into data-xpert ontology.",
        schema: queryContextSchema,
      }
    );
    ...
    const proposeActionTool = tool(
      async (input: z.infer<typeof proposeActionSchema>) =>
        stringifyResult(await this.service.proposeAction(scope, input)),
      {
    ...
    const createVisitRecordTool = tool(
        ...
        stringifyResult(await this.service.createVisitRecord(scope, input)),
      {
        ...
        description:
          "Create a Sales Ontology visit record/action event for a doctor or customer object.",
        schema: executeObjectActionSchema,
      }
    );
    ...
    const updateDoctorSentimentTool = tool(
        ...
        stringifyResult(await this.service.updateDoctorSentiment(scope, input)),
      {
        ...
        description:
          "Log or publish a governed Sales Ontology doctor sentiment update.",
        schema: executeObjectActionSchema,
      }
    );
    ...
    const sendNotificationTool = tool(
        ...
        stringifyResult(await this.service.sendNotification(scope, input)),
      {
        ...
        description:
          "Persist a Sales Ontology notification for a user or business object.",
        schema: notificationSchema,
      }
    );

    const createReminderTool = tool(
      async (input: z.infer<typeof reminderSchema>) =>
        stringifyResult(await this.service.createReminder(scope, input)),
      {
        name: "sales_ontology_create_reminder",
        description:
          "Create a Sales Ontology follow-up reminder attached to a user or ontology object.",
    ...
    const simulateScenarioTool = tool(
      async (input: z.infer<typeof scenarioSchema>) =>
        stringifyResult(await this.service.simulateScenario(scope, input)),
      {
        name: "sales_ontology_simulate_scenario",
        description:
          "Run and persist a Sales Ontology scenario forecast with target, baseline, delta, and adjustment parameters.",
        schema: scenarioSchema,
      }
    );

    const recordMemoryTool = tool(
      async (input: z.infer<typeof memorySchema>) =>
        stringifyResult(await this.service.recordMemory(scope, input)),
      {
        name: "sales_ontology_record_memory",
        description:
          "Persist Sales Ontology episodic or semantic memory for later learning.",
        schema: memorySchema,
      }
    );
    ...
    const getLearningSummaryTool = tool(
        ...
        stringifyResult(await this.service.getLearningSummary(scope, input)),
      {
        ...
        description:
          "Summarize recent Sales Ontology runs, memories, decision effects, and learning recommendations.",
        schema: effectQuerySchema,
      }
    );
...
export class SalesOntologyActionGovernanceMiddleware extends SalesOntologyMiddleware {
  ...
  override createMiddleware(
    ...
    return {
      ...
      tools: selectTools(
        this.createAllTools(context),
        ACTION_GOVERNANCE_TOOL_NAMES
      ),
    };
  }
}
...
function scopeFromContext(
  ...
  return {
    ...
    actorTokenProvider: createActorTokenProvider(context),
  };
}

function createActorTokenProvider(
  context: IAgentMiddlewareContext
): (() => Promise<string>) | undefined {
  const actorTokenApi = context.runtime?.capabilities?.get(
  ...
  return async () =>
    (
      await actorTokenApi.getToken({
        act: {
          sub: "sales_ontology",
          middleware_node: context.node?.key,
        },
      })
    ).token;
}
