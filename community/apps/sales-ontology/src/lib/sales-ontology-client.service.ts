...
export interface SalesOntologyBusinessOntologyPublishRequest {
  ...
  relations?: Array<
    SalesOntologyRelationInput | SalesOntologyRelationPublishRecord
  >;
...
export interface SalesOntologyEntityPublishRecord
  extends SalesOntologyEntityInput {
  displayName?: string | null;
  evidence?: SalesOntologyJsonObject;
  provenance?: SalesOntologyProvenanceRecord[];
}

export interface SalesOntologyRelationPublishRecord
  extends SalesOntologyRelationInput {
  evidence?: SalesOntologyJsonObject;
  provenance?: SalesOntologyProvenanceRecord[];
}
...
export interface SalesOntologyBusinessOntologyEntityRecord
  extends SalesOntologyBusinessOntologyEntityRefResponse {
  label?: string;
  displayName?: string | null;
  name?: string;
...
export interface SalesOntologyBusinessOntologyActionRecord
  extends Partial<SalesOntologyActionInput> {
  id?: string;
  actionName?: string;
  status?: string;
  attributes?: SalesOntologyJsonObject;
...
export class SalesOntologyClientService {
  ...
  async publish(
    ...
    request: SalesOntologyBusinessOntologyPublishRequest,
    scope?: SalesOntologyScope
  ): Promise<SalesOntologyBusinessOntologyPublishResponse> {
    return this.requestJson<SalesOntologyBusinessOntologyPublishResponse>(
      "POST",
      `/uose/business-ontology/resources/${encodeURIComponent(
  ...
  async getNeighborhood(
    ...
    return this.requestJson<SalesOntologyBusinessOntologyNeighborhoodResponse>(
      ...
      `/uose/business-ontology/resources/${encodeURIComponent(
        resourceId ?? this.defaultResourceId()
      )}/entities/${encodeURIComponent(entityTypeCode)}/${encodeURIComponent(
        externalKey
      )}/neighborhood`,
      undefined,
      scope
  ...
  private async requestJson<TResponse extends object>(
    method: "GET" | "POST",
    path: string,
    body?: unknown,
    scope?: SalesOntologyScope
    ...
    const token = await this.resolveBearerToken(scope);
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `data-xpert business ontology request failed with HTTP ${
          response.status
        }${text ? `: ${text}` : ""}`
      );
    }
  ...
  private async resolveBearerToken(
    ...
    const actorToken = (await scope?.actorTokenProvider?.())?.trim();
    if (actorToken) {
      return actorToken;
    }
  ...
  private resolveApiBase(
    config = this.resolveDataXpertConfig()
  ): string | undefined {
  ...
  private resolveDataXpertConfig(): ResolvedSalesOntologyPluginConfig {
    const pluginConfig =
      this.pluginConfigResolver?.resolve<SalesOntologyPluginConfig>(
