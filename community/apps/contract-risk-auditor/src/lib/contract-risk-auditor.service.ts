...
export class ContractRiskAuditorService {
  ...
  analyzeContractText(content: string): ClauseRiskItem[] {
    ...
    // 6. 附录表格·付款拖延与惩罚性扣款陷阱（PDF表格抽取专项）
    if (content.includes('须待第三方上级审计单位出具无保留最终审计报告') || content.includes('未设定最迟付款时间') || content.includes('延期按日扣除合同总价 1%')) {
      risks.push({
        id: 'risk-table-milestone-delay',
        originalText: '须待第三方上级审计单位出具无保留最终审计报告，且集团资金池审批后方可支付（未设定最迟付款时间）',
        category: '附录表格·以审代付拖延陷阱',
        riskLevel: 'HIGH',
        riskAnalysis: '在《项目实施里程碑与付款周期表》中将30%大额尾款捆绑于“第三方审计”与内部审批且无截止期限，构成典型“以审代付”违法霸王条款。违反最高人民法院关于大型工程物资采购的款项支付司法解释。',
        suggestedRevision: '终验合格后30日内支付30%尾款；如因甲方原因导致第三方审计超过60日未出具报告的，视为付款条件成就并于5日内全额结清',
        status: 'PENDING'
      })
    }
