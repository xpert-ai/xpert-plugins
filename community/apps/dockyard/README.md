...
在可调整的多面板文本工作台中，选中文字或右键文件，通过“帮我改 / 解释一下”把引用交给 ChatKit。用户补充问题，模型解释，或按明确要求修改并保存已引用文件。当前源码版本 0.4.1，包含 AI 写入工具与保存后编辑器同步；已部署到本任务本地测试实例。详见 [AI 修改并保存](docs/ai-edit-tool.md)。
...
两个操作不会自动发送消息。0.4.0 的“帮我改”可在用户明确修改要求后调用工具保存；截图为 0.3.0 历史运行界面。使用前成功保存文件，工具成功后，未编辑的文件会在工作台可见时约 5 秒内同步；有未保存编辑则保留并提示冲突。详细步骤见[运行说明](docs/interview/02-runbook.md)。
