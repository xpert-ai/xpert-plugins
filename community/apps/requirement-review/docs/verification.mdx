...
- 当前版本已重新安装到实际 Xpert；自动化闭环在确认快照中显示 1 次成功分析历史，并在完整刷新后恢复。
...
- 当前版本已重新安装到实际 Xpert；自动化闭环保存人工描述修改、清空问题并排除其余需求后，审计入口显示 3 项差异，完整刷新后仍存在。
...
- 为避免误用模型额度，真实运行必须显式设置 `REQTRACE_E2E_ALLOW_MODEL=1`。当前版本重新安装后已执行完整真实闭环：DeepSeek 提交 3 条需求，脚本保留并修改第一条、排除其余两条，确认后完整刷新并通过全部 6 个检查点。
...
- P1 当前版本通过本地工作区重新安装后，工作台能直接打开已有评审的“分析历史”和“审计差异”。`smoke:installed` 创建合成评审 `ReqTrace E2E 20260917101450`，真实 DeepSeek 提交 3 条需求；脚本保留第一条、追加唯一人工核对标记、清空待澄清问题并排除其余两条，保存和确认后完整刷新。刷新后人工描述、只读确认状态、1 次分析历史和 3 项审计差异均从持久化数据恢复，6/6 检查点通过。
...
需要观察过程时设置 `REQTRACE_E2E_HEADED=1`；模型较慢时可提高 `REQTRACE_E2E_TIMEOUT_MS`。脚本复用同级 Xpert 工作区安装的 Playwright，不修改插件仓库 lockfile；其他目录布局可通过 `REQTRACE_E2E_PLAYWRIGHT_ROOT` 指向包含 `playwright` 的 Node 项目。若 Playwright 自带 Chromium 不存在，Windows 会依次复用已安装的 Chrome 或 Edge；也可用 `REQTRACE_E2E_BROWSER_EXECUTABLE` 显式指定浏览器可执行文件，不需要为 smoke test 下载新浏览器。
