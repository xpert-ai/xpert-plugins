...
核心流程已实现：创建访谈评审 → 助手提取需求、证据和验收草稿 → 人工核对与取舍 → 保存草稿 → 确认只读快照。分析失败、超时或未发现需求时，可在工作台点击“修改原文”，以当前标题和原文为起点修订后重新分析。服务端持久化原文、原始 AI 草稿、人工草稿与每次分析尝试，处理范围校验、并发分析、重复请求、超时恢复、版本冲突和迟到结果。工作台可展开最近 20 次分析历史，按新到旧展示状态、原文版本、提示词版本、耗时、错误码，以及宿主已上报时的脱敏模型名和 token 用量；审计差异汇总 AI 原稿到人工草稿的字段修改，以及确认时的纳入/排除数量、原文版本和时间。
...
14 个单元、契约、生产预览状态与已安装 smoke 安全门测试已通过。独立 PostgreSQL 集成测试覆盖并发分析、幂等、失败重试、范围拒绝、版本冲突、确认恢复、失败/空结果修改原文、跨原文版本的尝试历史及迟到提交；只使用随机测试 schema。实际安装、模型流程与截图记录见验证文档。

已安装平台的真实闭环可用 `smoke:installed` 重复执行。脚本使用合成原文，覆盖创建、真实模型提交、人工编辑、保存、确认、完整刷新后的数据库恢复，以及分析历史和审计入口；默认不会运行，必须显式提供目标 URL、登录态并设置 `REQTRACE_E2E_ALLOW_MODEL=1`。登录态只保存在工作区的 `.xpert-local-environment`，不得提交。具体命令见验证文档。
...
P1 的“分析尝试历史”和“人工审计差异”已完成代码、PostgreSQL/契约、生产 Preview Host 与实际 Xpert 验证。显式开启的已安装平台脚本使用真实 DeepSeek 完整通过创建、模型提交、人工编辑、保存、确认、完整刷新恢复、分析历史和审计入口检查；测试登录态与截图只保存在不提交的 `.xpert-local-environment`。当前宿主调用路径未提供模型与 token 元数据时，界面明确显示“未上报”，不会由模型工具参数补写这些受信信息。审计 DTO 不包含 `confirmedBy` 或其他内部用户 ID。
