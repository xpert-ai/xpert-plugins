...
批量导入（JSON / CSV，对应会话存档接口或运营导表）   手工补录单条（散单）
              ...
              工作台先静默同步当前记录（assistant.context.set），再把纯人话指令注入 Assistant 对话
                     ——完整 recordId 不出现在聊天气泡里，由工具在执行时自动从上下文取；
                       气泡里客户名旁只带一个 #XXXXXXXX 短徽章（record.id 前 8 位大写），
                       方便对照左侧历史列表核对是哪一条记录
                                 │
                                 ▼
...
> **验证「同名不同客户」**：`conversations.sample.json` 与 `conversations.sample.csv` 里各有一个名叫「华东精密制造」的客户，`customerId` 分别是 `CRM-EAST-1001` 和 `CRM-EAST-9001`——同名但来自两个不同的编号。两个文件一起导入（点「获取聊天会话记录」即可）后，「历史记录」列表里这两组卡片名字相同；卡片名旁的短徽章是**这条记录自己的 id**（`#XXXXXXXX`，取 `record.id` 前 8 位，跟 Assistant 重新分析这条记录时聊天气泡里的短徽章是同一个值，方便对照哪条聊天对应哪张卡片），不是客户标识，所以每张卡片的徽章天然都不同，不能用来判断是否同一个客户——要验证「同名不同客户」是否被正确区分，把鼠标悬停在徽章上，`title` 提示里会同时带出这条记录的 `customerId`，两组卡片的 `customerId` 不同即为区分成功；各自的历史和「历史遗留事项」也互不可见。
...
### 7. 聊天框拖拽附件导入
![聊天框拖拽附件导入](docs/images/07-chat-import.png)
聊天框直接拖入微信聊天截图（`image/png`），助手识别后重建对话文本并调用 `conversation_review_import_conversations` 完成导入；同一入口也接受 JSON / CSV / Excel 附件。
...
| **聊天框拖拽附件导入（真机，单文件）** | 真实 Assistant 对话框验证 `conversation_review_import_conversations` 在模型不传 `content` 时的自动导入路径：一条消息只带一个 JSON/CSV/Excel 附件，以及本次新增的微信聊天截图附件（`wrapToolCall` 排队 → `wrapModelCall` 注入图片 → 模型重建对话 → 再次调用同一工具完成导入） | **已验证**，单文件均能正确解析并落库 |
| **聊天框拖拽附件导入（真机，一条消息多文件）** | 同一条消息同时拖 2 个附件（JSON + CSV），不传 `content` | **偶发失败**：至少复现过一次命中 `resolveChatAttachmentImport` 里 `!workspaceFiles` 的兜底分支（"reading chat-attachment bytes is not available in this environment"），即宿主没有把 `WorkspaceFilesRuntimeCapability` 注入到那次 Agent 执行的 `runtime.capabilities` 里；不是每次都失败，同一操作重试后曾恢复正常。插件代码本身没问题——取用方式与平台已上线的 `view-image` 中间件完全一致，失败时也按设计优雅降级成提示语而非报错炸工具调用。单文件能稳定读到、多文件偶发读不到，判断是宿主侧该能力的注入存在时机/会话相关的不稳定性，不是「文件数量」这个条件本身决定的，具体原因需要平台侧介入，插件仓库这边无法单独定位。批量场景如果踩到这个问题，改用工作台「选择文件导入」即可，该路径与此能力无关，不受影响 |
...
| **聊天框一条消息拖多个附件，自动导入偶发失败** | 单文件走 `resolveChatAttachmentImport` 自动读取已真机验证通过；同一条消息拖 2 个附件时真机偶发命中 `!workspaceFiles` 兜底分支（宿主没把 `WorkspaceFilesRuntimeCapability` 注入这次 Agent 执行的 `runtime.capabilities`），重试后又能恢复正常，不是稳定复现。取用方式和平台已上线的 `view-image` 中间件一致，失败已按设计优雅降级为提示语，不是插件抛异常；单文件稳定、多文件偶发失败，说明宿主侧该能力的注入存在时机/会话相关的不稳定性，不是单纯"文件数量"决定的，具体原因需要平台侧配合排查 | 批量场景改用工作台「选择文件导入」（不受此问题影响，最稳妥）；如需继续用聊天入口批量导入，失败时重试通常能恢复，等平台侧确认该能力的注入条件后再定论 |
