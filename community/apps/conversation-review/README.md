...
| Redis | 同上 | 平台依赖 |
| 模型凭证 | 任一 LLM provider | 在平台 `/copilot` 页面配置，**不在本插件里配置** |
...
需要同一客户的两条记录，且第一条已确认——历史只认 `confirmed`。示例 JSON 里 `华东精密制造` 有三次沟通（9/2 答应本周发 ERP 对接方案 → 9/9 客户追问 → 9/15 第三次追问且竞品已搭好 POC），按时间顺序分析并逐条确认，第二、三条的「历史遗留事项」会指出承诺连续落空。示例里的 `西部新材料` 只有两句话，用来触发 `report_failure` 失败路径。
...
### 1. 工作台总览
![工作台总览](docs/images/01-workbench.png)
看板 + 导入区 + 列表，一屏说明这是什么。
...
![结构化结果与人工修改](docs/images/05-result.png)
六维评分卡、分类顾虑及其原文引证、字段可改。

### 6. 历史遗留事项
![历史遗留事项](docs/images/06-carried-over.png)
同一客户第二/三次沟通，指出上次承诺仍未落实。

### 7. 保存与恢复
![保存与恢复](docs/images/07-restore.png)
确认后刷新或重进，记录与结果仍在（非页面内存）。

### 8. 异常场景（失败与重试）
![异常场景](docs/images/08-failure-retry.png)
...
Claude Code（Claude Opus 5，1M context）全程协作：需求澄清、架构设计、编码、测试编写、自查与文档。
...
| **遗留事项只识别不闭环** | 「历史遗留事项」被识别出来后没有归属人和到期时间 | 接入待办体系，让「答应过两次仍未做」主动找人 |
