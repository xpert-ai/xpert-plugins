...
export function installBridgeListener(handlers: BridgeHandlers) {
  const listener = (event: MessageEvent) => {
    ...
    if (message.type === 'init') {
      ...
      handlers.onInit({
        manifest: isObject(message.manifest) ? message.manifest : undefined,
        ...
        theme: typeof message.theme === 'string' ? message.theme : undefined
      })
...
/** 让宿主在当前会话里代发一条消息 —— 这是把「让 Agent 审查」变成真实模型调用的入口。 */
