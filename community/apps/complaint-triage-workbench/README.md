...
- Isolated-host replay: on 2026-09-19 the candidate repeated the workflow against the dedicated `complaint-stock-182f2f4` environment with a fresh database. Plugin `0.2.1` loaded, the DeepSeek official provider and `deepseek-v4-flash` Primary Copilot were configured, and fictitious cases passed create/validation, DRAFT reload, real structured AI analysis, review save/reload, confirmation/reload, controlled Primary-Copilot failure, and Retry on the same case without increasing the case count. SOURCE: USER confirmations; this is manual acceptance, not an automated end-to-end suite. The host still contains the separately disclosed Windows/Auth compatibility fixes, so this is not an unmodified-upstream-host certification.
...
Codex assistance was used for source reconnaissance, implementation, test drafting and delivery. [Representative collaboration decisions](docs/ai-collaboration.md) cover failure propagation, dependency isolation and evidence review. The candidate confirmed Codex gpt5.6sol (GPT-5.6 Sol), the DeepSeek official provider, and Xpert model deepseek-v4-flash (SOURCE: USER); these names are not inferred from a reply or independently traced for every request. No credential is recorded. The acceptance evidence remains the repository diff, automated checks, lifecycle Harness result, and the separately recorded real-platform validation; generated code or an AI statement is not treated as proof of runtime success.
