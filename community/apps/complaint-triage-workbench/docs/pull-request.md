...
- Dedicated fresh-database replay on 2026-09-19: DeepSeek official `deepseek-v4-flash`, create/reload, real structured analysis, human save/confirm/reload, controlled failure, and same-case Retry without a duplicate passed by candidate confirmation. The source-built host includes disclosed local Windows/Auth fixes and is not claimed as an unmodified-host pass.
...
- README directly embeds real input/AI/confirmed/failure images with captions and relative paths. Product scope and retrospective wireframes, source/AGPL reuse and representative Codex decisions are in product.md, source-reuse.md and ai-collaboration.md. Candidate-reported models: Codex gpt5.6sol (GPT-5.6 Sol), DeepSeek official provider, Xpert deepseek-v4-flash; no per-request API trace or credential is claimed.
