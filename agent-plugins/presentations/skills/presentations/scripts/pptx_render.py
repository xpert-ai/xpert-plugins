"""Independent PPTX -> PDF -> PNG pipeline with fresh output and bounded work."""
import hashlib
...
def render_deck(source, output, executable, dpi=120):
    from pptx_package import Package
    package = Package(source)
    package.assert_renderable()
    ...
    with tempfile.TemporaryDirectory(prefix='xpert-presentations-', dir=output.parent) as temp:
        temp = Path(temp)
        copied = temp / 'input.pptx'
        ...
        command = [executable, '-env:UserInstallation=' + profile.as_uri(), '--headless',
                   ...
                   'pdf:impress_pdf_Export', '--outdir', str(temp), str(copied)]
        ...
        stage = temp / 'result'
        stage.mkdir()
        shutil.copyfile(pdf_path, stage / 'presentation.pdf')
        pages = []
        with closing(pdfium.PdfDocument(str(pdf_path))) as pdf:
            if len(pdf) != len(package.slides):
                raise ValueError('Rendered page count differs from PPTX slide count')
        ...
        receipt = {'input': workspace_path(source), 'sha256': digest, 'pageCount': len(pages),
                   'pdf': workspace_path(output / 'presentation.pdf'), 'pages': pages,
                   'slideCount': len(package.slides), 'visualReview': 'pending'}
