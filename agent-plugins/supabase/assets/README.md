# Plugin icon

`icon.png` is copied unchanged from the Codex supabase plugin in
[openai/plugins](https://github.com/openai/plugins/tree/1dc195897af4161d039b80d8471ec0a10c9bbc89/plugins/supabase).

- Upstream plugin version: `1.0.0`
- Upstream commit: `1dc195897af4161d039b80d8471ec0a10c9bbc89`
- Asset: [`assets/logo.png`](https://raw.githubusercontent.com/openai/plugins/1dc195897af4161d039b80d8471ec0a10c9bbc89/plugins/supabase/assets/logo.png) (`interface.logo`)
- Upstream author: Supabase
- Upstream plugin manifest license declaration: MIT
- SHA-256: `3fba9515ad8fc71691380d71f2f414285176a077c09eb2e55e170fe5c1c720a7`

This brand asset identifies the connected service. It is separate from the
independently authored Xpert preset; no upstream Skill or runtime code is copied.

`plugin.json` embeds the same bytes in
`extensions.xpertai.interface.icon` as a data URL, so the imported package
does not need an external image request. Preserve the asset bytes and update
the data URL together when replacing the icon.

The upstream MIT notice is preserved in [LICENSE.txt](LICENSE.txt).
