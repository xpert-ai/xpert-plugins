...
for (const id of ['documents', 'pdf', 'presentations']) test(`${id} packages helpers and references and installs without an MCP server`, async () => {
...
test("document presets require explicit presentation with the paths-only host tool", async () => {
  for (const id of ["documents", "pdf", "presentations"]) {
