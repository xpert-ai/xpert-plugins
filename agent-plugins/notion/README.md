...
Version 1.1.0 declares `xpertai.connectors.notion` for the generic MCP OAuth Connector. Use the installer `--replace` option to migrate an existing legacy binding; old conversations keep the old version. Authorize the new Connector once; subsequent matching package versions reuse it.
