...
| ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| Standard Agent Plugins 1.0.0 root `plugin.json`, `skills/`, `mcp.json` | Portable components can be imported if they satisfy the supported schema and transports | Check each Skill's tools and host assumptions; display metadata may need `cn.xpertai` |
...
| Top-level `interface`, local `assets/` paths and OpenAI display metadata                                                                                                      | `extensions["cn.xpertai"].interface`                                                        | Only display name and icon are projected. Icon uses a URL to upstream `notion-small.svg`; no upstream asset file is bundled. Categories, screenshots, brand colors and default prompts are not mapped                                                                                                             |
...
4. Replace host-specific connection instructions and reconcile tool names and
   ...
   `Notion:` is enough.
5. Extend the packaging allowlist for the reviewed reference/license/assets files;
   the current quickstart packer intentionally permits only its minimal files.
...
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| Notion 1.1.0   | Keeps `https://mcp.notion.com/mcp`; adds `cn.xpertai.connectors.notion` with `type: mcp_oauth`; removes legacy installer OAuth selection     | Generic workspace OAuth Connector                                |
